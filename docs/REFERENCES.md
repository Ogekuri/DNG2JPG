...
# dng2jpg.py | Python | 9853L | 282 symbols | 24 imports | 201 comments
...
- var `DEFAULT_LUMINANCE_HDR_MODEL = "debevec"` (L98)
- var `DEFAULT_LUMINANCE_HDR_WEIGHT = "flat"` (L99)
- var `DEFAULT_LUMINANCE_HDR_RESPONSE_CURVE = "srgb"` (L100)
- var `DEFAULT_LUMINANCE_TMO = "mantiuk08"` (L101)
- var `DEFAULT_AUTO_ADJUST_ENABLED = True` (L102)
- var `HDR_MERGE_MODE_LUMINANCE = "Luminace-HDR"` (L103)
- var `HDR_MERGE_MODE_OPENCV = "OpenCV"` (L104)
- var `HDR_MERGE_MODE_HDR_PLUS = "HDR-Plus"` (L105)
- var `OPENCV_MERGE_ALGORITHM_DEBEVEC = "Debevec"` (L106)
- var `OPENCV_MERGE_ALGORITHM_ROBERTSON = "Robertson"` (L107)
- var `OPENCV_MERGE_ALGORITHM_MERTENS = "Mertens"` (L108)
- var `DEFAULT_REINHARD02_BRIGHTNESS = 1.25` (L109)
- var `DEFAULT_REINHARD02_CONTRAST = 0.85` (L110)
- var `DEFAULT_REINHARD02_SATURATION = 0.55` (L111)
- var `DEFAULT_MANTIUK08_CONTRAST = 1.2` (L112)
- var `DEFAULT_OPENCV_POST_GAMMA = 1.0` (L113)
- var `DEFAULT_OPENCV_BRIGHTNESS = 1.0` (L114)
- var `DEFAULT_OPENCV_CONTRAST = 1.0` (L115)
- var `DEFAULT_OPENCV_SATURATION = 1.0` (L116)
- var `DEFAULT_OPENCV_MERGE_ALGORITHM = OPENCV_MERGE_ALGORITHM_ROBERTSON` (L117)
- var `DEFAULT_OPENCV_TONEMAP_ENABLED = True` (L118)
- var `DEFAULT_OPENCV_TONEMAP_GAMMA = 2.2` (L119)
- var `DEFAULT_HDRPLUS_PROXY_MODE = "rggb"` (L120)
- var `DEFAULT_HDRPLUS_SEARCH_RADIUS = 4` (L121)
- var `DEFAULT_HDRPLUS_TEMPORAL_FACTOR = 8.0` (L122)
- var `DEFAULT_HDRPLUS_TEMPORAL_MIN_DIST = 10.0` (L123)
- var `DEFAULT_HDRPLUS_TEMPORAL_MAX_DIST = 300.0` (L124)
- var `HDRPLUS_TILE_SIZE = 32` (L125)
- var `HDRPLUS_TILE_STRIDE = HDRPLUS_TILE_SIZE // 2` (L126)
- var `HDRPLUS_DOWNSAMPLED_TILE_SIZE = HDRPLUS_TILE_STRIDE` (L127)
- var `HDRPLUS_ALIGNMENT_LEVELS = 3` (L128)
- var `HDRPLUS_ALIGNMENT_DOWNSAMPLE_RATE = 4` (L129)
- var `HDRPLUS_TEMPORAL_FACTOR = DEFAULT_HDRPLUS_TEMPORAL_FACTOR` (L130)
- var `HDRPLUS_TEMPORAL_MIN_DIST = DEFAULT_HDRPLUS_TEMPORAL_MIN_DIST` (L131)
- var `HDRPLUS_TEMPORAL_MAX_DIST = DEFAULT_HDRPLUS_TEMPORAL_MAX_DIST` (L132)
- var `EV_STEP = 0.25` (L134)
- var `MIN_SUPPORTED_BITS_PER_COLOR = 9` (L135)
- var `DEFAULT_DNG_BITS_PER_COLOR = 14` (L136)
- var `SUPPORTED_EV_VALUES = tuple(` (L137)
### class `class AutoAdjustOptions` `@dataclass(frozen=True)` (L357-392)
...
### class `class AutoBrightnessOptions` `@dataclass(frozen=True)` (L394-422)
...
### class `class AutoLevelsOptions` `@dataclass(frozen=True)` (L424-447)
...
### class `class PostprocessOptions` `@dataclass(frozen=True)` (L449-485)
...
### class `class DebugArtifactContext` `@dataclass(frozen=True)` (L487-503)
...
### class `class SourceGammaInfo` `@dataclass(frozen=True)` (L505-523)
...
### class `class LuminanceOptions` `@dataclass(frozen=True)` (L525-545)
...
### class `class OpenCvMergeOptions` `@dataclass(frozen=True)` (L547-567)
...
### class `class HdrPlusOptions` `@dataclass(frozen=True)` (L569-592)
...
### class `class HdrPlusTemporalRuntimeOptions` `@dataclass(frozen=True)` (L594-613)
...
### class `class AutoEvBracketingParams` `@dataclass(frozen=True)` (L615-644)
- @brief Hold deterministic `--auto-ev` parameter values.
- @details Encapsulates the refactored automatic bracketing solver constants. The solver scans signed EV candidates inside the `BASE_MAX` interval using `ev_step`, evaluates clipped safety after EV scaling on the normalized linear RGB base image, and conservatively quantizes the returned triplet on `ev_resolution`.
...
### class `class AutoEvBracketingSolution` `@dataclass(frozen=True)` (L646-669)
- @brief Hold one automatic EV bracketing triplet.
- @details Stores the selected automatic triplet `(ev_minus, ev_zero, ev_plus)` together with the safe signed limits and the deterministic solver parameters used to derive it from the normalized linear RGB base image.
...
### fn `def _print_box_table(headers, rows, header_rows=())` `priv` (L670-706)
...
### fn `def _border(left, middle, right)` `priv` (L690-692)
- @brief Print one Unicode box-drawing table.
...
### fn `def _line(values)` `priv` (L693-696)

### fn `def _build_two_line_operator_rows(operator_entries)` `priv` (L707-723)
...
### fn `def _print_help_section(title)` `priv` (L724-738)
...
### fn `def _print_help_option(option_label, description, detail_lines=())` `priv` (L739-780)
...
### fn `def print_help(version)` (L781-980)
...
### fn `def _calculate_max_ev_from_bits(bits_per_color)` `priv` (L1139-1157)
...
### fn `def _calculate_safe_ev_zero_max(base_max_ev)` `priv` (L1158-1170)
...
### fn `def _derive_supported_ev_values(` `priv` (L1171-1174)

### fn `def _detect_dng_bits_per_color(raw_handle)` `priv` (L1215-1260)
...
### fn `def _is_ev_value_on_supported_step(ev_value)` `priv` (L1261-1274)
...
### fn `def _parse_ev_option(ev_raw)` `priv` (L1275-1306)
...
### fn `def _parse_ev_zero_option(ev_zero_raw)` `priv` (L1307-1337)
...
### fn `def _parse_auto_ev_option(auto_ev_raw)` `priv` (L1338-1357)
...
### fn `def _parse_auto_brightness_option(auto_brightness_raw)` `priv` (L1358-1377)
...
### fn `def _parse_auto_levels_option(auto_levels_raw)` `priv` (L1378-1397)
...
### fn `def _parse_explicit_boolean_option(option_name, option_raw)` `priv` (L1398-1418)
...
### fn `def _parse_opencv_merge_algorithm_option(algorithm_raw)` `priv` (L1419-1443)
...
### fn `def _parse_opencv_options(opencv_raw_values)` `priv` (L1444-1490)
...
### fn `def _quantize_ev_to_resolution(ev_value, resolution)` `priv` (L1491-1505)
- @brief Quantize one EV value to the nearest configured resolution.
- @details Converts the input EV scalar to integer resolution units, rounds to the nearest unit, and reconstructs the quantized EV value with deterministic two-decimal output precision.
...
### fn `def _max_symmetric_half_range(center_ev, safe_ev_min, safe_ev_max)` `priv` (L1506-1520)
- @brief Compute the maximum symmetric half-range around one EV center.
- @details Returns the largest half-range that keeps `(center-half, center+half)` inside the discovered safe interval.
...
### fn `def _extract_base_rgb_linear_float(raw_handle, np_module)` `priv` (L1521-1575)
- @brief Extract one linear normalized RGB base image from one RAW handle.
- @details Executes exactly one `rawpy.postprocess` call with deterministic parameters `bright=1.0`, `output_bps=16`, `use_camera_wb=True`, `no_auto_bright=True`, `gamma=(1.0,1.0)`, and `user_flip=0`, then normalizes the demosaiced maximum-resolution RGB output to float `[0,1]`. When RAW metadata exposes one positive `white_level`, normalization clamps against that effective sensor ceiling instead of the wider integer container range so automatic EV analysis observes the same scene dynamic range implied by the RAW metadata. Complexity: O(H*W). Side effects: one RAW postprocess invocation.
- @param raw_handle {Any} Opened RAW handle from `rawpy.imread`.
...
### fn `def _normalize_source_gamma_label(label_raw)` `priv` (L1576-1594)
...
### fn `def _decode_raw_metadata_text(metadata_raw)` `priv` (L1595-1626)
...
### fn `def _classify_explicit_source_gamma(raw_handle)` `priv` (L1627-1676)
...
### fn `def _classify_tone_curve_gamma(raw_handle)` `priv` (L1677-1725)
...
### fn `def _has_nonzero_matrix(matrix_raw)` `priv` (L1726-1746)
...
### fn `def _classify_matrix_hint_gamma(raw_handle)` `priv` (L1747-1777)
...
### fn `def _extract_source_gamma_info(raw_handle)` `priv` (L1778-1805)
...
### fn `def _describe_source_gamma_info(source_gamma_info)` `priv` (L1806-1827)
...
### fn `def _calculate_bt709_luminance(np_module, image_rgb_float)` `priv` (L1828-1850)
...
### fn `def _smoothstep(np_module, values, edge0, edge1)` `priv` (L1851-1869)
...
### fn `def _is_auto_ev_candidate_clipped(np_module, image_rgb_float, exposure_ev, params)` `priv` (L1870-1928)
- @brief Classify one automatic EV candidate as safe or clipped.
- @details Applies `2**exposure_ev` gain to the normalized linear RGB base image, clamps the simulated image to `[0,1]`, then rejects the candidate when BT.709 luminance or any RGB channel exceeds the configured hard or soft clipping fractions. Complexity: `O(H*W)`. Side effects: none.
- @param np_module {ModuleType} Imported numpy module.
...
### fn `def _fraction(mask)` `priv` (L1902-1904)
- @brief Classify one automatic EV candidate as safe or clipped.
...
### fn `def _compute_auto_ev_bracketing_solution(np_module, base_rgb_float, base_max_ev, params)` `priv` (L1929-2029)
- @brief Compute one automatic EV bracketing triplet from the linear base image.
- @details Ports the `/tmp/auto-ev.py::compute_bracketing_ev` algorithm to the repository RGB float contract. Scans signed EV candidates inside `[-BASE_MAX,+BASE_MAX]`, records the last safe negative and positive exposures, derives the center from the safe interval midpoint, selects the resolution-quantized center that preserves the larger symmetric half-range, then conservatively quantizes the final symmetric triplet inside the safe interval. Complexity: `O(K*H*W)` where `K` is the number of scanned EV candidates. Side effects: none.
- @param np_module {ModuleType} Imported numpy module.
...
### fn `def _resolve_auto_ev_bracketing_solution(np_module, base_rgb_float, base_max_ev)` `priv` (L2030-2076)
- @brief Resolve the automatic EV triplet and emit deterministic diagnostics.
- @details Instantiates the deterministic `--auto-ev` parameter set, computes the automatic safe interval and the final symmetric triplet from the normalized linear RGB base image, and emits the required runtime diagnostics describing parameters, safe limits, and selected values.
- @param np_module {ModuleType} Imported numpy module.
- @param base_rgb_float {object} Normalized linear RGB base image on `[0,1]`.
- @param base_max_ev {float} Bit-derived `BASE_MAX` ceiling.
...
### fn `def _parse_luminance_text_option(option_name, option_raw)` `priv` (L2077-2097)
...
### fn `def _parse_positive_float_option(option_name, option_raw)` `priv` (L2098-2121)
...
### fn `def _parse_positive_int_option(option_name, option_raw)` `priv` (L2122-2145)
...
### fn `def _parse_tmo_passthrough_value(option_name, option_raw)` `priv` (L2146-2162)
...
### fn `def _parse_jpg_compression_option(compression_raw)` `priv` (L2163-2185)
...
### fn `def _parse_float_exclusive_range_option(option_name, option_raw, min_value, max_value)` `priv` (L2186-2210)
...
### fn `def _parse_non_negative_float_option(option_name, option_raw)` `priv` (L2211-2233)
...
### fn `def _parse_float_in_range_option(option_name, option_raw, min_value, max_value)` `priv` (L2234-2259)
...
### fn `def _parse_positive_int_pair_option(option_name, option_raw)` `priv` (L2260-2291)
...
### fn `def _parse_auto_brightness_options(auto_brightness_raw_values)` `priv` (L2292-2388)
...
### fn `def _parse_auto_levels_hr_method_option(auto_levels_method_raw)` `priv` (L2389-2420)
...
### fn `def _parse_auto_levels_options(auto_levels_raw_values)` `priv` (L2421-2493)
...
### fn `def _parse_auto_adjust_options(auto_adjust_raw_values)` `priv` (L2494-2643)
...
### fn `def _parse_hdrplus_proxy_mode_option(proxy_mode_raw)` `priv` (L2644-2662)
...
### fn `def _parse_hdrplus_options(hdrplus_raw_values)` `priv` (L2663-2739)
...
### fn `def _parse_auto_adjust_option(auto_adjust_raw)` `priv` (L2740-2763)
...
### fn `def _parse_hdr_merge_option(hdr_merge_raw)` `priv` (L2764-2793)
...
### fn `def _resolve_default_postprocess(` `priv` (L2794-2797)

### fn `def _parse_run_options(args)` `priv` (L2869-3068)
...
- @return {tuple[Path, Path, float|None, bool, PostprocessOptions, bool, bool, LuminanceOptions, OpenCvMergeOptions, HdrPlusOptions, bool, float]|None} Parsed `(input, output, ev, auto_ev, postprocess, enable_luminance, enable_opencv, luminance_options, opencv_merge_options, hdrplus_options, enable_hdr_plus, ev_zero)` tuple; `None` on parse failure.
...
### fn `def _load_image_dependencies()` `priv` (L3613-3650)
...
### fn `def _parse_exif_datetime_to_timestamp(datetime_raw)` `priv` (L3651-3681)
...
### fn `def _parse_exif_exposure_time_to_seconds(exposure_raw)` `priv` (L3682-3744)
...
### fn `def _extract_dng_exif_payload_and_timestamp(pil_image_module, input_dng)` `priv` (L3745-3839)
...
### fn `def _read_exif_value(exif_tag)` `priv` (L3789-3806)
- @brief Extract DNG EXIF payload bytes, preferred datetime timestamp, source orientation, and exposure time.
...
### fn `def _resolve_thumbnail_transpose_map(pil_image_module)` `priv` (L3840-3871)
...
### fn `def _apply_orientation_transform(pil_image_module, pil_image, source_orientation)` `priv` (L3872-3894)
...
### fn `def _build_oriented_thumbnail_jpeg_bytes(` `priv` (L3895-3896)

### fn `def _coerce_exif_int_like_value(raw_value)` `priv` (L3927-3969)
...
### fn `def _normalize_ifd_integer_like_values_for_piexif_dump(piexif_module, exif_dict)` `priv` (L3970-4103)
...
### fn `def _refresh_output_jpg_exif_thumbnail_after_save(` `priv` (L4104-4110)

### fn `def _set_output_file_timestamps(output_jpg, exif_timestamp)` `priv` (L4160-4174)
...
### fn `def _sync_output_file_timestamps_from_exif(output_jpg, exif_timestamp)` `priv` (L4175-4192)
...
### fn `def _build_exposure_multipliers(ev_minus, ev_zero, ev_plus)` `priv` (L4193-4212)
- @brief Compute bracketing brightness multipliers from explicit EV triplet.
- @details Produces exactly three multipliers mapped to exposure stops `[ev_minus, ev_zero, ev_plus]` as powers of two for float-domain HDR base-image scaling.
- @param ev_minus {float} Darkest EV value.
- @param ev_zero {float} Central EV value.
- @param ev_plus {float} Brightest EV value.
- @return {tuple[float, float, float]} Multipliers in order `(under, base, over)`.
- @satisfies REQ-009, REQ-159, REQ-160

### fn `def _build_bracket_images_from_linear_base_float(np_module, base_rgb_float, multipliers)` `priv` (L4213-4241)
...
### fn `def _extract_bracket_images_float(` `priv` (L4242-4246)

### fn `def _order_bracket_paths(bracket_paths)` `priv` (L4280-4305)
...
### fn `def _order_hdr_plus_reference_paths(bracket_paths)` `priv` (L4306-4321)
...
### fn `def _run_luminance_hdr_cli(` `priv` (L4322-4329)

### fn `def _build_opencv_radiance_exposure_times(` `priv` (L4393-4396)
...
### fn `def _build_ev_times_from_ev_zero_and_delta(ev_zero, ev_delta)` `priv` (L4430-4449)
...
### fn `def _normalize_opencv_hdr_to_unit_range(np_module, hdr_rgb_float32)` `priv` (L4450-4473)
...
### fn `def _run_opencv_merge_mertens(cv2_module, np_module, exposures_float)` `priv` (L4474-4495)
...
### fn `def _estimate_opencv_camera_response(` `priv` (L4496-4500)

### fn `def _run_opencv_merge_radiance(` `priv` (L4529-4536)
...
### fn `def _normalize_debevec_hdr_to_unit_range(np_module, hdr_rgb_float32, white_point_percentile)` `priv` (L4600-4619)
...
### fn `def _run_opencv_hdr_merge(` `priv` (L4620-4626)

### fn `def _hdrplus_box_down2_float32(np_module, frames_float32)` `priv` (L4687-4715)
...
### fn `def _hdrplus_gauss_down4_float32(np_module, frames_float32)` `priv` (L4716-4762)
...
### fn `def _hdrplus_build_scalar_proxy_float32(np_module, frames_rgb_float32, hdrplus_options)` `priv` (L4763-4796)
...
### fn `def _hdrplus_compute_tile_start_positions(np_module, axis_length, tile_stride, pad_margin)` `priv` (L4797-4817)
...
### fn `def _hdrplus_trunc_divide_int32(np_module, values_int32, divisor)` `priv` (L4818-4836)
...
### fn `def _hdrplus_compute_alignment_bounds(search_radius)` `priv` (L4837-4861)
...
### fn `def _hdrplus_compute_alignment_margin(search_radius, divisor=1)` `priv` (L4862-4880)
...
### fn `def _hdrplus_extract_overlapping_tiles(` `priv` (L4881-4886)

### fn `def _hdrplus_extract_aligned_tiles(` `priv` (L4939-4945)

### fn `def _hdrplus_align_layer(` `priv` (L5018-5025)
...
### fn `def _hdrplus_align_layers(np_module, scalar_frames, hdrplus_options)` `priv` (L5115-5202)
...
### fn `def _hdrplus_resolve_temporal_runtime_options(hdrplus_options)` `priv` (L5203-5227)
...
### fn `def _hdrplus_compute_temporal_weights(` `priv` (L5228-5232)

### fn `def _hdrplus_merge_temporal_rgb(` `priv` (L5313-5319)
...
### fn `def _hdrplus_merge_spatial_rgb(np_module, temporal_tiles, width, height)` `priv` (L5368-5440)
...
### fn `def _run_hdr_plus_merge(` `priv` (L5441-5444)

### fn `def _convert_compression_to_quality(jpg_compression)` `priv` (L5521-5531)
...
### fn `def _collect_missing_external_executables(` `priv` (L5532-5534)

### fn `def _resolve_auto_adjust_dependencies()` `priv` (L5553-5578)
...
### fn `def _resolve_numpy_dependency()` `priv` (L5579-5598)
...
### fn `def _to_float32_image_array(np_module, image_data)` `priv` (L5599-5630)
...
### fn `def _normalize_float_rgb_image(np_module, image_data)` `priv` (L5631-5658)
...
### fn `def _write_rgb_float_tiff16(imageio_module, np_module, output_path, image_rgb_float)` `priv` (L5659-5682)
...
### fn `def _write_debug_rgb_float_tiff(` `priv` (L5683-5688)

### fn `def _build_debug_artifact_context(output_jpg, input_dng, postprocess_options)` `priv` (L5718-5738)
...
### fn `def _format_debug_ev_suffix_value(ev_value)` `priv` (L5739-5756)
...
### fn `def _materialize_bracket_tiffs_from_float(` `priv` (L5757-5761)

### fn `def _to_uint8_image_array(np_module, image_data)` `priv` (L5791-5837)
...
### fn `def _to_uint16_image_array(np_module, image_data)` `priv` (L5838-5882)
...
### fn `def _apply_post_gamma_float(np_module, image_rgb_float, gamma_value)` `priv` (L5883-5908)
...
### fn `def _apply_brightness_float(np_module, image_rgb_float, brightness_factor)` `priv` (L5909-5931)
...
### fn `def _apply_contrast_float(np_module, image_rgb_float, contrast_factor)` `priv` (L5932-5956)
...
### fn `def _apply_saturation_float(np_module, image_rgb_float, saturation_factor)` `priv` (L5957-5986)
...
### fn `def _apply_static_postprocess_float(` `priv` (L5987-5992)

### fn `def _to_linear_srgb(np_module, image_srgb)` `priv` (L6069-6086)
...
### fn `def _from_linear_srgb(np_module, image_linear)` `priv` (L6087-6104)
...
### fn `def _compute_bt709_luminance(np_module, linear_rgb)` `priv` (L6105-6122)
...
### fn `def _analyze_luminance_key(np_module, luminance, eps)` `priv` (L6123-6162)
...
### fn `def _choose_auto_key_value(key_analysis, auto_brightness_options)` `priv` (L6163-6208)
...
### fn `def _reinhard_global_tonemap_luminance(` `priv` (L6209-6214)

### fn `def _luminance_preserving_desaturate_to_fit(np_module, rgb_linear, luminance, eps)` `priv` (L6248-6275)
...
### fn `def _apply_mild_local_contrast_bgr_uint16(cv2_module, np_module, image_bgr_uint16, options)` `priv` (L6276-6314)
...
### fn `def _quantize_clahe_luminance_bins(np_module, luminance_values, histogram_size)` `priv` (L6315-6340)
...
### fn `def _build_clahe_float_tile_histogram(np_module, luminance_tile, histogram_size)` `priv` (L6341-6362)
...
### fn `def _clip_clahe_histogram(np_module, histogram, clip_limit, tile_population)` `priv` (L6363-6410)
...
### fn `def _build_clahe_float_lut(np_module, histogram, tile_population)` `priv` (L6411-6430)
...
### fn `def _pad_clahe_luminance_float(np_module, luminance_float, tile_grid_size)` `priv` (L6431-6469)
...
### fn `def _build_clahe_axis_interpolation(np_module, axis_length, tile_size, tile_count)` `priv` (L6470-6497)
...
### fn `def _build_clahe_tile_luts_float(np_module, luminance_float, clip_limit, tile_grid_size, histogram_size)` `priv` (L6498-6549)
...
### fn `def _interpolate_clahe_bilinear_float(np_module, luminance_float, tile_luts, tile_height, tile_width)` `priv` (L6550-6602)
...
### fn `def _apply_clahe_luminance_float(np_module, luminance_float, clip_limit, tile_grid_size)` `priv` (L6603-6634)
...
### fn `def _reconstruct_rgb_from_ycrcb_luma_float(cv2_module, np_module, luminance_float, cr_channel, cb_channel)` `priv` (L6635-6658)
...
### fn `def _apply_clahe_luma_rgb_float(cv2_module, np_module, image_rgb_float, auto_adjust_options)` `priv` (L6659-6708)
...
### fn `def _rt_gamma2(np_module, values)` `priv` (L6709-6728)
...
### fn `def _rt_igamma2(np_module, values)` `priv` (L6729-6749)
...
### fn `def _auto_levels_index_to_normalized_value(histogram_value, histcompr)` `priv` (L6750-6766)
...
### fn `def _auto_levels_normalized_to_legacy_code_value(value)` `priv` (L6767-6782)
...
### fn `def _auto_levels_normalized_to_legacy_code(np_module, values)` `priv` (L6783-6799)
...
### fn `def _auto_levels_legacy_code_to_normalized(np_module, values)` `priv` (L6800-6815)
...
### fn `def _pack_auto_levels_metrics(` `priv` (L6816-6831)

### fn `def _build_autoexp_histogram_rgb_float(np_module, image_rgb_float, histcompr)` `priv` (L6883-6918)
...
### fn `def _build_autoexp_histogram_rgb_uint16(np_module, image_rgb_uint16, histcompr)` `priv` (L6919-6951)
...
### fn `def _compute_auto_levels_from_histogram(np_module, histogram, histcompr, clip_percent)` `priv` (L6952-7151)
...
### fn `def _rt_simplebasecurve_scalar(x_value, black, shadow_recovery)` `priv` (L7225-7317)
...
### fn `def _basel(x_input, slope_start, slope_end)` `priv` (L7238-7254)
- @brief Evaluate RawTherapee `simplebasecurve` for one normalized sample.
...
### fn `def _baseu(x_input, slope_start, slope_end)` `priv` (L7255-7257)

### fn `def _cupper(x_input, slope_value, highlight_recovery)` `priv` (L7258-7277)

### fn `def _clower(x_input, slope_value, shadow_value)` `priv` (L7278-7280)

### fn `def _clower2(x_input, slope_value, shadow_value)` `priv` (L7281-7292)

### fn `def _build_rt_nurbs_curve_lut(np_module, x_points, y_points, sample_count)` `priv` (L7318-7451)
...
### fn `def _sample_auto_levels_lut_float(` `priv` (L7452-7458)

### fn `def _build_auto_levels_full_histogram_rgb_float(np_module, image_rgb_float)` `priv` (L7490-7521)
...
### fn `def _rt_hlcurve_float(np_module, exp_scale, comp, hlrange, levels_code)` `priv` (L7522-7552)
...
### fn `def _build_auto_levels_tone_curve_state(np_module, image_rgb_float, auto_levels_metrics)` `priv` (L7553-7742)
...
### fn `def _apply_auto_levels_tonal_transform_float(` `priv` (L7743-7746)

### fn `def _auto_levels_has_full_tone_metrics(auto_levels_metrics)` `priv` (L7830-7853)
...
### fn `def _call_auto_levels_compat_helper(` `priv` (L7854-7859)

### fn `def _apply_auto_levels_float(np_module, image_rgb_float, auto_levels_options)` `priv` (L7903-8009)
...
### fn `def _clip_auto_levels_out_of_gamut_float(np_module, image_rgb, maxval=1.0)` `priv` (L8010-8170)
...
### fn `def _filmlike_clip_rgb_tone(primary, middle, lower)` `priv` (L8040-8073)
- @brief Clip overflowing RGB triplets with RawTherapee film-like gamut logic.
...
### fn `def _clip_auto_levels_out_of_gamut_uint16(` `priv` (L8171-8172)

### fn `def _hlrecovery_luminance_float(np_module, image_rgb, maxval=1.0)` `priv` (L8202-8248)
...
### fn `def _hlrecovery_luminance_uint16(np_module, image_rgb, maxval=_AUTO_LEVELS_CODE_MAX)` `priv` (L8249-8277)
...
### fn `def _hlrecovery_cielab_float(` `priv` (L8278-8279)

### fn `def _f_lab(values)` `priv` (L8312-8319)
...
### fn `def _f2xyz(values)` `priv` (L8320-8326)

### fn `def _hlrecovery_cielab_uint16(` `priv` (L8362-8363)

### fn `def _hlrecovery_blend_float(np_module, image_rgb, hlmax, maxval=1.0)` `priv` (L8397-8502)
...
### fn `def _hlrecovery_blend_uint16(` `priv` (L8503-8504)

### fn `def _dilate_mask_float(np_module, mask)` `priv` (L8539-8561)
...
### fn `def _box_mean_3x3_float(np_module, image_2d)` `priv` (L8562-8585)
...
### fn `def _hlrecovery_color_propagation_float(np_module, image_rgb, maxval=1.0)` `priv` (L8586-8630)
...
### fn `def _hlrecovery_color_propagation_uint16(` `priv` (L8631-8632)

### fn `def _hlrecovery_inpaint_opposed_float(` `priv` (L8662-8663)
...
### fn `def _hlrecovery_inpaint_opposed_uint16(` `priv` (L8716-8717)
...
### fn `def _apply_auto_brightness_rgb_float(` `priv` (L8759-8762)
...
### fn `def _clamp01(np_module, values)` `priv` (L8819-8832)
...
### fn `def _gaussian_kernel_2d(np_module, sigma, radius=None)` `priv` (L8833-8855)
...
### fn `def _rgb_to_hsl(np_module, rgb)` `priv` (L8856-8889)
...
### fn `def _hue_to_rgb(np_module, p_values, q_values, t_values)` `priv` (L8890-8920)
...
### fn `def _hsl_to_rgb(np_module, hue, saturation, lightness)` `priv` (L8921-8961)
...
### fn `def _selective_blur_contrast_gated_vectorized(` `priv` (L8962-8963)

### fn `def _level_per_channel_adaptive(np_module, rgb, low_pct=0.1, high_pct=99.9)` `priv` (L9012-9034)
...
### fn `def _sigmoidal_contrast(np_module, rgb, contrast=3.0, midpoint=0.5)` `priv` (L9035-9059)
...
### fn `def logistic(z_values)` (L9050-9052)
- @brief Execute sigmoidal contrast stage.
...
### fn `def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=0.8)` `priv` (L9060-9077)
...
### fn `def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)` `priv` (L9078-9101)
...
### fn `def _high_pass_math_gray(cv2_module, np_module, rgb, blur_sigma=2.5)` `priv` (L9102-9125)
...
### fn `def _overlay_composite(np_module, base_rgb, overlay_gray)` `priv` (L9126-9147)
...
### fn `def _apply_validated_auto_adjust_pipeline(` `priv` (L9148-9154)

### fn `def _load_piexif_dependency()` `priv` (L9263-9280)
...
### fn `def _encode_jpg(` `priv` (L9281-9292)

### fn `def _collect_processing_errors(rawpy_module)` `priv` (L9424-9452)
...
### fn `def _is_supported_runtime_os()` `priv` (L9453-9472)
...
|`DEFAULT_LUMINANCE_HDR_MODEL`|var|pub|98||
|`DEFAULT_LUMINANCE_HDR_WEIGHT`|var|pub|99||
|`DEFAULT_LUMINANCE_HDR_RESPONSE_CURVE`|var|pub|100||
|`DEFAULT_LUMINANCE_TMO`|var|pub|101||
|`DEFAULT_AUTO_ADJUST_ENABLED`|var|pub|102||
|`HDR_MERGE_MODE_LUMINANCE`|var|pub|103||
|`HDR_MERGE_MODE_OPENCV`|var|pub|104||
|`HDR_MERGE_MODE_HDR_PLUS`|var|pub|105||
|`OPENCV_MERGE_ALGORITHM_DEBEVEC`|var|pub|106||
|`OPENCV_MERGE_ALGORITHM_ROBERTSON`|var|pub|107||
|`OPENCV_MERGE_ALGORITHM_MERTENS`|var|pub|108||
|`DEFAULT_REINHARD02_BRIGHTNESS`|var|pub|109||
|`DEFAULT_REINHARD02_CONTRAST`|var|pub|110||
|`DEFAULT_REINHARD02_SATURATION`|var|pub|111||
|`DEFAULT_MANTIUK08_CONTRAST`|var|pub|112||
|`DEFAULT_OPENCV_POST_GAMMA`|var|pub|113||
|`DEFAULT_OPENCV_BRIGHTNESS`|var|pub|114||
|`DEFAULT_OPENCV_CONTRAST`|var|pub|115||
|`DEFAULT_OPENCV_SATURATION`|var|pub|116||
|`DEFAULT_OPENCV_MERGE_ALGORITHM`|var|pub|117||
|`DEFAULT_OPENCV_TONEMAP_ENABLED`|var|pub|118||
|`DEFAULT_OPENCV_TONEMAP_GAMMA`|var|pub|119||
|`DEFAULT_HDRPLUS_PROXY_MODE`|var|pub|120||
|`DEFAULT_HDRPLUS_SEARCH_RADIUS`|var|pub|121||
|`DEFAULT_HDRPLUS_TEMPORAL_FACTOR`|var|pub|122||
|`DEFAULT_HDRPLUS_TEMPORAL_MIN_DIST`|var|pub|123||
|`DEFAULT_HDRPLUS_TEMPORAL_MAX_DIST`|var|pub|124||
|`HDRPLUS_TILE_SIZE`|var|pub|125||
|`HDRPLUS_TILE_STRIDE`|var|pub|126||
|`HDRPLUS_DOWNSAMPLED_TILE_SIZE`|var|pub|127||
|`HDRPLUS_ALIGNMENT_LEVELS`|var|pub|128||
|`HDRPLUS_ALIGNMENT_DOWNSAMPLE_RATE`|var|pub|129||
|`HDRPLUS_TEMPORAL_FACTOR`|var|pub|130||
|`HDRPLUS_TEMPORAL_MIN_DIST`|var|pub|131||
|`HDRPLUS_TEMPORAL_MAX_DIST`|var|pub|132||
|`EV_STEP`|var|pub|134||
|`MIN_SUPPORTED_BITS_PER_COLOR`|var|pub|135||
|`DEFAULT_DNG_BITS_PER_COLOR`|var|pub|136||
|`SUPPORTED_EV_VALUES`|var|pub|137||
|`AutoAdjustOptions`|class|pub|357-392|class AutoAdjustOptions|
|`AutoBrightnessOptions`|class|pub|394-422|class AutoBrightnessOptions|
|`AutoLevelsOptions`|class|pub|424-447|class AutoLevelsOptions|
|`PostprocessOptions`|class|pub|449-485|class PostprocessOptions|
|`DebugArtifactContext`|class|pub|487-503|class DebugArtifactContext|
|`SourceGammaInfo`|class|pub|505-523|class SourceGammaInfo|
|`LuminanceOptions`|class|pub|525-545|class LuminanceOptions|
|`OpenCvMergeOptions`|class|pub|547-567|class OpenCvMergeOptions|
|`HdrPlusOptions`|class|pub|569-592|class HdrPlusOptions|
|`HdrPlusTemporalRuntimeOptions`|class|pub|594-613|class HdrPlusTemporalRuntimeOptions|
|`AutoEvBracketingParams`|class|pub|615-644|class AutoEvBracketingParams|
|`AutoEvBracketingSolution`|class|pub|646-669|class AutoEvBracketingSolution|
|`_print_box_table`|fn|priv|670-706|def _print_box_table(headers, rows, header_rows=())|
|`_border`|fn|priv|690-692|def _border(left, middle, right)|
|`_line`|fn|priv|693-696|def _line(values)|
|`_build_two_line_operator_rows`|fn|priv|707-723|def _build_two_line_operator_rows(operator_entries)|
|`_print_help_section`|fn|priv|724-738|def _print_help_section(title)|
|`_print_help_option`|fn|priv|739-780|def _print_help_option(option_label, description, detail_...|
|`print_help`|fn|pub|781-980|def print_help(version)|
|`_calculate_max_ev_from_bits`|fn|priv|1139-1157|def _calculate_max_ev_from_bits(bits_per_color)|
|`_calculate_safe_ev_zero_max`|fn|priv|1158-1170|def _calculate_safe_ev_zero_max(base_max_ev)|
|`_derive_supported_ev_values`|fn|priv|1171-1174|def _derive_supported_ev_values(|
|`_detect_dng_bits_per_color`|fn|priv|1215-1260|def _detect_dng_bits_per_color(raw_handle)|
|`_is_ev_value_on_supported_step`|fn|priv|1261-1274|def _is_ev_value_on_supported_step(ev_value)|
|`_parse_ev_option`|fn|priv|1275-1306|def _parse_ev_option(ev_raw)|
|`_parse_ev_zero_option`|fn|priv|1307-1337|def _parse_ev_zero_option(ev_zero_raw)|
|`_parse_auto_ev_option`|fn|priv|1338-1357|def _parse_auto_ev_option(auto_ev_raw)|
|`_parse_auto_brightness_option`|fn|priv|1358-1377|def _parse_auto_brightness_option(auto_brightness_raw)|
|`_parse_auto_levels_option`|fn|priv|1378-1397|def _parse_auto_levels_option(auto_levels_raw)|
|`_parse_explicit_boolean_option`|fn|priv|1398-1418|def _parse_explicit_boolean_option(option_name, option_raw)|
|`_parse_opencv_merge_algorithm_option`|fn|priv|1419-1443|def _parse_opencv_merge_algorithm_option(algorithm_raw)|
|`_parse_opencv_options`|fn|priv|1444-1490|def _parse_opencv_options(opencv_raw_values)|
|`_quantize_ev_to_resolution`|fn|priv|1491-1505|def _quantize_ev_to_resolution(ev_value, resolution)|
|`_max_symmetric_half_range`|fn|priv|1506-1520|def _max_symmetric_half_range(center_ev, safe_ev_min, saf...|
|`_extract_base_rgb_linear_float`|fn|priv|1521-1575|def _extract_base_rgb_linear_float(raw_handle, np_module)|
|`_normalize_source_gamma_label`|fn|priv|1576-1594|def _normalize_source_gamma_label(label_raw)|
|`_decode_raw_metadata_text`|fn|priv|1595-1626|def _decode_raw_metadata_text(metadata_raw)|
|`_classify_explicit_source_gamma`|fn|priv|1627-1676|def _classify_explicit_source_gamma(raw_handle)|
|`_classify_tone_curve_gamma`|fn|priv|1677-1725|def _classify_tone_curve_gamma(raw_handle)|
|`_has_nonzero_matrix`|fn|priv|1726-1746|def _has_nonzero_matrix(matrix_raw)|
|`_classify_matrix_hint_gamma`|fn|priv|1747-1777|def _classify_matrix_hint_gamma(raw_handle)|
|`_extract_source_gamma_info`|fn|priv|1778-1805|def _extract_source_gamma_info(raw_handle)|
|`_describe_source_gamma_info`|fn|priv|1806-1827|def _describe_source_gamma_info(source_gamma_info)|
|`_calculate_bt709_luminance`|fn|priv|1828-1850|def _calculate_bt709_luminance(np_module, image_rgb_float)|
|`_smoothstep`|fn|priv|1851-1869|def _smoothstep(np_module, values, edge0, edge1)|
|`_is_auto_ev_candidate_clipped`|fn|priv|1870-1928|def _is_auto_ev_candidate_clipped(np_module, image_rgb_fl...|
|`_fraction`|fn|priv|1902-1904|def _fraction(mask)|
|`_compute_auto_ev_bracketing_solution`|fn|priv|1929-2029|def _compute_auto_ev_bracketing_solution(np_module, base_...|
|`_resolve_auto_ev_bracketing_solution`|fn|priv|2030-2076|def _resolve_auto_ev_bracketing_solution(np_module, base_...|
|`_parse_luminance_text_option`|fn|priv|2077-2097|def _parse_luminance_text_option(option_name, option_raw)|
|`_parse_positive_float_option`|fn|priv|2098-2121|def _parse_positive_float_option(option_name, option_raw)|
|`_parse_positive_int_option`|fn|priv|2122-2145|def _parse_positive_int_option(option_name, option_raw)|
|`_parse_tmo_passthrough_value`|fn|priv|2146-2162|def _parse_tmo_passthrough_value(option_name, option_raw)|
|`_parse_jpg_compression_option`|fn|priv|2163-2185|def _parse_jpg_compression_option(compression_raw)|
|`_parse_float_exclusive_range_option`|fn|priv|2186-2210|def _parse_float_exclusive_range_option(option_name, opti...|
|`_parse_non_negative_float_option`|fn|priv|2211-2233|def _parse_non_negative_float_option(option_name, option_...|
|`_parse_float_in_range_option`|fn|priv|2234-2259|def _parse_float_in_range_option(option_name, option_raw,...|
|`_parse_positive_int_pair_option`|fn|priv|2260-2291|def _parse_positive_int_pair_option(option_name, option_raw)|
|`_parse_auto_brightness_options`|fn|priv|2292-2388|def _parse_auto_brightness_options(auto_brightness_raw_va...|
|`_parse_auto_levels_hr_method_option`|fn|priv|2389-2420|def _parse_auto_levels_hr_method_option(auto_levels_metho...|
|`_parse_auto_levels_options`|fn|priv|2421-2493|def _parse_auto_levels_options(auto_levels_raw_values)|
|`_parse_auto_adjust_options`|fn|priv|2494-2643|def _parse_auto_adjust_options(auto_adjust_raw_values)|
|`_parse_hdrplus_proxy_mode_option`|fn|priv|2644-2662|def _parse_hdrplus_proxy_mode_option(proxy_mode_raw)|
|`_parse_hdrplus_options`|fn|priv|2663-2739|def _parse_hdrplus_options(hdrplus_raw_values)|
|`_parse_auto_adjust_option`|fn|priv|2740-2763|def _parse_auto_adjust_option(auto_adjust_raw)|
|`_parse_hdr_merge_option`|fn|priv|2764-2793|def _parse_hdr_merge_option(hdr_merge_raw)|
|`_resolve_default_postprocess`|fn|priv|2794-2797|def _resolve_default_postprocess(|
|`_parse_run_options`|fn|priv|2869-3068|def _parse_run_options(args)|
|`_load_image_dependencies`|fn|priv|3613-3650|def _load_image_dependencies()|
|`_parse_exif_datetime_to_timestamp`|fn|priv|3651-3681|def _parse_exif_datetime_to_timestamp(datetime_raw)|
|`_parse_exif_exposure_time_to_seconds`|fn|priv|3682-3744|def _parse_exif_exposure_time_to_seconds(exposure_raw)|
|`_extract_dng_exif_payload_and_timestamp`|fn|priv|3745-3839|def _extract_dng_exif_payload_and_timestamp(pil_image_mod...|
|`_read_exif_value`|fn|priv|3789-3806|def _read_exif_value(exif_tag)|
|`_resolve_thumbnail_transpose_map`|fn|priv|3840-3871|def _resolve_thumbnail_transpose_map(pil_image_module)|
|`_apply_orientation_transform`|fn|priv|3872-3894|def _apply_orientation_transform(pil_image_module, pil_im...|
|`_build_oriented_thumbnail_jpeg_bytes`|fn|priv|3895-3896|def _build_oriented_thumbnail_jpeg_bytes(|
|`_coerce_exif_int_like_value`|fn|priv|3927-3969|def _coerce_exif_int_like_value(raw_value)|
|`_normalize_ifd_integer_like_values_for_piexif_dump`|fn|priv|3970-4103|def _normalize_ifd_integer_like_values_for_piexif_dump(pi...|
|`_refresh_output_jpg_exif_thumbnail_after_save`|fn|priv|4104-4110|def _refresh_output_jpg_exif_thumbnail_after_save(|
|`_set_output_file_timestamps`|fn|priv|4160-4174|def _set_output_file_timestamps(output_jpg, exif_timestamp)|
|`_sync_output_file_timestamps_from_exif`|fn|priv|4175-4192|def _sync_output_file_timestamps_from_exif(output_jpg, ex...|
|`_build_exposure_multipliers`|fn|priv|4193-4212|def _build_exposure_multipliers(ev_minus, ev_zero, ev_plus)|
|`_build_bracket_images_from_linear_base_float`|fn|priv|4213-4241|def _build_bracket_images_from_linear_base_float(np_modul...|
|`_extract_bracket_images_float`|fn|priv|4242-4246|def _extract_bracket_images_float(|
|`_order_bracket_paths`|fn|priv|4280-4305|def _order_bracket_paths(bracket_paths)|
|`_order_hdr_plus_reference_paths`|fn|priv|4306-4321|def _order_hdr_plus_reference_paths(bracket_paths)|
|`_run_luminance_hdr_cli`|fn|priv|4322-4329|def _run_luminance_hdr_cli(|
|`_build_opencv_radiance_exposure_times`|fn|priv|4393-4396|def _build_opencv_radiance_exposure_times(|
|`_build_ev_times_from_ev_zero_and_delta`|fn|priv|4430-4449|def _build_ev_times_from_ev_zero_and_delta(ev_zero, ev_de...|
|`_normalize_opencv_hdr_to_unit_range`|fn|priv|4450-4473|def _normalize_opencv_hdr_to_unit_range(np_module, hdr_rg...|
|`_run_opencv_merge_mertens`|fn|priv|4474-4495|def _run_opencv_merge_mertens(cv2_module, np_module, expo...|
|`_estimate_opencv_camera_response`|fn|priv|4496-4500|def _estimate_opencv_camera_response(|
|`_run_opencv_merge_radiance`|fn|priv|4529-4536|def _run_opencv_merge_radiance(|
|`_normalize_debevec_hdr_to_unit_range`|fn|priv|4600-4619|def _normalize_debevec_hdr_to_unit_range(np_module, hdr_r...|
|`_run_opencv_hdr_merge`|fn|priv|4620-4626|def _run_opencv_hdr_merge(|
|`_hdrplus_box_down2_float32`|fn|priv|4687-4715|def _hdrplus_box_down2_float32(np_module, frames_float32)|
|`_hdrplus_gauss_down4_float32`|fn|priv|4716-4762|def _hdrplus_gauss_down4_float32(np_module, frames_float32)|
|`_hdrplus_build_scalar_proxy_float32`|fn|priv|4763-4796|def _hdrplus_build_scalar_proxy_float32(np_module, frames...|
|`_hdrplus_compute_tile_start_positions`|fn|priv|4797-4817|def _hdrplus_compute_tile_start_positions(np_module, axis...|
|`_hdrplus_trunc_divide_int32`|fn|priv|4818-4836|def _hdrplus_trunc_divide_int32(np_module, values_int32, ...|
|`_hdrplus_compute_alignment_bounds`|fn|priv|4837-4861|def _hdrplus_compute_alignment_bounds(search_radius)|
|`_hdrplus_compute_alignment_margin`|fn|priv|4862-4880|def _hdrplus_compute_alignment_margin(search_radius, divi...|
|`_hdrplus_extract_overlapping_tiles`|fn|priv|4881-4886|def _hdrplus_extract_overlapping_tiles(|
|`_hdrplus_extract_aligned_tiles`|fn|priv|4939-4945|def _hdrplus_extract_aligned_tiles(|
|`_hdrplus_align_layer`|fn|priv|5018-5025|def _hdrplus_align_layer(|
|`_hdrplus_align_layers`|fn|priv|5115-5202|def _hdrplus_align_layers(np_module, scalar_frames, hdrpl...|
|`_hdrplus_resolve_temporal_runtime_options`|fn|priv|5203-5227|def _hdrplus_resolve_temporal_runtime_options(hdrplus_opt...|
|`_hdrplus_compute_temporal_weights`|fn|priv|5228-5232|def _hdrplus_compute_temporal_weights(|
|`_hdrplus_merge_temporal_rgb`|fn|priv|5313-5319|def _hdrplus_merge_temporal_rgb(|
|`_hdrplus_merge_spatial_rgb`|fn|priv|5368-5440|def _hdrplus_merge_spatial_rgb(np_module, temporal_tiles,...|
|`_run_hdr_plus_merge`|fn|priv|5441-5444|def _run_hdr_plus_merge(|
|`_convert_compression_to_quality`|fn|priv|5521-5531|def _convert_compression_to_quality(jpg_compression)|
|`_collect_missing_external_executables`|fn|priv|5532-5534|def _collect_missing_external_executables(|
|`_resolve_auto_adjust_dependencies`|fn|priv|5553-5578|def _resolve_auto_adjust_dependencies()|
|`_resolve_numpy_dependency`|fn|priv|5579-5598|def _resolve_numpy_dependency()|
|`_to_float32_image_array`|fn|priv|5599-5630|def _to_float32_image_array(np_module, image_data)|
|`_normalize_float_rgb_image`|fn|priv|5631-5658|def _normalize_float_rgb_image(np_module, image_data)|
|`_write_rgb_float_tiff16`|fn|priv|5659-5682|def _write_rgb_float_tiff16(imageio_module, np_module, ou...|
|`_write_debug_rgb_float_tiff`|fn|priv|5683-5688|def _write_debug_rgb_float_tiff(|
|`_build_debug_artifact_context`|fn|priv|5718-5738|def _build_debug_artifact_context(output_jpg, input_dng, ...|
|`_format_debug_ev_suffix_value`|fn|priv|5739-5756|def _format_debug_ev_suffix_value(ev_value)|
|`_materialize_bracket_tiffs_from_float`|fn|priv|5757-5761|def _materialize_bracket_tiffs_from_float(|
|`_to_uint8_image_array`|fn|priv|5791-5837|def _to_uint8_image_array(np_module, image_data)|
|`_to_uint16_image_array`|fn|priv|5838-5882|def _to_uint16_image_array(np_module, image_data)|
|`_apply_post_gamma_float`|fn|priv|5883-5908|def _apply_post_gamma_float(np_module, image_rgb_float, g...|
|`_apply_brightness_float`|fn|priv|5909-5931|def _apply_brightness_float(np_module, image_rgb_float, b...|
|`_apply_contrast_float`|fn|priv|5932-5956|def _apply_contrast_float(np_module, image_rgb_float, con...|
|`_apply_saturation_float`|fn|priv|5957-5986|def _apply_saturation_float(np_module, image_rgb_float, s...|
|`_apply_static_postprocess_float`|fn|priv|5987-5992|def _apply_static_postprocess_float(|
|`_to_linear_srgb`|fn|priv|6069-6086|def _to_linear_srgb(np_module, image_srgb)|
|`_from_linear_srgb`|fn|priv|6087-6104|def _from_linear_srgb(np_module, image_linear)|
|`_compute_bt709_luminance`|fn|priv|6105-6122|def _compute_bt709_luminance(np_module, linear_rgb)|
|`_analyze_luminance_key`|fn|priv|6123-6162|def _analyze_luminance_key(np_module, luminance, eps)|
|`_choose_auto_key_value`|fn|priv|6163-6208|def _choose_auto_key_value(key_analysis, auto_brightness_...|
|`_reinhard_global_tonemap_luminance`|fn|priv|6209-6214|def _reinhard_global_tonemap_luminance(|
|`_luminance_preserving_desaturate_to_fit`|fn|priv|6248-6275|def _luminance_preserving_desaturate_to_fit(np_module, rg...|
|`_apply_mild_local_contrast_bgr_uint16`|fn|priv|6276-6314|def _apply_mild_local_contrast_bgr_uint16(cv2_module, np_...|
|`_quantize_clahe_luminance_bins`|fn|priv|6315-6340|def _quantize_clahe_luminance_bins(np_module, luminance_v...|
|`_build_clahe_float_tile_histogram`|fn|priv|6341-6362|def _build_clahe_float_tile_histogram(np_module, luminanc...|
|`_clip_clahe_histogram`|fn|priv|6363-6410|def _clip_clahe_histogram(np_module, histogram, clip_limi...|
|`_build_clahe_float_lut`|fn|priv|6411-6430|def _build_clahe_float_lut(np_module, histogram, tile_pop...|
|`_pad_clahe_luminance_float`|fn|priv|6431-6469|def _pad_clahe_luminance_float(np_module, luminance_float...|
|`_build_clahe_axis_interpolation`|fn|priv|6470-6497|def _build_clahe_axis_interpolation(np_module, axis_lengt...|
|`_build_clahe_tile_luts_float`|fn|priv|6498-6549|def _build_clahe_tile_luts_float(np_module, luminance_flo...|
|`_interpolate_clahe_bilinear_float`|fn|priv|6550-6602|def _interpolate_clahe_bilinear_float(np_module, luminanc...|
|`_apply_clahe_luminance_float`|fn|priv|6603-6634|def _apply_clahe_luminance_float(np_module, luminance_flo...|
|`_reconstruct_rgb_from_ycrcb_luma_float`|fn|priv|6635-6658|def _reconstruct_rgb_from_ycrcb_luma_float(cv2_module, np...|
|`_apply_clahe_luma_rgb_float`|fn|priv|6659-6708|def _apply_clahe_luma_rgb_float(cv2_module, np_module, im...|
|`_rt_gamma2`|fn|priv|6709-6728|def _rt_gamma2(np_module, values)|
|`_rt_igamma2`|fn|priv|6729-6749|def _rt_igamma2(np_module, values)|
|`_auto_levels_index_to_normalized_value`|fn|priv|6750-6766|def _auto_levels_index_to_normalized_value(histogram_valu...|
|`_auto_levels_normalized_to_legacy_code_value`|fn|priv|6767-6782|def _auto_levels_normalized_to_legacy_code_value(value)|
|`_auto_levels_normalized_to_legacy_code`|fn|priv|6783-6799|def _auto_levels_normalized_to_legacy_code(np_module, val...|
|`_auto_levels_legacy_code_to_normalized`|fn|priv|6800-6815|def _auto_levels_legacy_code_to_normalized(np_module, val...|
|`_pack_auto_levels_metrics`|fn|priv|6816-6831|def _pack_auto_levels_metrics(|
|`_build_autoexp_histogram_rgb_float`|fn|priv|6883-6918|def _build_autoexp_histogram_rgb_float(np_module, image_r...|
|`_build_autoexp_histogram_rgb_uint16`|fn|priv|6919-6951|def _build_autoexp_histogram_rgb_uint16(np_module, image_...|
|`_compute_auto_levels_from_histogram`|fn|priv|6952-7151|def _compute_auto_levels_from_histogram(np_module, histog...|
|`_rt_simplebasecurve_scalar`|fn|priv|7225-7317|def _rt_simplebasecurve_scalar(x_value, black, shadow_rec...|
|`_basel`|fn|priv|7238-7254|def _basel(x_input, slope_start, slope_end)|
|`_baseu`|fn|priv|7255-7257|def _baseu(x_input, slope_start, slope_end)|
|`_cupper`|fn|priv|7258-7277|def _cupper(x_input, slope_value, highlight_recovery)|
|`_clower`|fn|priv|7278-7280|def _clower(x_input, slope_value, shadow_value)|
|`_clower2`|fn|priv|7281-7292|def _clower2(x_input, slope_value, shadow_value)|
|`_build_rt_nurbs_curve_lut`|fn|priv|7318-7451|def _build_rt_nurbs_curve_lut(np_module, x_points, y_poin...|
|`_sample_auto_levels_lut_float`|fn|priv|7452-7458|def _sample_auto_levels_lut_float(|
|`_build_auto_levels_full_histogram_rgb_float`|fn|priv|7490-7521|def _build_auto_levels_full_histogram_rgb_float(np_module...|
|`_rt_hlcurve_float`|fn|priv|7522-7552|def _rt_hlcurve_float(np_module, exp_scale, comp, hlrange...|
|`_build_auto_levels_tone_curve_state`|fn|priv|7553-7742|def _build_auto_levels_tone_curve_state(np_module, image_...|
|`_apply_auto_levels_tonal_transform_float`|fn|priv|7743-7746|def _apply_auto_levels_tonal_transform_float(|
|`_auto_levels_has_full_tone_metrics`|fn|priv|7830-7853|def _auto_levels_has_full_tone_metrics(auto_levels_metrics)|
|`_call_auto_levels_compat_helper`|fn|priv|7854-7859|def _call_auto_levels_compat_helper(|
|`_apply_auto_levels_float`|fn|priv|7903-8009|def _apply_auto_levels_float(np_module, image_rgb_float, ...|
|`_clip_auto_levels_out_of_gamut_float`|fn|priv|8010-8170|def _clip_auto_levels_out_of_gamut_float(np_module, image...|
|`_filmlike_clip_rgb_tone`|fn|priv|8040-8073|def _filmlike_clip_rgb_tone(primary, middle, lower)|
|`_clip_auto_levels_out_of_gamut_uint16`|fn|priv|8171-8172|def _clip_auto_levels_out_of_gamut_uint16(|
|`_hlrecovery_luminance_float`|fn|priv|8202-8248|def _hlrecovery_luminance_float(np_module, image_rgb, max...|
|`_hlrecovery_luminance_uint16`|fn|priv|8249-8277|def _hlrecovery_luminance_uint16(np_module, image_rgb, ma...|
|`_hlrecovery_cielab_float`|fn|priv|8278-8279|def _hlrecovery_cielab_float(|
|`_f_lab`|fn|priv|8312-8319|def _f_lab(values)|
|`_f2xyz`|fn|priv|8320-8326|def _f2xyz(values)|
|`_hlrecovery_cielab_uint16`|fn|priv|8362-8363|def _hlrecovery_cielab_uint16(|
|`_hlrecovery_blend_float`|fn|priv|8397-8502|def _hlrecovery_blend_float(np_module, image_rgb, hlmax, ...|
|`_hlrecovery_blend_uint16`|fn|priv|8503-8504|def _hlrecovery_blend_uint16(|
|`_dilate_mask_float`|fn|priv|8539-8561|def _dilate_mask_float(np_module, mask)|
|`_box_mean_3x3_float`|fn|priv|8562-8585|def _box_mean_3x3_float(np_module, image_2d)|
|`_hlrecovery_color_propagation_float`|fn|priv|8586-8630|def _hlrecovery_color_propagation_float(np_module, image_...|
|`_hlrecovery_color_propagation_uint16`|fn|priv|8631-8632|def _hlrecovery_color_propagation_uint16(|
|`_hlrecovery_inpaint_opposed_float`|fn|priv|8662-8663|def _hlrecovery_inpaint_opposed_float(|
|`_hlrecovery_inpaint_opposed_uint16`|fn|priv|8716-8717|def _hlrecovery_inpaint_opposed_uint16(|
|`_apply_auto_brightness_rgb_float`|fn|priv|8759-8762|def _apply_auto_brightness_rgb_float(|
|`_clamp01`|fn|priv|8819-8832|def _clamp01(np_module, values)|
|`_gaussian_kernel_2d`|fn|priv|8833-8855|def _gaussian_kernel_2d(np_module, sigma, radius=None)|
|`_rgb_to_hsl`|fn|priv|8856-8889|def _rgb_to_hsl(np_module, rgb)|
|`_hue_to_rgb`|fn|priv|8890-8920|def _hue_to_rgb(np_module, p_values, q_values, t_values)|
|`_hsl_to_rgb`|fn|priv|8921-8961|def _hsl_to_rgb(np_module, hue, saturation, lightness)|
|`_selective_blur_contrast_gated_vectorized`|fn|priv|8962-8963|def _selective_blur_contrast_gated_vectorized(|
|`_level_per_channel_adaptive`|fn|priv|9012-9034|def _level_per_channel_adaptive(np_module, rgb, low_pct=0...|
|`_sigmoidal_contrast`|fn|priv|9035-9059|def _sigmoidal_contrast(np_module, rgb, contrast=3.0, mid...|
|`logistic`|fn|pub|9050-9052|def logistic(z_values)|
|`_vibrance_hsl_gamma`|fn|priv|9060-9077|def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=...|
|`_gaussian_blur_rgb`|fn|priv|9078-9101|def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)|
|`_high_pass_math_gray`|fn|priv|9102-9125|def _high_pass_math_gray(cv2_module, np_module, rgb, blur...|
|`_overlay_composite`|fn|priv|9126-9147|def _overlay_composite(np_module, base_rgb, overlay_gray)|
|`_apply_validated_auto_adjust_pipeline`|fn|priv|9148-9154|def _apply_validated_auto_adjust_pipeline(|
|`_load_piexif_dependency`|fn|priv|9263-9280|def _load_piexif_dependency()|
|`_encode_jpg`|fn|priv|9281-9292|def _encode_jpg(|
|`_collect_processing_errors`|fn|priv|9424-9452|def _collect_processing_errors(rawpy_module)|
|`_is_supported_runtime_os`|fn|priv|9453-9472|def _is_supported_runtime_os()|
|`run`|fn|pub|9473-9672|def run(args)|
