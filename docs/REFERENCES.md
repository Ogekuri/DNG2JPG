...
# dng2jpg.py | Python | 9405L | 291 symbols | 24 imports | 204 comments
...
- var `AUTO_ZERO_SCENE_KEY_LOW_THRESHOLD = 0.35` (L144)
- var `AUTO_ZERO_SCENE_KEY_HIGH_THRESHOLD = 0.65` (L145)
- var `AUTO_ZERO_TARGET_LOW_KEY = 0.35` (L146)
- var `AUTO_ZERO_TARGET_HIGH_KEY = 0.65` (L147)
### class `class AutoAdjustOptions` `@dataclass(frozen=True)` (L361-396)
...
### class `class AutoBrightnessOptions` `@dataclass(frozen=True)` (L398-426)
...
### class `class AutoLevelsOptions` `@dataclass(frozen=True)` (L428-451)
...
### class `class PostprocessOptions` `@dataclass(frozen=True)` (L453-489)
...
### class `class DebugArtifactContext` `@dataclass(frozen=True)` (L491-507)
...
### class `class SourceGammaInfo` `@dataclass(frozen=True)` (L509-527)
...
### class `class LuminanceOptions` `@dataclass(frozen=True)` (L529-549)
...
### class `class OpenCvMergeOptions` `@dataclass(frozen=True)` (L551-571)
...
### class `class HdrPlusOptions` `@dataclass(frozen=True)` (L573-596)
...
### class `class HdrPlusTemporalRuntimeOptions` `@dataclass(frozen=True)` (L598-617)
...
### class `class JointAutoEvSolution` `@dataclass(frozen=True)` (L619-650)
...
### class `class AutoZeroEvaluation` `@dataclass(frozen=True)` (L652-670)
...
### fn `def _print_box_table(headers, rows, header_rows=())` `priv` (L671-707)
...
### fn `def _border(left, middle, right)` `priv` (L691-693)
- @brief Print one Unicode box-drawing table.
...
### fn `def _line(values)` `priv` (L694-697)

### fn `def _build_two_line_operator_rows(operator_entries)` `priv` (L708-724)
...
### fn `def _print_help_section(title)` `priv` (L725-739)
...
### fn `def _print_help_option(option_label, description, detail_lines=())` `priv` (L740-781)
...
### fn `def print_help(version)` (L782-981)
...
### fn `def _calculate_max_ev_from_bits(bits_per_color)` `priv` (L1136-1154)
...
### fn `def _calculate_safe_ev_zero_max(base_max_ev)` `priv` (L1155-1167)
...
### fn `def _derive_supported_ev_zero_values(base_max_ev)` `priv` (L1168-1184)
...
### fn `def _derive_supported_signed_ev_zero_values(base_max_ev)` `priv` (L1185-1206)
...
### fn `def _derive_supported_ev_values(bits_per_color, ev_zero=0.0)` `priv` (L1207-1235)
...
### fn `def _detect_dng_bits_per_color(raw_handle)` `priv` (L1236-1281)
...
### fn `def _is_ev_value_on_supported_step(ev_value)` `priv` (L1282-1295)
...
### fn `def _parse_ev_option(ev_raw)` `priv` (L1296-1327)
...
### fn `def _parse_ev_zero_option(ev_zero_raw)` `priv` (L1328-1358)
...
### fn `def _parse_auto_ev_option(auto_ev_raw)` `priv` (L1359-1378)
...
### fn `def _parse_percentage_option(option_name, option_raw)` `priv` (L1379-1401)
...
### fn `def _parse_auto_brightness_option(auto_brightness_raw)` `priv` (L1402-1421)
...
### fn `def _parse_auto_levels_option(auto_levels_raw)` `priv` (L1422-1441)
...
### fn `def _parse_explicit_boolean_option(option_name, option_raw)` `priv` (L1442-1462)
...
### fn `def _parse_opencv_merge_algorithm_option(algorithm_raw)` `priv` (L1463-1487)
...
### fn `def _parse_opencv_options(opencv_raw_values)` `priv` (L1488-1534)
...
### fn `def _clamp_ev_to_supported(ev_candidate, ev_values)` `priv` (L1535-1548)
...
### fn `def _quantize_ev_to_supported(ev_value, ev_values)` `priv` (L1549-1570)
...
### fn `def _quantize_ev_toward_zero_step(ev_value, step=EV_STEP)` `priv` (L1571-1592)
...
### fn `def _apply_auto_percentage_scaling(ev_value, percentage)` `priv` (L1593-1607)
...
### fn `def _extract_normalized_preview_luminance_stats(raw_handle)` `priv` (L1608-1667)
...
### fn `def _percentile(percentile_value)` `priv` (L1642-1652)
- @brief Extract normalized preview luminance percentiles from RAW handle.
...
### fn `def _extract_base_rgb_linear_float(raw_handle, np_module)` `priv` (L1668-1696)
...
### fn `def _normalize_source_gamma_label(label_raw)` `priv` (L1697-1715)
...
### fn `def _decode_raw_metadata_text(metadata_raw)` `priv` (L1716-1747)
...
### fn `def _classify_explicit_source_gamma(raw_handle)` `priv` (L1748-1797)
...
### fn `def _classify_tone_curve_gamma(raw_handle)` `priv` (L1798-1846)
...
### fn `def _has_nonzero_matrix(matrix_raw)` `priv` (L1847-1867)
...
### fn `def _classify_matrix_hint_gamma(raw_handle)` `priv` (L1868-1898)
...
### fn `def _extract_source_gamma_info(raw_handle)` `priv` (L1899-1926)
...
### fn `def _describe_source_gamma_info(source_gamma_info)` `priv` (L1927-1948)
...
### fn `def _coerce_positive_luminance(value, fallback)` `priv` (L1949-1968)
...
### fn `def _calculate_bt709_luminance(np_module, image_rgb_float)` `priv` (L1969-1991)
...
### fn `def _smoothstep(np_module, values, edge0, edge1)` `priv` (L1992-2010)
...
### fn `def _calculate_entropy_optimized_ev(cv2_module, np_module, luminance_float)` `priv` (L2011-2056)
...
### fn `def _calculate_ettr_ev(np_module, luminance_float)` `priv` (L2057-2076)
...
### fn `def _calculate_detail_preservation_ev(cv2_module, np_module, luminance_float)` `priv` (L2077-2143)
...
### fn `def _calculate_auto_zero_evaluations(cv2_module, np_module, image_rgb_float)` `priv` (L2144-2181)
...
### fn `def _build_joint_auto_ev_regularization_anchors(evaluations, safe_ev_zero_max)` `priv` (L2182-2204)
...
### fn `def _evaluate_joint_auto_ev_candidate(` `priv` (L2205-2211)

### fn `def _optimize_joint_ev_zero_and_delta(` `priv` (L2270-2275)
...
### fn `def _resolve_joint_auto_ev_solution(` `priv` (L2326-2333)
...
### fn `def _parse_luminance_text_option(option_name, option_raw)` `priv` (L2398-2418)
...
### fn `def _parse_positive_float_option(option_name, option_raw)` `priv` (L2419-2442)
...
### fn `def _parse_positive_int_option(option_name, option_raw)` `priv` (L2443-2466)
...
### fn `def _parse_tmo_passthrough_value(option_name, option_raw)` `priv` (L2467-2483)
...
### fn `def _parse_jpg_compression_option(compression_raw)` `priv` (L2484-2506)
...
### fn `def _parse_float_exclusive_range_option(option_name, option_raw, min_value, max_value)` `priv` (L2507-2531)
...
### fn `def _parse_non_negative_float_option(option_name, option_raw)` `priv` (L2532-2554)
...
### fn `def _parse_float_in_range_option(option_name, option_raw, min_value, max_value)` `priv` (L2555-2580)
...
### fn `def _parse_positive_int_pair_option(option_name, option_raw)` `priv` (L2581-2612)
...
### fn `def _parse_auto_brightness_options(auto_brightness_raw_values)` `priv` (L2613-2709)
...
### fn `def _parse_auto_levels_hr_method_option(auto_levels_method_raw)` `priv` (L2710-2741)
...
### fn `def _parse_auto_levels_options(auto_levels_raw_values)` `priv` (L2742-2805)
...
### fn `def _parse_auto_adjust_options(auto_adjust_raw_values)` `priv` (L2806-2955)
...
### fn `def _parse_hdrplus_proxy_mode_option(proxy_mode_raw)` `priv` (L2956-2974)
...
### fn `def _parse_hdrplus_options(hdrplus_raw_values)` `priv` (L2975-3051)
...
### fn `def _parse_auto_adjust_option(auto_adjust_raw)` `priv` (L3052-3075)
...
### fn `def _parse_hdr_merge_option(hdr_merge_raw)` `priv` (L3076-3105)
...
### fn `def _resolve_default_postprocess(` `priv` (L3106-3109)

### fn `def _parse_run_options(args)` `priv` (L3181-3380)
...
### fn `def _load_image_dependencies()` `priv` (L3941-3978)
...
### fn `def _parse_exif_datetime_to_timestamp(datetime_raw)` `priv` (L3979-4009)
...
### fn `def _parse_exif_exposure_time_to_seconds(exposure_raw)` `priv` (L4010-4072)
...
### fn `def _extract_dng_exif_payload_and_timestamp(pil_image_module, input_dng)` `priv` (L4073-4167)
...
### fn `def _read_exif_value(exif_tag)` `priv` (L4117-4134)
- @brief Extract DNG EXIF payload bytes, preferred datetime timestamp, source orientation, and exposure time.
...
### fn `def _resolve_thumbnail_transpose_map(pil_image_module)` `priv` (L4168-4199)
...
### fn `def _apply_orientation_transform(pil_image_module, pil_image, source_orientation)` `priv` (L4200-4222)
...
### fn `def _build_oriented_thumbnail_jpeg_bytes(` `priv` (L4223-4224)

### fn `def _coerce_exif_int_like_value(raw_value)` `priv` (L4255-4297)
...
### fn `def _normalize_ifd_integer_like_values_for_piexif_dump(piexif_module, exif_dict)` `priv` (L4298-4431)
...
### fn `def _refresh_output_jpg_exif_thumbnail_after_save(` `priv` (L4432-4438)

### fn `def _set_output_file_timestamps(output_jpg, exif_timestamp)` `priv` (L4488-4502)
...
### fn `def _sync_output_file_timestamps_from_exif(output_jpg, exif_timestamp)` `priv` (L4503-4520)
...
### fn `def _build_exposure_multipliers(ev_value, ev_zero=0.0)` `priv` (L4521-4539)
...
### fn `def _build_bracket_images_from_linear_base_float(np_module, base_rgb_float, multipliers)` `priv` (L4540-4568)
...
### fn `def _extract_bracket_images_float(` `priv` (L4569-4573)

### fn `def _order_bracket_paths(bracket_paths)` `priv` (L4607-4632)
...
### fn `def _order_hdr_plus_reference_paths(bracket_paths)` `priv` (L4633-4648)
...
### fn `def _run_luminance_hdr_cli(` `priv` (L4649-4656)

### fn `def _build_opencv_radiance_exposure_times(` `priv` (L4720-4723)
...
### fn `def _build_ev_times_from_ev_zero_and_delta(ev_zero, ev_delta)` `priv` (L4757-4776)
...
### fn `def _normalize_opencv_hdr_to_unit_range(np_module, hdr_rgb_float32)` `priv` (L4777-4800)
...
### fn `def _run_opencv_merge_mertens(cv2_module, np_module, exposures_float)` `priv` (L4801-4822)
...
### fn `def _estimate_opencv_camera_response(` `priv` (L4823-4827)

### fn `def _run_opencv_merge_radiance(` `priv` (L4856-4863)
...
### fn `def _normalize_debevec_hdr_to_unit_range(np_module, hdr_rgb_float32, white_point_percentile)` `priv` (L4927-4946)
...
### fn `def _run_opencv_hdr_merge(` `priv` (L4947-4953)

### fn `def _hdrplus_box_down2_float32(np_module, frames_float32)` `priv` (L5014-5042)
...
### fn `def _hdrplus_gauss_down4_float32(np_module, frames_float32)` `priv` (L5043-5089)
...
### fn `def _hdrplus_build_scalar_proxy_float32(np_module, frames_rgb_float32, hdrplus_options)` `priv` (L5090-5123)
...
### fn `def _hdrplus_compute_tile_start_positions(np_module, axis_length, tile_stride, pad_margin)` `priv` (L5124-5144)
...
### fn `def _hdrplus_trunc_divide_int32(np_module, values_int32, divisor)` `priv` (L5145-5163)
...
### fn `def _hdrplus_compute_alignment_bounds(search_radius)` `priv` (L5164-5188)
...
### fn `def _hdrplus_compute_alignment_margin(search_radius, divisor=1)` `priv` (L5189-5207)
...
### fn `def _hdrplus_extract_overlapping_tiles(` `priv` (L5208-5213)

### fn `def _hdrplus_extract_aligned_tiles(` `priv` (L5266-5272)

### fn `def _hdrplus_align_layer(` `priv` (L5345-5352)
...
### fn `def _hdrplus_align_layers(np_module, scalar_frames, hdrplus_options)` `priv` (L5442-5529)
...
### fn `def _hdrplus_resolve_temporal_runtime_options(hdrplus_options)` `priv` (L5530-5554)
...
### fn `def _hdrplus_compute_temporal_weights(` `priv` (L5555-5559)

### fn `def _hdrplus_merge_temporal_rgb(` `priv` (L5640-5646)
...
### fn `def _hdrplus_merge_spatial_rgb(np_module, temporal_tiles, width, height)` `priv` (L5695-5767)
...
### fn `def _run_hdr_plus_merge(` `priv` (L5768-5771)

### fn `def _convert_compression_to_quality(jpg_compression)` `priv` (L5848-5858)
...
### fn `def _collect_missing_external_executables(` `priv` (L5859-5861)

### fn `def _resolve_auto_adjust_dependencies()` `priv` (L5880-5905)
...
### fn `def _resolve_numpy_dependency()` `priv` (L5906-5925)
...
### fn `def _to_float32_image_array(np_module, image_data)` `priv` (L5926-5957)
...
### fn `def _normalize_float_rgb_image(np_module, image_data)` `priv` (L5958-5985)
...
### fn `def _write_rgb_float_tiff16(imageio_module, np_module, output_path, image_rgb_float)` `priv` (L5986-6009)
...
### fn `def _write_debug_rgb_float_tiff(` `priv` (L6010-6015)

### fn `def _build_debug_artifact_context(output_jpg, input_dng, postprocess_options)` `priv` (L6045-6065)
...
### fn `def _format_debug_ev_suffix_value(ev_value)` `priv` (L6066-6083)
...
### fn `def _materialize_bracket_tiffs_from_float(` `priv` (L6084-6088)

### fn `def _to_uint8_image_array(np_module, image_data)` `priv` (L6118-6164)
...
### fn `def _to_uint16_image_array(np_module, image_data)` `priv` (L6165-6209)
...
### fn `def _apply_post_gamma_float(np_module, image_rgb_float, gamma_value)` `priv` (L6210-6235)
...
### fn `def _apply_brightness_float(np_module, image_rgb_float, brightness_factor)` `priv` (L6236-6258)
...
### fn `def _apply_contrast_float(np_module, image_rgb_float, contrast_factor)` `priv` (L6259-6283)
...
### fn `def _apply_saturation_float(np_module, image_rgb_float, saturation_factor)` `priv` (L6284-6313)
...
### fn `def _apply_static_postprocess_float(` `priv` (L6314-6319)

### fn `def _to_linear_srgb(np_module, image_srgb)` `priv` (L6396-6413)
...
### fn `def _from_linear_srgb(np_module, image_linear)` `priv` (L6414-6431)
...
### fn `def _compute_bt709_luminance(np_module, linear_rgb)` `priv` (L6432-6449)
...
### fn `def _analyze_luminance_key(np_module, luminance, eps)` `priv` (L6450-6489)
...
### fn `def _choose_auto_key_value(key_analysis, auto_brightness_options)` `priv` (L6490-6535)
...
### fn `def _reinhard_global_tonemap_luminance(` `priv` (L6536-6541)

### fn `def _luminance_preserving_desaturate_to_fit(np_module, rgb_linear, luminance, eps)` `priv` (L6575-6602)
...
### fn `def _apply_mild_local_contrast_bgr_uint16(cv2_module, np_module, image_bgr_uint16, options)` `priv` (L6603-6641)
...
### fn `def _quantize_clahe_luminance_bins(np_module, luminance_values, histogram_size)` `priv` (L6642-6667)
...
### fn `def _build_clahe_float_tile_histogram(np_module, luminance_tile, histogram_size)` `priv` (L6668-6689)
...
### fn `def _clip_clahe_histogram(np_module, histogram, clip_limit, tile_population)` `priv` (L6690-6737)
...
### fn `def _build_clahe_float_lut(np_module, histogram, tile_population)` `priv` (L6738-6757)
...
### fn `def _pad_clahe_luminance_float(np_module, luminance_float, tile_grid_size)` `priv` (L6758-6796)
...
### fn `def _build_clahe_axis_interpolation(np_module, axis_length, tile_size, tile_count)` `priv` (L6797-6824)
...
### fn `def _build_clahe_tile_luts_float(np_module, luminance_float, clip_limit, tile_grid_size, histogram_size)` `priv` (L6825-6876)
...
### fn `def _interpolate_clahe_bilinear_float(np_module, luminance_float, tile_luts, tile_height, tile_width)` `priv` (L6877-6929)
...
### fn `def _apply_clahe_luminance_float(np_module, luminance_float, clip_limit, tile_grid_size)` `priv` (L6930-6961)
...
### fn `def _reconstruct_rgb_from_ycrcb_luma_float(cv2_module, np_module, luminance_float, cr_channel, cb_channel)` `priv` (L6962-6985)
...
### fn `def _apply_clahe_luma_rgb_float(cv2_module, np_module, image_rgb_float, auto_adjust_options)` `priv` (L6986-7035)
...
### fn `def _rt_gamma2(np_module, values)` `priv` (L7036-7055)
...
### fn `def _rt_igamma2(np_module, values)` `priv` (L7056-7076)
...
### fn `def _auto_levels_index_to_normalized_value(histogram_value, histcompr)` `priv` (L7077-7093)
...
### fn `def _auto_levels_normalized_to_legacy_code_value(value)` `priv` (L7094-7109)
...
### fn `def _auto_levels_normalized_to_legacy_code(np_module, values)` `priv` (L7110-7126)
...
### fn `def _auto_levels_legacy_code_to_normalized(np_module, values)` `priv` (L7127-7142)
...
### fn `def _pack_auto_levels_metrics(` `priv` (L7143-7158)

### fn `def _build_autoexp_histogram_rgb_float(np_module, image_rgb_float, histcompr)` `priv` (L7210-7245)
...
### fn `def _build_autoexp_histogram_rgb_uint16(np_module, image_rgb_uint16, histcompr)` `priv` (L7246-7278)
...
### fn `def _compute_auto_levels_from_histogram(np_module, histogram, histcompr, clip_percent)` `priv` (L7279-7478)
...
### fn `def _call_auto_levels_compat_helper(` `priv` (L7552-7557)

### fn `def _apply_auto_levels_float(np_module, image_rgb_float, auto_levels_options)` `priv` (L7601-7700)
...
### fn `def _clip_auto_levels_out_of_gamut_float(np_module, image_rgb, maxval=1.0)` `priv` (L7701-7719)
...
### fn `def _clip_auto_levels_out_of_gamut_uint16(` `priv` (L7720-7721)

### fn `def _hlrecovery_luminance_float(np_module, image_rgb, maxval=1.0)` `priv` (L7751-7797)
...
### fn `def _hlrecovery_luminance_uint16(np_module, image_rgb, maxval=_AUTO_LEVELS_CODE_MAX)` `priv` (L7798-7826)
...
### fn `def _hlrecovery_cielab_float(` `priv` (L7827-7828)

### fn `def _f_lab(values)` `priv` (L7861-7868)
...
### fn `def _f2xyz(values)` `priv` (L7869-7875)

### fn `def _hlrecovery_cielab_uint16(` `priv` (L7911-7912)

### fn `def _hlrecovery_blend_float(np_module, image_rgb, hlmax, maxval=1.0)` `priv` (L7946-8051)
...
### fn `def _hlrecovery_blend_uint16(` `priv` (L8052-8053)

### fn `def _dilate_mask_float(np_module, mask)` `priv` (L8088-8110)
...
### fn `def _box_mean_3x3_float(np_module, image_2d)` `priv` (L8111-8134)
...
### fn `def _hlrecovery_color_propagation_float(np_module, image_rgb, maxval=1.0)` `priv` (L8135-8179)
...
### fn `def _hlrecovery_color_propagation_uint16(` `priv` (L8180-8181)

### fn `def _hlrecovery_inpaint_opposed_float(` `priv` (L8211-8212)
...
### fn `def _hlrecovery_inpaint_opposed_uint16(` `priv` (L8265-8266)
...
### fn `def _apply_auto_brightness_rgb_float(` `priv` (L8308-8311)
...
### fn `def _clamp01(np_module, values)` `priv` (L8368-8381)
...
### fn `def _gaussian_kernel_2d(np_module, sigma, radius=None)` `priv` (L8382-8404)
...
### fn `def _rgb_to_hsl(np_module, rgb)` `priv` (L8405-8438)
...
### fn `def _hue_to_rgb(np_module, p_values, q_values, t_values)` `priv` (L8439-8469)
...
### fn `def _hsl_to_rgb(np_module, hue, saturation, lightness)` `priv` (L8470-8510)
...
### fn `def _selective_blur_contrast_gated_vectorized(` `priv` (L8511-8512)

### fn `def _level_per_channel_adaptive(np_module, rgb, low_pct=0.1, high_pct=99.9)` `priv` (L8561-8583)
...
### fn `def _sigmoidal_contrast(np_module, rgb, contrast=3.0, midpoint=0.5)` `priv` (L8584-8608)
...
### fn `def logistic(z_values)` (L8599-8601)
- @brief Execute sigmoidal contrast stage.
...
### fn `def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=0.8)` `priv` (L8609-8626)
...
### fn `def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)` `priv` (L8627-8650)
...
### fn `def _high_pass_math_gray(cv2_module, np_module, rgb, blur_sigma=2.5)` `priv` (L8651-8674)
...
### fn `def _overlay_composite(np_module, base_rgb, overlay_gray)` `priv` (L8675-8696)
...
### fn `def _apply_validated_auto_adjust_pipeline(` `priv` (L8697-8703)

### fn `def _load_piexif_dependency()` `priv` (L8812-8829)
...
### fn `def _encode_jpg(` `priv` (L8830-8841)

### fn `def _collect_processing_errors(rawpy_module)` `priv` (L8973-9001)
...
### fn `def _is_supported_runtime_os()` `priv` (L9002-9021)
...
|`AUTO_ZERO_SCENE_KEY_LOW_THRESHOLD`|var|pub|144||
|`AUTO_ZERO_SCENE_KEY_HIGH_THRESHOLD`|var|pub|145||
|`AUTO_ZERO_TARGET_LOW_KEY`|var|pub|146||
|`AUTO_ZERO_TARGET_HIGH_KEY`|var|pub|147||
|`AutoAdjustOptions`|class|pub|361-396|class AutoAdjustOptions|
|`AutoBrightnessOptions`|class|pub|398-426|class AutoBrightnessOptions|
|`AutoLevelsOptions`|class|pub|428-451|class AutoLevelsOptions|
|`PostprocessOptions`|class|pub|453-489|class PostprocessOptions|
|`DebugArtifactContext`|class|pub|491-507|class DebugArtifactContext|
|`SourceGammaInfo`|class|pub|509-527|class SourceGammaInfo|
|`LuminanceOptions`|class|pub|529-549|class LuminanceOptions|
|`OpenCvMergeOptions`|class|pub|551-571|class OpenCvMergeOptions|
|`HdrPlusOptions`|class|pub|573-596|class HdrPlusOptions|
|`HdrPlusTemporalRuntimeOptions`|class|pub|598-617|class HdrPlusTemporalRuntimeOptions|
|`JointAutoEvSolution`|class|pub|619-650|class JointAutoEvSolution|
|`AutoZeroEvaluation`|class|pub|652-670|class AutoZeroEvaluation|
|`_print_box_table`|fn|priv|671-707|def _print_box_table(headers, rows, header_rows=())|
|`_border`|fn|priv|691-693|def _border(left, middle, right)|
|`_line`|fn|priv|694-697|def _line(values)|
|`_build_two_line_operator_rows`|fn|priv|708-724|def _build_two_line_operator_rows(operator_entries)|
|`_print_help_section`|fn|priv|725-739|def _print_help_section(title)|
|`_print_help_option`|fn|priv|740-781|def _print_help_option(option_label, description, detail_...|
|`print_help`|fn|pub|782-981|def print_help(version)|
|`_calculate_max_ev_from_bits`|fn|priv|1136-1154|def _calculate_max_ev_from_bits(bits_per_color)|
|`_calculate_safe_ev_zero_max`|fn|priv|1155-1167|def _calculate_safe_ev_zero_max(base_max_ev)|
|`_derive_supported_ev_zero_values`|fn|priv|1168-1184|def _derive_supported_ev_zero_values(base_max_ev)|
|`_derive_supported_signed_ev_zero_values`|fn|priv|1185-1206|def _derive_supported_signed_ev_zero_values(base_max_ev)|
|`_derive_supported_ev_values`|fn|priv|1207-1235|def _derive_supported_ev_values(bits_per_color, ev_zero=0.0)|
|`_detect_dng_bits_per_color`|fn|priv|1236-1281|def _detect_dng_bits_per_color(raw_handle)|
|`_is_ev_value_on_supported_step`|fn|priv|1282-1295|def _is_ev_value_on_supported_step(ev_value)|
|`_parse_ev_option`|fn|priv|1296-1327|def _parse_ev_option(ev_raw)|
|`_parse_ev_zero_option`|fn|priv|1328-1358|def _parse_ev_zero_option(ev_zero_raw)|
|`_parse_auto_ev_option`|fn|priv|1359-1378|def _parse_auto_ev_option(auto_ev_raw)|
|`_parse_percentage_option`|fn|priv|1379-1401|def _parse_percentage_option(option_name, option_raw)|
|`_parse_auto_brightness_option`|fn|priv|1402-1421|def _parse_auto_brightness_option(auto_brightness_raw)|
|`_parse_auto_levels_option`|fn|priv|1422-1441|def _parse_auto_levels_option(auto_levels_raw)|
|`_parse_explicit_boolean_option`|fn|priv|1442-1462|def _parse_explicit_boolean_option(option_name, option_raw)|
|`_parse_opencv_merge_algorithm_option`|fn|priv|1463-1487|def _parse_opencv_merge_algorithm_option(algorithm_raw)|
|`_parse_opencv_options`|fn|priv|1488-1534|def _parse_opencv_options(opencv_raw_values)|
|`_clamp_ev_to_supported`|fn|priv|1535-1548|def _clamp_ev_to_supported(ev_candidate, ev_values)|
|`_quantize_ev_to_supported`|fn|priv|1549-1570|def _quantize_ev_to_supported(ev_value, ev_values)|
|`_quantize_ev_toward_zero_step`|fn|priv|1571-1592|def _quantize_ev_toward_zero_step(ev_value, step=EV_STEP)|
|`_apply_auto_percentage_scaling`|fn|priv|1593-1607|def _apply_auto_percentage_scaling(ev_value, percentage)|
|`_extract_normalized_preview_luminance_stats`|fn|priv|1608-1667|def _extract_normalized_preview_luminance_stats(raw_handle)|
|`_percentile`|fn|priv|1642-1652|def _percentile(percentile_value)|
|`_extract_base_rgb_linear_float`|fn|priv|1668-1696|def _extract_base_rgb_linear_float(raw_handle, np_module)|
|`_normalize_source_gamma_label`|fn|priv|1697-1715|def _normalize_source_gamma_label(label_raw)|
|`_decode_raw_metadata_text`|fn|priv|1716-1747|def _decode_raw_metadata_text(metadata_raw)|
|`_classify_explicit_source_gamma`|fn|priv|1748-1797|def _classify_explicit_source_gamma(raw_handle)|
|`_classify_tone_curve_gamma`|fn|priv|1798-1846|def _classify_tone_curve_gamma(raw_handle)|
|`_has_nonzero_matrix`|fn|priv|1847-1867|def _has_nonzero_matrix(matrix_raw)|
|`_classify_matrix_hint_gamma`|fn|priv|1868-1898|def _classify_matrix_hint_gamma(raw_handle)|
|`_extract_source_gamma_info`|fn|priv|1899-1926|def _extract_source_gamma_info(raw_handle)|
|`_describe_source_gamma_info`|fn|priv|1927-1948|def _describe_source_gamma_info(source_gamma_info)|
|`_coerce_positive_luminance`|fn|priv|1949-1968|def _coerce_positive_luminance(value, fallback)|
|`_calculate_bt709_luminance`|fn|priv|1969-1991|def _calculate_bt709_luminance(np_module, image_rgb_float)|
|`_smoothstep`|fn|priv|1992-2010|def _smoothstep(np_module, values, edge0, edge1)|
|`_calculate_entropy_optimized_ev`|fn|priv|2011-2056|def _calculate_entropy_optimized_ev(cv2_module, np_module...|
|`_calculate_ettr_ev`|fn|priv|2057-2076|def _calculate_ettr_ev(np_module, luminance_float)|
|`_calculate_detail_preservation_ev`|fn|priv|2077-2143|def _calculate_detail_preservation_ev(cv2_module, np_modu...|
|`_calculate_auto_zero_evaluations`|fn|priv|2144-2181|def _calculate_auto_zero_evaluations(cv2_module, np_modul...|
|`_build_joint_auto_ev_regularization_anchors`|fn|priv|2182-2204|def _build_joint_auto_ev_regularization_anchors(evaluatio...|
|`_evaluate_joint_auto_ev_candidate`|fn|priv|2205-2211|def _evaluate_joint_auto_ev_candidate(|
|`_optimize_joint_ev_zero_and_delta`|fn|priv|2270-2275|def _optimize_joint_ev_zero_and_delta(|
|`_resolve_joint_auto_ev_solution`|fn|priv|2326-2333|def _resolve_joint_auto_ev_solution(|
|`_parse_luminance_text_option`|fn|priv|2398-2418|def _parse_luminance_text_option(option_name, option_raw)|
|`_parse_positive_float_option`|fn|priv|2419-2442|def _parse_positive_float_option(option_name, option_raw)|
|`_parse_positive_int_option`|fn|priv|2443-2466|def _parse_positive_int_option(option_name, option_raw)|
|`_parse_tmo_passthrough_value`|fn|priv|2467-2483|def _parse_tmo_passthrough_value(option_name, option_raw)|
|`_parse_jpg_compression_option`|fn|priv|2484-2506|def _parse_jpg_compression_option(compression_raw)|
|`_parse_float_exclusive_range_option`|fn|priv|2507-2531|def _parse_float_exclusive_range_option(option_name, opti...|
|`_parse_non_negative_float_option`|fn|priv|2532-2554|def _parse_non_negative_float_option(option_name, option_...|
|`_parse_float_in_range_option`|fn|priv|2555-2580|def _parse_float_in_range_option(option_name, option_raw,...|
|`_parse_positive_int_pair_option`|fn|priv|2581-2612|def _parse_positive_int_pair_option(option_name, option_raw)|
|`_parse_auto_brightness_options`|fn|priv|2613-2709|def _parse_auto_brightness_options(auto_brightness_raw_va...|
|`_parse_auto_levels_hr_method_option`|fn|priv|2710-2741|def _parse_auto_levels_hr_method_option(auto_levels_metho...|
|`_parse_auto_levels_options`|fn|priv|2742-2805|def _parse_auto_levels_options(auto_levels_raw_values)|
|`_parse_auto_adjust_options`|fn|priv|2806-2955|def _parse_auto_adjust_options(auto_adjust_raw_values)|
|`_parse_hdrplus_proxy_mode_option`|fn|priv|2956-2974|def _parse_hdrplus_proxy_mode_option(proxy_mode_raw)|
|`_parse_hdrplus_options`|fn|priv|2975-3051|def _parse_hdrplus_options(hdrplus_raw_values)|
|`_parse_auto_adjust_option`|fn|priv|3052-3075|def _parse_auto_adjust_option(auto_adjust_raw)|
|`_parse_hdr_merge_option`|fn|priv|3076-3105|def _parse_hdr_merge_option(hdr_merge_raw)|
|`_resolve_default_postprocess`|fn|priv|3106-3109|def _resolve_default_postprocess(|
|`_parse_run_options`|fn|priv|3181-3380|def _parse_run_options(args)|
|`_load_image_dependencies`|fn|priv|3941-3978|def _load_image_dependencies()|
|`_parse_exif_datetime_to_timestamp`|fn|priv|3979-4009|def _parse_exif_datetime_to_timestamp(datetime_raw)|
|`_parse_exif_exposure_time_to_seconds`|fn|priv|4010-4072|def _parse_exif_exposure_time_to_seconds(exposure_raw)|
|`_extract_dng_exif_payload_and_timestamp`|fn|priv|4073-4167|def _extract_dng_exif_payload_and_timestamp(pil_image_mod...|
|`_read_exif_value`|fn|priv|4117-4134|def _read_exif_value(exif_tag)|
|`_resolve_thumbnail_transpose_map`|fn|priv|4168-4199|def _resolve_thumbnail_transpose_map(pil_image_module)|
|`_apply_orientation_transform`|fn|priv|4200-4222|def _apply_orientation_transform(pil_image_module, pil_im...|
|`_build_oriented_thumbnail_jpeg_bytes`|fn|priv|4223-4224|def _build_oriented_thumbnail_jpeg_bytes(|
|`_coerce_exif_int_like_value`|fn|priv|4255-4297|def _coerce_exif_int_like_value(raw_value)|
|`_normalize_ifd_integer_like_values_for_piexif_dump`|fn|priv|4298-4431|def _normalize_ifd_integer_like_values_for_piexif_dump(pi...|
|`_refresh_output_jpg_exif_thumbnail_after_save`|fn|priv|4432-4438|def _refresh_output_jpg_exif_thumbnail_after_save(|
|`_set_output_file_timestamps`|fn|priv|4488-4502|def _set_output_file_timestamps(output_jpg, exif_timestamp)|
|`_sync_output_file_timestamps_from_exif`|fn|priv|4503-4520|def _sync_output_file_timestamps_from_exif(output_jpg, ex...|
|`_build_exposure_multipliers`|fn|priv|4521-4539|def _build_exposure_multipliers(ev_value, ev_zero=0.0)|
|`_build_bracket_images_from_linear_base_float`|fn|priv|4540-4568|def _build_bracket_images_from_linear_base_float(np_modul...|
|`_extract_bracket_images_float`|fn|priv|4569-4573|def _extract_bracket_images_float(|
|`_order_bracket_paths`|fn|priv|4607-4632|def _order_bracket_paths(bracket_paths)|
|`_order_hdr_plus_reference_paths`|fn|priv|4633-4648|def _order_hdr_plus_reference_paths(bracket_paths)|
|`_run_luminance_hdr_cli`|fn|priv|4649-4656|def _run_luminance_hdr_cli(|
|`_build_opencv_radiance_exposure_times`|fn|priv|4720-4723|def _build_opencv_radiance_exposure_times(|
|`_build_ev_times_from_ev_zero_and_delta`|fn|priv|4757-4776|def _build_ev_times_from_ev_zero_and_delta(ev_zero, ev_de...|
|`_normalize_opencv_hdr_to_unit_range`|fn|priv|4777-4800|def _normalize_opencv_hdr_to_unit_range(np_module, hdr_rg...|
|`_run_opencv_merge_mertens`|fn|priv|4801-4822|def _run_opencv_merge_mertens(cv2_module, np_module, expo...|
|`_estimate_opencv_camera_response`|fn|priv|4823-4827|def _estimate_opencv_camera_response(|
|`_run_opencv_merge_radiance`|fn|priv|4856-4863|def _run_opencv_merge_radiance(|
|`_normalize_debevec_hdr_to_unit_range`|fn|priv|4927-4946|def _normalize_debevec_hdr_to_unit_range(np_module, hdr_r...|
|`_run_opencv_hdr_merge`|fn|priv|4947-4953|def _run_opencv_hdr_merge(|
|`_hdrplus_box_down2_float32`|fn|priv|5014-5042|def _hdrplus_box_down2_float32(np_module, frames_float32)|
|`_hdrplus_gauss_down4_float32`|fn|priv|5043-5089|def _hdrplus_gauss_down4_float32(np_module, frames_float32)|
|`_hdrplus_build_scalar_proxy_float32`|fn|priv|5090-5123|def _hdrplus_build_scalar_proxy_float32(np_module, frames...|
|`_hdrplus_compute_tile_start_positions`|fn|priv|5124-5144|def _hdrplus_compute_tile_start_positions(np_module, axis...|
|`_hdrplus_trunc_divide_int32`|fn|priv|5145-5163|def _hdrplus_trunc_divide_int32(np_module, values_int32, ...|
|`_hdrplus_compute_alignment_bounds`|fn|priv|5164-5188|def _hdrplus_compute_alignment_bounds(search_radius)|
|`_hdrplus_compute_alignment_margin`|fn|priv|5189-5207|def _hdrplus_compute_alignment_margin(search_radius, divi...|
|`_hdrplus_extract_overlapping_tiles`|fn|priv|5208-5213|def _hdrplus_extract_overlapping_tiles(|
|`_hdrplus_extract_aligned_tiles`|fn|priv|5266-5272|def _hdrplus_extract_aligned_tiles(|
|`_hdrplus_align_layer`|fn|priv|5345-5352|def _hdrplus_align_layer(|
|`_hdrplus_align_layers`|fn|priv|5442-5529|def _hdrplus_align_layers(np_module, scalar_frames, hdrpl...|
|`_hdrplus_resolve_temporal_runtime_options`|fn|priv|5530-5554|def _hdrplus_resolve_temporal_runtime_options(hdrplus_opt...|
|`_hdrplus_compute_temporal_weights`|fn|priv|5555-5559|def _hdrplus_compute_temporal_weights(|
|`_hdrplus_merge_temporal_rgb`|fn|priv|5640-5646|def _hdrplus_merge_temporal_rgb(|
|`_hdrplus_merge_spatial_rgb`|fn|priv|5695-5767|def _hdrplus_merge_spatial_rgb(np_module, temporal_tiles,...|
|`_run_hdr_plus_merge`|fn|priv|5768-5771|def _run_hdr_plus_merge(|
|`_convert_compression_to_quality`|fn|priv|5848-5858|def _convert_compression_to_quality(jpg_compression)|
|`_collect_missing_external_executables`|fn|priv|5859-5861|def _collect_missing_external_executables(|
|`_resolve_auto_adjust_dependencies`|fn|priv|5880-5905|def _resolve_auto_adjust_dependencies()|
|`_resolve_numpy_dependency`|fn|priv|5906-5925|def _resolve_numpy_dependency()|
|`_to_float32_image_array`|fn|priv|5926-5957|def _to_float32_image_array(np_module, image_data)|
|`_normalize_float_rgb_image`|fn|priv|5958-5985|def _normalize_float_rgb_image(np_module, image_data)|
|`_write_rgb_float_tiff16`|fn|priv|5986-6009|def _write_rgb_float_tiff16(imageio_module, np_module, ou...|
|`_write_debug_rgb_float_tiff`|fn|priv|6010-6015|def _write_debug_rgb_float_tiff(|
|`_build_debug_artifact_context`|fn|priv|6045-6065|def _build_debug_artifact_context(output_jpg, input_dng, ...|
|`_format_debug_ev_suffix_value`|fn|priv|6066-6083|def _format_debug_ev_suffix_value(ev_value)|
|`_materialize_bracket_tiffs_from_float`|fn|priv|6084-6088|def _materialize_bracket_tiffs_from_float(|
|`_to_uint8_image_array`|fn|priv|6118-6164|def _to_uint8_image_array(np_module, image_data)|
|`_to_uint16_image_array`|fn|priv|6165-6209|def _to_uint16_image_array(np_module, image_data)|
|`_apply_post_gamma_float`|fn|priv|6210-6235|def _apply_post_gamma_float(np_module, image_rgb_float, g...|
|`_apply_brightness_float`|fn|priv|6236-6258|def _apply_brightness_float(np_module, image_rgb_float, b...|
|`_apply_contrast_float`|fn|priv|6259-6283|def _apply_contrast_float(np_module, image_rgb_float, con...|
|`_apply_saturation_float`|fn|priv|6284-6313|def _apply_saturation_float(np_module, image_rgb_float, s...|
|`_apply_static_postprocess_float`|fn|priv|6314-6319|def _apply_static_postprocess_float(|
|`_to_linear_srgb`|fn|priv|6396-6413|def _to_linear_srgb(np_module, image_srgb)|
|`_from_linear_srgb`|fn|priv|6414-6431|def _from_linear_srgb(np_module, image_linear)|
|`_compute_bt709_luminance`|fn|priv|6432-6449|def _compute_bt709_luminance(np_module, linear_rgb)|
|`_analyze_luminance_key`|fn|priv|6450-6489|def _analyze_luminance_key(np_module, luminance, eps)|
|`_choose_auto_key_value`|fn|priv|6490-6535|def _choose_auto_key_value(key_analysis, auto_brightness_...|
|`_reinhard_global_tonemap_luminance`|fn|priv|6536-6541|def _reinhard_global_tonemap_luminance(|
|`_luminance_preserving_desaturate_to_fit`|fn|priv|6575-6602|def _luminance_preserving_desaturate_to_fit(np_module, rg...|
|`_apply_mild_local_contrast_bgr_uint16`|fn|priv|6603-6641|def _apply_mild_local_contrast_bgr_uint16(cv2_module, np_...|
|`_quantize_clahe_luminance_bins`|fn|priv|6642-6667|def _quantize_clahe_luminance_bins(np_module, luminance_v...|
|`_build_clahe_float_tile_histogram`|fn|priv|6668-6689|def _build_clahe_float_tile_histogram(np_module, luminanc...|
|`_clip_clahe_histogram`|fn|priv|6690-6737|def _clip_clahe_histogram(np_module, histogram, clip_limi...|
|`_build_clahe_float_lut`|fn|priv|6738-6757|def _build_clahe_float_lut(np_module, histogram, tile_pop...|
|`_pad_clahe_luminance_float`|fn|priv|6758-6796|def _pad_clahe_luminance_float(np_module, luminance_float...|
|`_build_clahe_axis_interpolation`|fn|priv|6797-6824|def _build_clahe_axis_interpolation(np_module, axis_lengt...|
|`_build_clahe_tile_luts_float`|fn|priv|6825-6876|def _build_clahe_tile_luts_float(np_module, luminance_flo...|
|`_interpolate_clahe_bilinear_float`|fn|priv|6877-6929|def _interpolate_clahe_bilinear_float(np_module, luminanc...|
|`_apply_clahe_luminance_float`|fn|priv|6930-6961|def _apply_clahe_luminance_float(np_module, luminance_flo...|
|`_reconstruct_rgb_from_ycrcb_luma_float`|fn|priv|6962-6985|def _reconstruct_rgb_from_ycrcb_luma_float(cv2_module, np...|
|`_apply_clahe_luma_rgb_float`|fn|priv|6986-7035|def _apply_clahe_luma_rgb_float(cv2_module, np_module, im...|
|`_rt_gamma2`|fn|priv|7036-7055|def _rt_gamma2(np_module, values)|
|`_rt_igamma2`|fn|priv|7056-7076|def _rt_igamma2(np_module, values)|
|`_auto_levels_index_to_normalized_value`|fn|priv|7077-7093|def _auto_levels_index_to_normalized_value(histogram_valu...|
|`_auto_levels_normalized_to_legacy_code_value`|fn|priv|7094-7109|def _auto_levels_normalized_to_legacy_code_value(value)|
|`_auto_levels_normalized_to_legacy_code`|fn|priv|7110-7126|def _auto_levels_normalized_to_legacy_code(np_module, val...|
|`_auto_levels_legacy_code_to_normalized`|fn|priv|7127-7142|def _auto_levels_legacy_code_to_normalized(np_module, val...|
|`_pack_auto_levels_metrics`|fn|priv|7143-7158|def _pack_auto_levels_metrics(|
|`_build_autoexp_histogram_rgb_float`|fn|priv|7210-7245|def _build_autoexp_histogram_rgb_float(np_module, image_r...|
|`_build_autoexp_histogram_rgb_uint16`|fn|priv|7246-7278|def _build_autoexp_histogram_rgb_uint16(np_module, image_...|
|`_compute_auto_levels_from_histogram`|fn|priv|7279-7478|def _compute_auto_levels_from_histogram(np_module, histog...|
|`_call_auto_levels_compat_helper`|fn|priv|7552-7557|def _call_auto_levels_compat_helper(|
|`_apply_auto_levels_float`|fn|priv|7601-7700|def _apply_auto_levels_float(np_module, image_rgb_float, ...|
|`_clip_auto_levels_out_of_gamut_float`|fn|priv|7701-7719|def _clip_auto_levels_out_of_gamut_float(np_module, image...|
|`_clip_auto_levels_out_of_gamut_uint16`|fn|priv|7720-7721|def _clip_auto_levels_out_of_gamut_uint16(|
|`_hlrecovery_luminance_float`|fn|priv|7751-7797|def _hlrecovery_luminance_float(np_module, image_rgb, max...|
|`_hlrecovery_luminance_uint16`|fn|priv|7798-7826|def _hlrecovery_luminance_uint16(np_module, image_rgb, ma...|
|`_hlrecovery_cielab_float`|fn|priv|7827-7828|def _hlrecovery_cielab_float(|
|`_f_lab`|fn|priv|7861-7868|def _f_lab(values)|
|`_f2xyz`|fn|priv|7869-7875|def _f2xyz(values)|
|`_hlrecovery_cielab_uint16`|fn|priv|7911-7912|def _hlrecovery_cielab_uint16(|
|`_hlrecovery_blend_float`|fn|priv|7946-8051|def _hlrecovery_blend_float(np_module, image_rgb, hlmax, ...|
|`_hlrecovery_blend_uint16`|fn|priv|8052-8053|def _hlrecovery_blend_uint16(|
|`_dilate_mask_float`|fn|priv|8088-8110|def _dilate_mask_float(np_module, mask)|
|`_box_mean_3x3_float`|fn|priv|8111-8134|def _box_mean_3x3_float(np_module, image_2d)|
|`_hlrecovery_color_propagation_float`|fn|priv|8135-8179|def _hlrecovery_color_propagation_float(np_module, image_...|
|`_hlrecovery_color_propagation_uint16`|fn|priv|8180-8181|def _hlrecovery_color_propagation_uint16(|
|`_hlrecovery_inpaint_opposed_float`|fn|priv|8211-8212|def _hlrecovery_inpaint_opposed_float(|
|`_hlrecovery_inpaint_opposed_uint16`|fn|priv|8265-8266|def _hlrecovery_inpaint_opposed_uint16(|
|`_apply_auto_brightness_rgb_float`|fn|priv|8308-8311|def _apply_auto_brightness_rgb_float(|
|`_clamp01`|fn|priv|8368-8381|def _clamp01(np_module, values)|
|`_gaussian_kernel_2d`|fn|priv|8382-8404|def _gaussian_kernel_2d(np_module, sigma, radius=None)|
|`_rgb_to_hsl`|fn|priv|8405-8438|def _rgb_to_hsl(np_module, rgb)|
|`_hue_to_rgb`|fn|priv|8439-8469|def _hue_to_rgb(np_module, p_values, q_values, t_values)|
|`_hsl_to_rgb`|fn|priv|8470-8510|def _hsl_to_rgb(np_module, hue, saturation, lightness)|
|`_selective_blur_contrast_gated_vectorized`|fn|priv|8511-8512|def _selective_blur_contrast_gated_vectorized(|
|`_level_per_channel_adaptive`|fn|priv|8561-8583|def _level_per_channel_adaptive(np_module, rgb, low_pct=0...|
|`_sigmoidal_contrast`|fn|priv|8584-8608|def _sigmoidal_contrast(np_module, rgb, contrast=3.0, mid...|
|`logistic`|fn|pub|8599-8601|def logistic(z_values)|
|`_vibrance_hsl_gamma`|fn|priv|8609-8626|def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=...|
|`_gaussian_blur_rgb`|fn|priv|8627-8650|def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)|
|`_high_pass_math_gray`|fn|priv|8651-8674|def _high_pass_math_gray(cv2_module, np_module, rgb, blur...|
|`_overlay_composite`|fn|priv|8675-8696|def _overlay_composite(np_module, base_rgb, overlay_gray)|
|`_apply_validated_auto_adjust_pipeline`|fn|priv|8697-8703|def _apply_validated_auto_adjust_pipeline(|
|`_load_piexif_dependency`|fn|priv|8812-8829|def _load_piexif_dependency()|
|`_encode_jpg`|fn|priv|8830-8841|def _encode_jpg(|
|`_collect_processing_errors`|fn|priv|8973-9001|def _collect_processing_errors(rawpy_module)|
|`_is_supported_runtime_os`|fn|priv|9002-9021|def _is_supported_runtime_os()|
|`run`|fn|pub|9022-9221|def run(args)|
