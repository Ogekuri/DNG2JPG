...
# dng2jpg.py | Python | 5141L | 158 symbols | 20 imports | 106 comments
...
- var `DEFAULT_AL_CLIP_PERCENT = 0.02` (L54)
- var `DEFAULT_AL_HISTCOMPR = 3` (L55)
- var `DEFAULT_LUMINANCE_HDR_MODEL = "debevec"` (L61)
- var `DEFAULT_LUMINANCE_HDR_WEIGHT = "flat"` (L62)
- var `DEFAULT_LUMINANCE_HDR_RESPONSE_CURVE = "srgb"` (L63)
- var `DEFAULT_LUMINANCE_TMO = "mantiuk08"` (L64)
- var `DEFAULT_REINHARD02_BRIGHTNESS = 1.25` (L65)
- var `DEFAULT_REINHARD02_CONTRAST = 0.85` (L66)
- var `DEFAULT_REINHARD02_SATURATION = 0.55` (L67)
- var `DEFAULT_MANTIUK08_CONTRAST = 1.2` (L68)
- var `EV_STEP = 0.25` (L69)
- var `MIN_SUPPORTED_BITS_PER_COLOR = 9` (L70)
- var `DEFAULT_DNG_BITS_PER_COLOR = 14` (L71)
- var `SUPPORTED_EV_VALUES = tuple(` (L72)
- var `AUTO_EV_LOW_PERCENTILE = 0.1` (L78)
- var `AUTO_EV_HIGH_PERCENTILE = 99.9` (L79)
- var `AUTO_EV_MEDIAN_PERCENTILE = 50.0` (L80)
- var `AUTO_EV_TARGET_SHADOW = 0.05` (L81)
- var `AUTO_EV_TARGET_HIGHLIGHT = 0.90` (L82)
- var `AUTO_EV_MEDIAN_TARGET = 0.5` (L83)
- var `AUTO_ZERO_SCENE_KEY_LOW_THRESHOLD = 0.35` (L84)
- var `AUTO_ZERO_SCENE_KEY_HIGH_THRESHOLD = 0.65` (L85)
- var `AUTO_ZERO_TARGET_LOW_KEY = 0.35` (L86)
- var `AUTO_ZERO_TARGET_HIGH_KEY = 0.65` (L87)
...
### class `class AutoLevelsOptions` `@dataclass(frozen=True)` (L313-332)
...
### class `class PostprocessOptions` `@dataclass(frozen=True)` (L334-368)
...
### class `class LuminanceOptions` `@dataclass(frozen=True)` (L370-390)
...
### class `class AutoEvInputs` `@dataclass(frozen=True)` (L392-419)
...
### fn `def _print_box_table(headers, rows, header_rows=())` `priv` (L420-456)
...
### fn `def _border(left, middle, right)` `priv` (L440-442)
- @brief Print one Unicode box-drawing table.
...
### fn `def _line(values)` `priv` (L443-446)

### fn `def _build_two_line_operator_rows(operator_entries)` `priv` (L457-473)
...
### fn `def print_help(version)` (L474-673)
...
### fn `def _calculate_max_ev_from_bits(bits_per_color)` `priv` (L681-699)
...
### fn `def _calculate_safe_ev_zero_max(base_max_ev)` `priv` (L700-712)
...
### fn `def _derive_supported_ev_zero_values(base_max_ev)` `priv` (L713-729)
...
### fn `def _derive_supported_ev_values(bits_per_color, ev_zero=0.0)` `priv` (L730-758)
...
### fn `def _detect_dng_bits_per_color(raw_handle)` `priv` (L759-804)
...
### fn `def _is_ev_value_on_supported_step(ev_value)` `priv` (L805-818)
...
### fn `def _parse_ev_option(ev_raw)` `priv` (L819-850)
...
### fn `def _parse_ev_zero_option(ev_zero_raw)` `priv` (L851-881)
...
### fn `def _parse_auto_ev_option(auto_ev_raw)` `priv` (L882-899)
...
### fn `def _parse_auto_zero_option(auto_zero_raw)` `priv` (L900-917)
...
### fn `def _parse_percentage_option(option_name, option_raw)` `priv` (L918-940)
...
### fn `def _parse_auto_brightness_option(auto_brightness_raw)` `priv` (L941-958)
...
### fn `def _parse_auto_levels_option(auto_levels_raw)` `priv` (L959-976)
...
### fn `def _clamp_ev_to_supported(ev_candidate, ev_values)` `priv` (L977-990)
...
### fn `def _quantize_ev_to_supported(ev_value, ev_values)` `priv` (L991-1012)
...
### fn `def _quantize_ev_toward_zero_step(ev_value, step=EV_STEP)` `priv` (L1013-1034)
...
### fn `def _apply_auto_percentage_scaling(ev_value, percentage)` `priv` (L1035-1049)
...
### fn `def _extract_normalized_preview_luminance_stats(raw_handle)` `priv` (L1050-1109)
...
### fn `def _percentile(percentile_value)` `priv` (L1084-1094)
- @brief Extract normalized preview luminance percentiles from RAW handle.
...
### fn `def _coerce_positive_luminance(value, fallback)` `priv` (L1110-1129)
...
### fn `def _derive_scene_key_preserving_median_target(p_median)` `priv` (L1130-1148)
...
### fn `def _optimize_auto_zero(auto_ev_inputs)` `priv` (L1149-1172)
...
### fn `def _optimize_adaptive_ev_delta(auto_ev_inputs)` `priv` (L1173-1202)
...
### fn `def _compute_auto_ev_value_from_stats(` `priv` (L1203-1208)

### fn `def _compute_auto_ev_value(raw_handle, supported_ev_values=None, ev_zero=0.0)` `priv` (L1236-1263)
...
### fn `def _resolve_ev_zero(` `priv` (L1264-1271)

### fn `def _resolve_ev_value(` `priv` (L1322-1329)
...
### fn `def _parse_luminance_text_option(option_name, option_raw)` `priv` (L1382-1402)
...
### fn `def _parse_gamma_option(gamma_raw)` `priv` (L1403-1439)
...
### fn `def _parse_positive_float_option(option_name, option_raw)` `priv` (L1440-1463)
...
### fn `def _parse_tmo_passthrough_value(option_name, option_raw)` `priv` (L1464-1480)
...
### fn `def _parse_jpg_compression_option(compression_raw)` `priv` (L1481-1503)
...
### fn `def _parse_float_exclusive_range_option(option_name, option_raw, min_value, max_value)` `priv` (L1504-1528)
...
### fn `def _parse_non_negative_float_option(option_name, option_raw)` `priv` (L1529-1551)
...
### fn `def _parse_float_in_range_option(option_name, option_raw, min_value, max_value)` `priv` (L1552-1577)
...
### fn `def _parse_auto_brightness_options(auto_brightness_raw_values)` `priv` (L1578-1601)
...
### fn `def _parse_auto_levels_hr_method_option(auto_levels_method_raw)` `priv` (L1602-1633)
...
### fn `def _parse_auto_levels_options(auto_levels_raw_values)` `priv` (L1634-1673)
...
### fn `def _parse_auto_adjust_options(auto_adjust_raw_values)` `priv` (L1674-1780)
...
### fn `def _parse_auto_adjust_mode_option(auto_adjust_raw)` `priv` (L1781-1804)
...
### fn `def _resolve_default_postprocess(enable_luminance, luminance_tmo)` `priv` (L1805-1847)
...
### fn `def _parse_run_options(args)` `priv` (L1848-2047)
...
### fn `def _load_image_dependencies()` `priv` (L2537-2575)
...
### fn `def _parse_exif_datetime_to_timestamp(datetime_raw)` `priv` (L2576-2606)
...
### fn `def _extract_dng_exif_payload_and_timestamp(pil_image_module, input_dng)` `priv` (L2607-2664)
...
### fn `def _resolve_thumbnail_transpose_map(pil_image_module)` `priv` (L2665-2696)
...
### fn `def _apply_orientation_transform(pil_image_module, pil_image, source_orientation)` `priv` (L2697-2719)
...
### fn `def _build_oriented_thumbnail_jpeg_bytes(` `priv` (L2720-2721)

### fn `def _coerce_exif_int_like_value(raw_value)` `priv` (L2750-2792)
...
### fn `def _normalize_ifd_integer_like_values_for_piexif_dump(piexif_module, exif_dict)` `priv` (L2793-2926)
...
### fn `def _refresh_output_jpg_exif_thumbnail_after_save(` `priv` (L2927-2932)

### fn `def _set_output_file_timestamps(output_jpg, exif_timestamp)` `priv` (L2981-2995)
...
### fn `def _sync_output_file_timestamps_from_exif(output_jpg, exif_timestamp)` `priv` (L2996-3012)
...
### fn `def _build_exposure_multipliers(ev_value, ev_zero=0.0)` `priv` (L3013-3031)
...
### fn `def _write_bracket_images(` `priv` (L3032-3033)

### fn `def _order_bracket_paths(bracket_paths)` `priv` (L3070-3095)
...
### fn `def _run_enfuse(bracket_paths, merged_tiff)` `priv` (L3096-3116)
...
### fn `def _run_luminance_hdr_cli(` `priv` (L3117-3118)

### fn `def _convert_compression_to_quality(jpg_compression)` `priv` (L3170-3182)
...
### fn `def _resolve_imagemagick_command()` `priv` (L3183-3200)
...
### fn `def _resolve_auto_adjust_opencv_dependencies()` `priv` (L3201-3225)
...
### fn `def _resolve_numpy_dependency()` `priv` (L3226-3243)
...
### fn `def _to_uint8_image_array(np_module, image_data)` `priv` (L3244-3290)
...
### fn `def _to_uint16_image_array(np_module, image_data)` `priv` (L3291-3335)
...
### fn `def _to_linear_srgb(np_module, image_srgb)` `priv` (L3336-3353)
...
### fn `def _from_linear_srgb(np_module, image_linear)` `priv` (L3354-3371)
...
### fn `def _compute_bt709_luminance(np_module, linear_rgb)` `priv` (L3372-3389)
...
### fn `def _apply_highlight_rolloff(np_module, linear_rgb)` `priv` (L3390-3408)
...
### fn `def _rt_gamma2(np_module, values)` `priv` (L3409-3428)
...
### fn `def _rt_igamma2(np_module, values)` `priv` (L3429-3449)
...
### fn `def _build_autoexp_histogram_rgb_uint16(np_module, image_rgb_uint16, histcompr)` `priv` (L3450-3476)
...
### fn `def _compute_auto_levels_from_histogram(np_module, histogram, histcompr, clip_percent)` `priv` (L3477-3676)
...
### fn `def _apply_auto_levels_uint16(np_module, image_rgb_uint16, auto_levels_options)` `priv` (L3730-3794)
...
### fn `def _hlrecovery_luminance_uint16(np_module, image_rgb, maxval=65535.0)` `priv` (L3795-3840)
...
### fn `def _hlrecovery_cielab_uint16(` `priv` (L3841-3842)

### fn `def _f_lab(values)` `priv` (L3875-3882)
...
### fn `def _f2xyz(values)` `priv` (L3883-3889)

### fn `def _hlrecovery_blend_uint16(np_module, image_rgb, hlmax, maxval=65535.0)` `priv` (L3925-4027)
...
### fn `def _apply_auto_brightness_rgb_uint8(np_module, image_rgb_uint8, auto_brightness_options)` `priv` (L4028-4083)
- @brief Apply BT.709 linear-sRGB auto-brightness on uint16 RGB tensor.
- @details Executes pipeline: uint16 normalization to `[0,1]` in float64, sRGB linearization, BT.709 luminance extraction, percentile statistics (`p50`, `p98`), raw gain `target_grey/p50`, EV-domain light correction `clamp(log2(raw_gain)*correction_strength,0,+max_ev_correction)`, global gain `2^ev` constrained by fixed gain cap `4.0` and highlight-safe cap `max(0.95,0.90/max(p98,1e-7))`, highlight rolloff, inverse sRGB transfer, and uint16 restoration.
- @param np_module {ModuleType} Imported numpy module.
...
### fn `def _apply_validated_auto_adjust_pipeline(` `priv` (L4084-4085)

### fn `def _clamp01(np_module, values)` `priv` (L4166-4179)
...
### fn `def _gaussian_kernel_2d(np_module, sigma, radius=None)` `priv` (L4180-4202)
...
### fn `def _rgb_to_hsl(np_module, rgb)` `priv` (L4203-4236)
...
### fn `def _hue_to_rgb(np_module, p_values, q_values, t_values)` `priv` (L4237-4267)
...
### fn `def _hsl_to_rgb(np_module, hue, saturation, lightness)` `priv` (L4268-4308)
...
### fn `def _selective_blur_contrast_gated_vectorized(` `priv` (L4309-4310)

### fn `def _level_per_channel_adaptive(np_module, rgb, low_pct=0.1, high_pct=99.9)` `priv` (L4359-4381)
...
### fn `def _sigmoidal_contrast(np_module, rgb, contrast=3.0, midpoint=0.5)` `priv` (L4382-4406)
...
### fn `def logistic(z_values)` (L4397-4399)
- @brief Execute sigmoidal contrast stage.
...
### fn `def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=0.8)` `priv` (L4407-4424)
...
### fn `def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)` `priv` (L4425-4448)
...
### fn `def _high_pass_math_gray(cv2_module, np_module, rgb, blur_sigma=2.5)` `priv` (L4449-4472)
...
### fn `def _overlay_composite(np_module, base_rgb, overlay_gray)` `priv` (L4473-4494)
...
### fn `def _apply_validated_auto_adjust_pipeline_opencv(` `priv` (L4495-4496)

### fn `def _load_piexif_dependency()` `priv` (L4575-4592)
...
### fn `def _encode_jpg(` `priv` (L4593-4606)

### fn `def _collect_processing_errors(rawpy_module)` `priv` (L4851-4879)
...
### fn `def _is_supported_runtime_os()` `priv` (L4880-4899)
...
|`DEFAULT_AL_CLIP_PERCENT`|var|pub|54||
|`DEFAULT_AL_HISTCOMPR`|var|pub|55||
|`DEFAULT_LUMINANCE_HDR_MODEL`|var|pub|61||
|`DEFAULT_LUMINANCE_HDR_WEIGHT`|var|pub|62||
|`DEFAULT_LUMINANCE_HDR_RESPONSE_CURVE`|var|pub|63||
|`DEFAULT_LUMINANCE_TMO`|var|pub|64||
|`DEFAULT_REINHARD02_BRIGHTNESS`|var|pub|65||
|`DEFAULT_REINHARD02_CONTRAST`|var|pub|66||
|`DEFAULT_REINHARD02_SATURATION`|var|pub|67||
|`DEFAULT_MANTIUK08_CONTRAST`|var|pub|68||
|`EV_STEP`|var|pub|69||
|`MIN_SUPPORTED_BITS_PER_COLOR`|var|pub|70||
|`DEFAULT_DNG_BITS_PER_COLOR`|var|pub|71||
|`SUPPORTED_EV_VALUES`|var|pub|72||
|`AUTO_EV_LOW_PERCENTILE`|var|pub|78||
|`AUTO_EV_HIGH_PERCENTILE`|var|pub|79||
|`AUTO_EV_MEDIAN_PERCENTILE`|var|pub|80||
|`AUTO_EV_TARGET_SHADOW`|var|pub|81||
|`AUTO_EV_TARGET_HIGHLIGHT`|var|pub|82||
|`AUTO_EV_MEDIAN_TARGET`|var|pub|83||
|`AUTO_ZERO_SCENE_KEY_LOW_THRESHOLD`|var|pub|84||
|`AUTO_ZERO_SCENE_KEY_HIGH_THRESHOLD`|var|pub|85||
|`AUTO_ZERO_TARGET_LOW_KEY`|var|pub|86||
|`AUTO_ZERO_TARGET_HIGH_KEY`|var|pub|87||
...
|`AutoLevelsOptions`|class|pub|313-332|class AutoLevelsOptions|
|`PostprocessOptions`|class|pub|334-368|class PostprocessOptions|
|`LuminanceOptions`|class|pub|370-390|class LuminanceOptions|
|`AutoEvInputs`|class|pub|392-419|class AutoEvInputs|
|`_print_box_table`|fn|priv|420-456|def _print_box_table(headers, rows, header_rows=())|
|`_border`|fn|priv|440-442|def _border(left, middle, right)|
|`_line`|fn|priv|443-446|def _line(values)|
|`_build_two_line_operator_rows`|fn|priv|457-473|def _build_two_line_operator_rows(operator_entries)|
|`print_help`|fn|pub|474-673|def print_help(version)|
|`_calculate_max_ev_from_bits`|fn|priv|681-699|def _calculate_max_ev_from_bits(bits_per_color)|
|`_calculate_safe_ev_zero_max`|fn|priv|700-712|def _calculate_safe_ev_zero_max(base_max_ev)|
|`_derive_supported_ev_zero_values`|fn|priv|713-729|def _derive_supported_ev_zero_values(base_max_ev)|
|`_derive_supported_ev_values`|fn|priv|730-758|def _derive_supported_ev_values(bits_per_color, ev_zero=0.0)|
|`_detect_dng_bits_per_color`|fn|priv|759-804|def _detect_dng_bits_per_color(raw_handle)|
|`_is_ev_value_on_supported_step`|fn|priv|805-818|def _is_ev_value_on_supported_step(ev_value)|
|`_parse_ev_option`|fn|priv|819-850|def _parse_ev_option(ev_raw)|
|`_parse_ev_zero_option`|fn|priv|851-881|def _parse_ev_zero_option(ev_zero_raw)|
|`_parse_auto_ev_option`|fn|priv|882-899|def _parse_auto_ev_option(auto_ev_raw)|
|`_parse_auto_zero_option`|fn|priv|900-917|def _parse_auto_zero_option(auto_zero_raw)|
|`_parse_percentage_option`|fn|priv|918-940|def _parse_percentage_option(option_name, option_raw)|
|`_parse_auto_brightness_option`|fn|priv|941-958|def _parse_auto_brightness_option(auto_brightness_raw)|
|`_parse_auto_levels_option`|fn|priv|959-976|def _parse_auto_levels_option(auto_levels_raw)|
|`_clamp_ev_to_supported`|fn|priv|977-990|def _clamp_ev_to_supported(ev_candidate, ev_values)|
|`_quantize_ev_to_supported`|fn|priv|991-1012|def _quantize_ev_to_supported(ev_value, ev_values)|
|`_quantize_ev_toward_zero_step`|fn|priv|1013-1034|def _quantize_ev_toward_zero_step(ev_value, step=EV_STEP)|
|`_apply_auto_percentage_scaling`|fn|priv|1035-1049|def _apply_auto_percentage_scaling(ev_value, percentage)|
|`_extract_normalized_preview_luminance_stats`|fn|priv|1050-1109|def _extract_normalized_preview_luminance_stats(raw_handle)|
|`_percentile`|fn|priv|1084-1094|def _percentile(percentile_value)|
|`_coerce_positive_luminance`|fn|priv|1110-1129|def _coerce_positive_luminance(value, fallback)|
|`_derive_scene_key_preserving_median_target`|fn|priv|1130-1148|def _derive_scene_key_preserving_median_target(p_median)|
|`_optimize_auto_zero`|fn|priv|1149-1172|def _optimize_auto_zero(auto_ev_inputs)|
|`_optimize_adaptive_ev_delta`|fn|priv|1173-1202|def _optimize_adaptive_ev_delta(auto_ev_inputs)|
|`_compute_auto_ev_value_from_stats`|fn|priv|1203-1208|def _compute_auto_ev_value_from_stats(|
|`_compute_auto_ev_value`|fn|priv|1236-1263|def _compute_auto_ev_value(raw_handle, supported_ev_value...|
|`_resolve_ev_zero`|fn|priv|1264-1271|def _resolve_ev_zero(|
|`_resolve_ev_value`|fn|priv|1322-1329|def _resolve_ev_value(|
|`_parse_luminance_text_option`|fn|priv|1382-1402|def _parse_luminance_text_option(option_name, option_raw)|
|`_parse_gamma_option`|fn|priv|1403-1439|def _parse_gamma_option(gamma_raw)|
|`_parse_positive_float_option`|fn|priv|1440-1463|def _parse_positive_float_option(option_name, option_raw)|
|`_parse_tmo_passthrough_value`|fn|priv|1464-1480|def _parse_tmo_passthrough_value(option_name, option_raw)|
|`_parse_jpg_compression_option`|fn|priv|1481-1503|def _parse_jpg_compression_option(compression_raw)|
|`_parse_float_exclusive_range_option`|fn|priv|1504-1528|def _parse_float_exclusive_range_option(option_name, opti...|
|`_parse_non_negative_float_option`|fn|priv|1529-1551|def _parse_non_negative_float_option(option_name, option_...|
|`_parse_float_in_range_option`|fn|priv|1552-1577|def _parse_float_in_range_option(option_name, option_raw,...|
|`_parse_auto_brightness_options`|fn|priv|1578-1601|def _parse_auto_brightness_options(auto_brightness_raw_va...|
|`_parse_auto_levels_hr_method_option`|fn|priv|1602-1633|def _parse_auto_levels_hr_method_option(auto_levels_metho...|
|`_parse_auto_levels_options`|fn|priv|1634-1673|def _parse_auto_levels_options(auto_levels_raw_values)|
|`_parse_auto_adjust_options`|fn|priv|1674-1780|def _parse_auto_adjust_options(auto_adjust_raw_values)|
|`_parse_auto_adjust_mode_option`|fn|priv|1781-1804|def _parse_auto_adjust_mode_option(auto_adjust_raw)|
|`_resolve_default_postprocess`|fn|priv|1805-1847|def _resolve_default_postprocess(enable_luminance, lumina...|
|`_parse_run_options`|fn|priv|1848-2047|def _parse_run_options(args)|
|`_load_image_dependencies`|fn|priv|2537-2575|def _load_image_dependencies()|
|`_parse_exif_datetime_to_timestamp`|fn|priv|2576-2606|def _parse_exif_datetime_to_timestamp(datetime_raw)|
|`_extract_dng_exif_payload_and_timestamp`|fn|priv|2607-2664|def _extract_dng_exif_payload_and_timestamp(pil_image_mod...|
|`_resolve_thumbnail_transpose_map`|fn|priv|2665-2696|def _resolve_thumbnail_transpose_map(pil_image_module)|
|`_apply_orientation_transform`|fn|priv|2697-2719|def _apply_orientation_transform(pil_image_module, pil_im...|
|`_build_oriented_thumbnail_jpeg_bytes`|fn|priv|2720-2721|def _build_oriented_thumbnail_jpeg_bytes(|
|`_coerce_exif_int_like_value`|fn|priv|2750-2792|def _coerce_exif_int_like_value(raw_value)|
|`_normalize_ifd_integer_like_values_for_piexif_dump`|fn|priv|2793-2926|def _normalize_ifd_integer_like_values_for_piexif_dump(pi...|
|`_refresh_output_jpg_exif_thumbnail_after_save`|fn|priv|2927-2932|def _refresh_output_jpg_exif_thumbnail_after_save(|
|`_set_output_file_timestamps`|fn|priv|2981-2995|def _set_output_file_timestamps(output_jpg, exif_timestamp)|
|`_sync_output_file_timestamps_from_exif`|fn|priv|2996-3012|def _sync_output_file_timestamps_from_exif(output_jpg, ex...|
|`_build_exposure_multipliers`|fn|priv|3013-3031|def _build_exposure_multipliers(ev_value, ev_zero=0.0)|
|`_write_bracket_images`|fn|priv|3032-3033|def _write_bracket_images(|
|`_order_bracket_paths`|fn|priv|3070-3095|def _order_bracket_paths(bracket_paths)|
|`_run_enfuse`|fn|priv|3096-3116|def _run_enfuse(bracket_paths, merged_tiff)|
|`_run_luminance_hdr_cli`|fn|priv|3117-3118|def _run_luminance_hdr_cli(|
|`_convert_compression_to_quality`|fn|priv|3170-3182|def _convert_compression_to_quality(jpg_compression)|
|`_resolve_imagemagick_command`|fn|priv|3183-3200|def _resolve_imagemagick_command()|
|`_resolve_auto_adjust_opencv_dependencies`|fn|priv|3201-3225|def _resolve_auto_adjust_opencv_dependencies()|
|`_resolve_numpy_dependency`|fn|priv|3226-3243|def _resolve_numpy_dependency()|
|`_to_uint8_image_array`|fn|priv|3244-3290|def _to_uint8_image_array(np_module, image_data)|
|`_to_uint16_image_array`|fn|priv|3291-3335|def _to_uint16_image_array(np_module, image_data)|
|`_to_linear_srgb`|fn|priv|3336-3353|def _to_linear_srgb(np_module, image_srgb)|
|`_from_linear_srgb`|fn|priv|3354-3371|def _from_linear_srgb(np_module, image_linear)|
|`_compute_bt709_luminance`|fn|priv|3372-3389|def _compute_bt709_luminance(np_module, linear_rgb)|
|`_apply_highlight_rolloff`|fn|priv|3390-3408|def _apply_highlight_rolloff(np_module, linear_rgb)|
|`_rt_gamma2`|fn|priv|3409-3428|def _rt_gamma2(np_module, values)|
|`_rt_igamma2`|fn|priv|3429-3449|def _rt_igamma2(np_module, values)|
|`_build_autoexp_histogram_rgb_uint16`|fn|priv|3450-3476|def _build_autoexp_histogram_rgb_uint16(np_module, image_...|
|`_compute_auto_levels_from_histogram`|fn|priv|3477-3676|def _compute_auto_levels_from_histogram(np_module, histog...|
|`_apply_auto_levels_uint16`|fn|priv|3730-3794|def _apply_auto_levels_uint16(np_module, image_rgb_uint16...|
|`_hlrecovery_luminance_uint16`|fn|priv|3795-3840|def _hlrecovery_luminance_uint16(np_module, image_rgb, ma...|
|`_hlrecovery_cielab_uint16`|fn|priv|3841-3842|def _hlrecovery_cielab_uint16(|
|`_f_lab`|fn|priv|3875-3882|def _f_lab(values)|
|`_f2xyz`|fn|priv|3883-3889|def _f2xyz(values)|
|`_hlrecovery_blend_uint16`|fn|priv|3925-4027|def _hlrecovery_blend_uint16(np_module, image_rgb, hlmax,...|
|`_apply_auto_brightness_rgb_uint8`|fn|priv|4028-4083|def _apply_auto_brightness_rgb_uint8(np_module, image_rgb...|
|`_apply_validated_auto_adjust_pipeline`|fn|priv|4084-4085|def _apply_validated_auto_adjust_pipeline(|
|`_clamp01`|fn|priv|4166-4179|def _clamp01(np_module, values)|
|`_gaussian_kernel_2d`|fn|priv|4180-4202|def _gaussian_kernel_2d(np_module, sigma, radius=None)|
|`_rgb_to_hsl`|fn|priv|4203-4236|def _rgb_to_hsl(np_module, rgb)|
|`_hue_to_rgb`|fn|priv|4237-4267|def _hue_to_rgb(np_module, p_values, q_values, t_values)|
|`_hsl_to_rgb`|fn|priv|4268-4308|def _hsl_to_rgb(np_module, hue, saturation, lightness)|
|`_selective_blur_contrast_gated_vectorized`|fn|priv|4309-4310|def _selective_blur_contrast_gated_vectorized(|
|`_level_per_channel_adaptive`|fn|priv|4359-4381|def _level_per_channel_adaptive(np_module, rgb, low_pct=0...|
|`_sigmoidal_contrast`|fn|priv|4382-4406|def _sigmoidal_contrast(np_module, rgb, contrast=3.0, mid...|
|`logistic`|fn|pub|4397-4399|def logistic(z_values)|
|`_vibrance_hsl_gamma`|fn|priv|4407-4424|def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=...|
|`_gaussian_blur_rgb`|fn|priv|4425-4448|def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)|
|`_high_pass_math_gray`|fn|priv|4449-4472|def _high_pass_math_gray(cv2_module, np_module, rgb, blur...|
|`_overlay_composite`|fn|priv|4473-4494|def _overlay_composite(np_module, base_rgb, overlay_gray)|
|`_apply_validated_auto_adjust_pipeline_opencv`|fn|priv|4495-4496|def _apply_validated_auto_adjust_pipeline_opencv(|
|`_load_piexif_dependency`|fn|priv|4575-4592|def _load_piexif_dependency()|
|`_encode_jpg`|fn|priv|4593-4606|def _encode_jpg(|
|`_collect_processing_errors`|fn|priv|4851-4879|def _collect_processing_errors(rawpy_module)|
|`_is_supported_runtime_os`|fn|priv|4880-4899|def _is_supported_runtime_os()|
|`run`|fn|pub|4900-5099|def run(args)|
