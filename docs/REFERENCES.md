...
# dng2jpg.py | Python | 11691L | 333 symbols | 29 imports | 242 comments
...
- var `DEFAULT_REINHARD02_BRIGHTNESS = 1.25` (L123)
- var `DEFAULT_REINHARD02_CONTRAST = 0.85` (L124)
- var `DEFAULT_REINHARD02_SATURATION = 0.55` (L125)
- var `DEFAULT_MANTIUK08_CONTRAST = 1.2` (L126)
- var `DEFAULT_OPENCV_POST_GAMMA = 1.0` (L127)
- var `DEFAULT_OPENCV_BRIGHTNESS = 1.0` (L128)
- var `DEFAULT_OPENCV_CONTRAST = 1.0` (L129)
- var `DEFAULT_OPENCV_SATURATION = 1.0` (L130)
- var `DEFAULT_OPENCV_MERGE_ALGORITHM = OPENCV_MERGE_ALGORITHM_ROBERTSON` (L131)
- var `DEFAULT_OPENCV_TONEMAP_ENABLED = True` (L132)
- var `DEFAULT_OPENCV_TONEMAP_GAMMA = 2.2` (L133)
- var `DEFAULT_HDRPLUS_PROXY_MODE = "rggb"` (L134)
- var `DEFAULT_HDRPLUS_SEARCH_RADIUS = 4` (L135)
- var `DEFAULT_HDRPLUS_TEMPORAL_FACTOR = 8.0` (L136)
- var `DEFAULT_HDRPLUS_TEMPORAL_MIN_DIST = 10.0` (L137)
- var `DEFAULT_HDRPLUS_TEMPORAL_MAX_DIST = 300.0` (L138)
- var `HDRPLUS_TILE_SIZE = 32` (L139)
- var `HDRPLUS_TILE_STRIDE = HDRPLUS_TILE_SIZE // 2` (L140)
- var `HDRPLUS_DOWNSAMPLED_TILE_SIZE = HDRPLUS_TILE_STRIDE` (L141)
- var `HDRPLUS_ALIGNMENT_LEVELS = 3` (L142)
- var `HDRPLUS_ALIGNMENT_DOWNSAMPLE_RATE = 4` (L143)
- var `HDRPLUS_TEMPORAL_FACTOR = DEFAULT_HDRPLUS_TEMPORAL_FACTOR` (L144)
- var `HDRPLUS_TEMPORAL_MIN_DIST = DEFAULT_HDRPLUS_TEMPORAL_MIN_DIST` (L145)
- var `HDRPLUS_TEMPORAL_MAX_DIST = DEFAULT_HDRPLUS_TEMPORAL_MAX_DIST` (L146)
- var `MIN_SUPPORTED_BITS_PER_COLOR = 9` (L148)
### class `class AutoAdjustOptions` `@dataclass(frozen=True)` (L377-412)
...
### class `class AutoBrightnessOptions` `@dataclass(frozen=True)` (L414-442)
...
### class `class AutoLevelsOptions` `@dataclass(frozen=True)` (L444-467)
...
### class `class PostGammaAutoOptions` `@dataclass(frozen=True)` (L469-487)
...
### class `class MergeGammaOption` `@dataclass(frozen=True)` (L489-507)
...
### class `class ResolvedMergeGamma` `@dataclass(frozen=True)` (L509-534)
...
### class `class ExifGammaTags` `@dataclass(frozen=True)` (L536-557)
...
### class `class PostprocessOptions` `@dataclass(frozen=True)` (L559-606)
...
### class `class DebugArtifactContext` `@dataclass(frozen=True)` (L608-624)
...
### class `class SourceGammaInfo` `@dataclass(frozen=True)` (L626-644)
...
### class `class LuminanceOptions` `@dataclass(frozen=True)` (L646-667)
...
### class `class HdrPlusOptions` `@dataclass(frozen=True)` (L691-714)
...
### class `class HdrPlusTemporalRuntimeOptions` `@dataclass(frozen=True)` (L716-735)
...
### class `class JointAutoEvSolution` `@dataclass(frozen=True)` (L737-757)
...
### class `class AutoEvIterationStep` `@dataclass(frozen=True)` (L759-776)
...
### class `class AutoEvOptions` `@dataclass(frozen=True)` (L778-795)
...
### class `class AutoZeroEvaluation` `@dataclass(frozen=True)` (L797-815)
...
### fn `def _print_box_table(headers, rows, header_rows=())` `priv` (L816-852)
...
### fn `def _border(left, middle, right)` `priv` (L836-838)
- @brief Print one Unicode box-drawing table.
...
### fn `def _line(values)` `priv` (L839-842)

### fn `def _build_two_line_operator_rows(operator_entries)` `priv` (L853-869)
...
### fn `def _print_help_section(title)` `priv` (L870-884)
...
### fn `def _print_help_option(option_label, description, detail_lines=())` `priv` (L885-926)
...
### fn `def print_help(version)` (L927-1126)
...
### fn `def _calculate_max_ev_from_bits(bits_per_color)` `priv` (L1336-1354)
...
### fn `def _calculate_safe_ev_zero_max(base_max_ev)` `priv` (L1355-1367)
...
### fn `def _derive_supported_ev_values(` `priv` (L1368-1370)

### fn `def _detect_dng_bits_per_color(raw_handle)` `priv` (L1394-1439)
...
### fn `def _is_ev_value_on_supported_step(ev_value)` `priv` (L1440-1452)
...
### fn `def _parse_ev_option(ev_raw)` `priv` (L1453-1478)
...
### fn `def _parse_ev_zero_option(ev_zero_raw)` `priv` (L1479-1503)
...
### fn `def _parse_auto_ev_option(auto_ev_raw)` `priv` (L1504-1523)
...
### fn `def _parse_percentage_option(option_name, option_raw)` `priv` (L1524-1546)
...
### fn `def _parse_auto_brightness_option(auto_brightness_raw)` `priv` (L1547-1566)
...
### fn `def _parse_auto_levels_option(auto_levels_raw)` `priv` (L1567-1586)
...
### fn `def _parse_explicit_boolean_option(option_name, option_raw)` `priv` (L1587-1607)
...
### fn `def _parse_opencv_merge_algorithm_option(algorithm_raw)` `priv` (L1608-1632)
...
### fn `def _extract_normalized_preview_luminance_stats(raw_handle)` `priv` (L1680-1739)
...
### fn `def _percentile(percentile_value)` `priv` (L1714-1724)
- @brief Extract normalized preview luminance percentiles from RAW handle.
...
### fn `def _extract_base_rgb_linear_float(raw_handle, np_module)` `priv` (L1740-1768)
...
### fn `def _normalize_source_gamma_label(label_raw)` `priv` (L1769-1787)
...
### fn `def _decode_raw_metadata_text(metadata_raw)` `priv` (L1788-1819)
...
### fn `def _classify_explicit_source_gamma(raw_handle)` `priv` (L1820-1869)
...
### fn `def _classify_tone_curve_gamma(raw_handle)` `priv` (L1870-1918)
...
### fn `def _has_nonzero_matrix(matrix_raw)` `priv` (L1919-1939)
...
### fn `def _classify_matrix_hint_gamma(raw_handle)` `priv` (L1940-1970)
...
### fn `def _extract_source_gamma_info(raw_handle)` `priv` (L1971-1998)
...
### fn `def _describe_source_gamma_info(source_gamma_info)` `priv` (L1999-2020)
...
### fn `def _coerce_positive_luminance(value, fallback)` `priv` (L2021-2040)
...
### fn `def _calculate_bt709_luminance(np_module, image_rgb_float)` `priv` (L2041-2063)
...
### fn `def _smoothstep(np_module, values, edge0, edge1)` `priv` (L2064-2082)
...
### fn `def _calculate_entropy_optimized_ev(_cv2_module, np_module, luminance_float)` `priv` (L2083-2128)
...
### fn `def _calculate_ettr_ev(np_module, luminance_float)` `priv` (L2129-2148)
...
### fn `def _calculate_detail_preservation_ev(_cv2_module, np_module, luminance_float)` `priv` (L2149-2216)
...
### fn `def _calculate_auto_zero_evaluations(cv2_module, np_module, image_rgb_float)` `priv` (L2217-2254)
...
### fn `def _select_ev_zero_candidate(evaluations, safe_ev_zero_max)` `priv` (L2255-2284)
...
### fn `def _build_unclipped_bracket_images_from_linear_base_float(` `priv` (L2285-2289)

### fn `def _measure_any_channel_highlight_clipping_pct(np_module, image_rgb_float)` `priv` (L2319-2336)
...
### fn `def _measure_any_channel_shadow_clipping_pct(np_module, image_rgb_float)` `priv` (L2337-2354)
...
### fn `def _resolve_joint_auto_ev_solution(` `priv` (L2355-2361)

### fn `def _parse_luminance_text_option(option_name, option_raw)` `priv` (L2460-2480)
...
### fn `def _parse_luminance_response_curve_option(option_raw)` `priv` (L2481-2507)
...
### fn `def _parse_positive_float_option(option_name, option_raw)` `priv` (L2508-2531)
...
### fn `def _parse_post_gamma_selector_option(option_raw)` `priv` (L2532-2553)
...
### fn `def _parse_positive_int_option(option_name, option_raw)` `priv` (L2554-2577)
...
### fn `def _parse_post_gamma_auto_options(post_gamma_auto_raw_values)` `priv` (L2578-2653)
...
### fn `def _parse_tmo_passthrough_value(option_name, option_raw)` `priv` (L2654-2670)
...
### fn `def _parse_jpg_compression_option(compression_raw)` `priv` (L2671-2693)
...
### fn `def _parse_float_exclusive_range_option(option_name, option_raw, min_value, max_value)` `priv` (L2694-2718)
...
### fn `def _parse_non_negative_float_option(option_name, option_raw)` `priv` (L2719-2741)
...
### fn `def _parse_float_in_range_option(option_name, option_raw, min_value, max_value)` `priv` (L2742-2767)
...
### fn `def _parse_positive_int_pair_option(option_name, option_raw)` `priv` (L2768-2799)
...
### fn `def _parse_auto_brightness_options(auto_brightness_raw_values)` `priv` (L2800-2896)
...
### fn `def _parse_auto_levels_hr_method_option(auto_levels_method_raw)` `priv` (L2897-2928)
...
### fn `def _parse_auto_levels_options(auto_levels_raw_values)` `priv` (L2929-3001)
...
### fn `def _parse_auto_adjust_options(auto_adjust_raw_values)` `priv` (L3002-3151)
...
### fn `def _parse_hdrplus_proxy_mode_option(proxy_mode_raw)` `priv` (L3152-3170)
...
### fn `def _parse_hdrplus_options(hdrplus_raw_values)` `priv` (L3171-3247)
...
### fn `def _parse_auto_adjust_option(auto_adjust_raw)` `priv` (L3248-3271)
...
### fn `def _parse_white_balance_mode_option(white_balance_raw)` `priv` (L3272-3300)
...
### fn `def _parse_hdr_merge_option(hdr_merge_raw)` `priv` (L3301-3330)
...
### fn `def _resolve_default_postprocess(` `priv` (L3331-3334)

### fn `def _parse_gamma_option(option_value)` `priv` (L3406-3444)
...
### fn `def _decode_exif_text_value(exif_value)` `priv` (L3445-3464)
...
### fn `def _exiftool_color_space_fallback(input_dng)` `priv` (L3465-3516)
...
### fn `def _extract_exif_gamma_tags(input_dng)` `priv` (L3517-3585)
...
### fn `def _resolve_auto_merge_gamma(exif_gamma_tags, source_gamma_info)` `priv` (L3586-3632)
...
### fn `def _describe_resolved_merge_gamma(resolved_merge_gamma)` `priv` (L3633-3692)
...
### fn `def _format_gamma_number(value)` `priv` (L3644-3657)
- @brief Format one deterministic merge-gamma runtime diagnostic line.
...
### fn `def _describe_exif_gamma_tags(exif_gamma_tags)` `priv` (L3693-3735)
...
### fn `def _ensure_three_channel_float_array_no_clip(np_module, image_data)` `priv` (L3736-3767)
...
### fn `def _apply_merge_gamma_float(np_module, image_rgb_float, resolved_merge_gamma)` `priv` (L3768-3820)
...
### fn `def _load_image_dependencies()` `priv` (L4746-4783)
...
### fn `def _parse_exif_datetime_to_timestamp(datetime_raw)` `priv` (L4784-4814)
...
### fn `def _parse_exif_exposure_time_to_seconds(exposure_raw)` `priv` (L4815-4877)
...
### fn `def _extract_dng_exif_payload_and_timestamp(pil_image_module, input_dng)` `priv` (L4878-4972)
...
### fn `def _read_exif_value(exif_tag)` `priv` (L4922-4939)
- @brief Extract DNG EXIF payload bytes, preferred datetime timestamp, source orientation, and exposure time.
...
### fn `def _resolve_thumbnail_transpose_map(pil_image_module)` `priv` (L4973-5004)
...
### fn `def _apply_orientation_transform(pil_image_module, pil_image, source_orientation)` `priv` (L5005-5027)
...
### fn `def _build_oriented_thumbnail_jpeg_bytes(` `priv` (L5028-5029)

### fn `def _coerce_exif_int_like_value(raw_value)` `priv` (L5060-5102)
...
### fn `def _normalize_ifd_integer_like_values_for_piexif_dump(piexif_module, exif_dict)` `priv` (L5103-5236)
...
### fn `def _refresh_output_jpg_exif_thumbnail_after_save(` `priv` (L5237-5243)

### fn `def _set_output_file_timestamps(output_jpg, exif_timestamp)` `priv` (L5293-5307)
...
### fn `def _sync_output_file_timestamps_from_exif(output_jpg, exif_timestamp)` `priv` (L5308-5325)
...
### fn `def _build_exposure_multipliers(ev_value, ev_zero=0.0)` `priv` (L5326-5344)
...
### fn `def _build_bracket_images_from_linear_base_float(np_module, base_rgb_float, multipliers)` `priv` (L5345-5373)
...
### fn `def _validate_white_balance_triplet_shape(np_module, bracket_images_float)` `priv` (L5374-5401)
...
### fn `def _build_xphoto_analysis_proxy_rgb_float(np_module, analysis_image_rgb_float)` `priv` (L5402-5441)
...
### fn `def _extract_white_balance_channel_gains_from_xphoto(` `priv` (L5442-5446)

### fn `def _estimate_xphoto_white_balance_gains_rgb(` `priv` (L5495-5499)
...
### fn `def _estimate_color_constancy_white_balance_gains_rgb(` `priv` (L5547-5550)
...
### fn `def _estimate_ttl_white_balance_gains_rgb(np_module, analysis_image_rgb_float)` `priv` (L5578-5602)
...
### fn `def _apply_channel_gains_to_white_balance_triplet(` `priv` (L5603-5606)

### fn `def _apply_white_balance_to_bracket_triplet(` `priv` (L5632-5635)
...
### fn `def _extract_bracket_images_float(` `priv` (L5710-5714)

### fn `def _order_bracket_paths(bracket_paths)` `priv` (L5748-5773)
...
### fn `def _order_hdr_plus_reference_paths(bracket_paths)` `priv` (L5774-5789)
...
### fn `def _format_external_command_for_log(command)` `priv` (L5790-5805)
...
### fn `def _resolve_luminance_cli_merge_gamma_options(resolved_merge_gamma)` `priv` (L5806-5847)
- @brief Resolve luminance CLI gamma/response parameters from merge-gamma payload.
- @details Enforces deterministic linear ingress for luminance backend by fixing pre-gamma `-g=1`, saturation `-S=1`, and HDR response `--hdrResponseCurve=linear`, while mapping resolved merge-gamma transfer to post-gamma `-G` using the closest representable scalar gamma supported by `luminance-hdr-cli`: `linear->1`, `power->param_a`, `srgb->2.2` approximation, `rec709->1/param_b`.
...
### fn `def _run_luminance_hdr_cli(` `priv` (L5848-5856)

### fn `def _build_opencv_radiance_exposure_times(` `priv` (L5955-5958)
...
### fn `def _build_ev_times_from_ev_zero_and_delta(ev_zero, ev_delta)` `priv` (L5992-6011)
...
### fn `def _normalize_opencv_hdr_to_unit_range(np_module, hdr_rgb_float32)` `priv` (L6012-6037)
...
### fn `def _run_opencv_merge_mertens(cv2_module, np_module, exposures_float)` `priv` (L6038-6060)
- @brief Execute OpenCV Mertens exposure fusion path.
- @details Runs `cv2.createMergeMertens().process(...)` on RGB float brackets that already share one identical merge-gamma transfer curve, rescales the float result by `255` to match OpenCV exposure-fusion brightness semantics observed on `uint8` inputs, and then normalizes the result to the repository RGB float contract.
- @param cv2_module {ModuleType} Imported OpenCV module.
- @param np_module {ModuleType} Imported numpy module.
...
### fn `def _estimate_opencv_camera_response(` `priv` (L6061-6065)

### fn `def _run_opencv_merge_radiance(` `priv` (L6094-6101)
...
### fn `def _normalize_debevec_hdr_to_unit_range(np_module, hdr_rgb_float32, white_point_percentile)` `priv` (L6165-6184)
...
### fn `def _run_opencv_merge_backend(` `priv` (L6185-6192)

### fn `def _hdrplus_box_down2_float32(np_module, frames_float32)` `priv` (L6277-6305)
...
### fn `def _hdrplus_gauss_down4_float32(np_module, frames_float32)` `priv` (L6306-6352)
...
### fn `def _hdrplus_build_scalar_proxy_float32(np_module, frames_rgb_float32, hdrplus_options)` `priv` (L6353-6386)
...
### fn `def _hdrplus_compute_tile_start_positions(np_module, axis_length, tile_stride, pad_margin)` `priv` (L6387-6407)
...
### fn `def _hdrplus_trunc_divide_int32(np_module, values_int32, divisor)` `priv` (L6408-6426)
...
### fn `def _hdrplus_compute_alignment_bounds(search_radius)` `priv` (L6427-6451)
...
### fn `def _hdrplus_compute_alignment_margin(search_radius, divisor=1)` `priv` (L6452-6470)
...
### fn `def _hdrplus_extract_overlapping_tiles(` `priv` (L6471-6476)

### fn `def _hdrplus_extract_aligned_tiles(` `priv` (L6529-6535)

### fn `def _hdrplus_align_layer(` `priv` (L6608-6615)
...
### fn `def _hdrplus_align_layers(np_module, scalar_frames, hdrplus_options)` `priv` (L6705-6792)
...
### fn `def _hdrplus_resolve_temporal_runtime_options(hdrplus_options)` `priv` (L6793-6817)
...
### fn `def _hdrplus_compute_temporal_weights(` `priv` (L6818-6822)

### fn `def _hdrplus_merge_temporal_rgb(` `priv` (L6903-6909)
...
### fn `def _hdrplus_merge_spatial_rgb(np_module, temporal_tiles, width, height)` `priv` (L6958-7030)
...
### fn `def _run_hdr_plus_merge(` `priv` (L7031-7035)

### fn `def _convert_compression_to_quality(jpg_compression)` `priv` (L7123-7133)
...
### fn `def _collect_missing_external_executables(` `priv` (L7134-7136)

### fn `def _resolve_auto_adjust_dependencies()` `priv` (L7155-7184)
...
### fn `def _resolve_numpy_dependency()` `priv` (L7185-7204)
...
### fn `def _to_float32_image_array(np_module, image_data)` `priv` (L7205-7236)
...
### fn `def _normalize_float_rgb_image(np_module, image_data)` `priv` (L7237-7264)
...
### fn `def _write_rgb_float_tiff16(imageio_module, np_module, output_path, image_rgb_float)` `priv` (L7265-7291)
...
### fn `def _write_rgb_float_tiff32(imageio_module, np_module, output_path, image_rgb_float)` `priv` (L7292-7316)
...
### fn `def _write_debug_rgb_float_tiff(` `priv` (L7317-7322)

### fn `def _build_debug_artifact_context(output_jpg, input_dng, postprocess_options)` `priv` (L7352-7372)
...
### fn `def _format_debug_ev_suffix_value(ev_value)` `priv` (L7373-7390)
...
### fn `def _materialize_bracket_tiffs_from_float(` `priv` (L7391-7395)

### fn `def _to_uint8_image_array(np_module, image_data)` `priv` (L7425-7471)
...
### fn `def _to_uint16_image_array(np_module, image_data)` `priv` (L7472-7516)
...
### fn `def _apply_post_gamma_float(np_module, image_rgb_float, gamma_value)` `priv` (L7517-7542)
...
### fn `def _build_auto_post_gamma_lut_float(np_module, gamma_value, lut_size)` `priv` (L7543-7559)
...
### fn `def _ensure_three_channel_float_array_no_range_adjust(np_module, image_data)` `priv` (L7560-7586)
...
### fn `def _apply_auto_post_gamma_float(np_module, image_rgb_float, post_gamma_auto_options)` `priv` (L7587-7631)
...
### fn `def _apply_brightness_float(np_module, image_rgb_float, brightness_factor)` `priv` (L7632-7654)
...
### fn `def _apply_contrast_float(np_module, image_rgb_float, contrast_factor)` `priv` (L7655-7679)
...
### fn `def _apply_saturation_float(np_module, image_rgb_float, saturation_factor)` `priv` (L7680-7710)
...
### fn `def _apply_static_postprocess_float(` `priv` (L7711-7716)

### fn `def _to_linear_srgb(np_module, image_srgb)` `priv` (L7833-7850)
...
### fn `def _from_linear_srgb(np_module, image_linear)` `priv` (L7851-7868)
...
### fn `def _compute_bt709_luminance(np_module, linear_rgb)` `priv` (L7869-7886)
...
### fn `def _analyze_luminance_key(np_module, luminance, eps)` `priv` (L7887-7926)
...
### fn `def _choose_auto_key_value(key_analysis, auto_brightness_options)` `priv` (L7927-7972)
...
### fn `def _reinhard_global_tonemap_luminance(` `priv` (L7973-7978)

### fn `def _luminance_preserving_desaturate_to_fit(np_module, rgb_linear, luminance, eps)` `priv` (L8012-8039)
...
### fn `def _apply_mild_local_contrast_bgr_uint16(cv2_module, np_module, image_bgr_uint16, options)` `priv` (L8040-8078)
...
### fn `def _quantize_clahe_luminance_bins(np_module, luminance_values, histogram_size)` `priv` (L8079-8104)
...
### fn `def _build_clahe_float_tile_histogram(np_module, luminance_tile, histogram_size)` `priv` (L8105-8126)
...
### fn `def _clip_clahe_histogram(np_module, histogram, clip_limit, tile_population)` `priv` (L8127-8174)
...
### fn `def _build_clahe_float_lut(np_module, histogram, tile_population)` `priv` (L8175-8194)
...
### fn `def _pad_clahe_luminance_float(np_module, luminance_float, tile_grid_size)` `priv` (L8195-8233)
...
### fn `def _build_clahe_axis_interpolation(np_module, axis_length, tile_size, tile_count)` `priv` (L8234-8261)
...
### fn `def _build_clahe_tile_luts_float(np_module, luminance_float, clip_limit, tile_grid_size, histogram_size)` `priv` (L8262-8313)
...
### fn `def _interpolate_clahe_bilinear_float(np_module, luminance_float, tile_luts, tile_height, tile_width)` `priv` (L8314-8366)
...
### fn `def _apply_clahe_luminance_float(np_module, luminance_float, clip_limit, tile_grid_size)` `priv` (L8367-8398)
...
### fn `def _reconstruct_rgb_from_ycrcb_luma_float(cv2_module, np_module, luminance_float, cr_channel, cb_channel)` `priv` (L8399-8422)
...
### fn `def _apply_clahe_luma_rgb_float(cv2_module, np_module, image_rgb_float, auto_adjust_options)` `priv` (L8423-8472)
...
### fn `def _rt_gamma2(np_module, values)` `priv` (L8473-8492)
...
### fn `def _rt_igamma2(np_module, values)` `priv` (L8493-8513)
...
### fn `def _auto_levels_index_to_normalized_value(histogram_value, histcompr)` `priv` (L8514-8530)
...
### fn `def _auto_levels_normalized_to_legacy_code_value(value)` `priv` (L8531-8546)
...
### fn `def _auto_levels_normalized_to_legacy_code(np_module, values)` `priv` (L8547-8563)
...
### fn `def _auto_levels_legacy_code_to_normalized(np_module, values)` `priv` (L8564-8579)
...
### fn `def _pack_auto_levels_metrics(` `priv` (L8580-8595)

### fn `def _build_autoexp_histogram_rgb_float(np_module, image_rgb_float, histcompr)` `priv` (L8647-8682)
...
### fn `def _build_autoexp_histogram_rgb_uint16(np_module, image_rgb_uint16, histcompr)` `priv` (L8683-8715)
...
### fn `def _compute_auto_levels_from_histogram(np_module, histogram, histcompr, clip_percent)` `priv` (L8716-8915)
...
### fn `def _rt_simplebasecurve_scalar(x_value, black, shadow_recovery)` `priv` (L8989-9081)
...
### fn `def _basel(x_input, slope_start, slope_end)` `priv` (L9002-9018)
- @brief Evaluate RawTherapee `simplebasecurve` for one normalized sample.
...
### fn `def _baseu(x_input, slope_start, slope_end)` `priv` (L9019-9021)

### fn `def _cupper(x_input, slope_value, highlight_recovery)` `priv` (L9022-9041)

### fn `def _clower(x_input, slope_value, shadow_value)` `priv` (L9042-9044)

### fn `def _clower2(x_input, slope_value, shadow_value)` `priv` (L9045-9056)

### fn `def _build_rt_nurbs_curve_lut(np_module, x_points, y_points, sample_count)` `priv` (L9082-9215)
...
### fn `def _sample_auto_levels_lut_float(` `priv` (L9216-9222)

### fn `def _build_auto_levels_full_histogram_rgb_float(np_module, image_rgb_float)` `priv` (L9254-9285)
...
### fn `def _rt_hlcurve_float(np_module, exp_scale, comp, hlrange, levels_code)` `priv` (L9286-9316)
...
### fn `def _build_auto_levels_tone_curve_state(np_module, image_rgb_float, auto_levels_metrics)` `priv` (L9317-9506)
...
### fn `def _apply_auto_levels_tonal_transform_float(` `priv` (L9507-9510)

### fn `def _auto_levels_has_full_tone_metrics(auto_levels_metrics)` `priv` (L9594-9617)
...
### fn `def _call_auto_levels_compat_helper(` `priv` (L9618-9623)

### fn `def _apply_auto_levels_float(np_module, image_rgb_float, auto_levels_options)` `priv` (L9667-9770)
...
### fn `def _clip_auto_levels_out_of_gamut_float(np_module, image_rgb, maxval=1.0)` `priv` (L9771-9931)
...
### fn `def _filmlike_clip_rgb_tone(primary, middle, lower)` `priv` (L9801-9834)
- @brief Clip overflowing RGB triplets with RawTherapee film-like gamut logic.
...
### fn `def _clip_auto_levels_out_of_gamut_uint16(` `priv` (L9932-9933)

### fn `def _hlrecovery_luminance_float(np_module, image_rgb, maxval=1.0)` `priv` (L9963-10009)
...
### fn `def _hlrecovery_luminance_uint16(np_module, image_rgb, maxval=_AUTO_LEVELS_CODE_MAX)` `priv` (L10010-10038)
...
### fn `def _hlrecovery_cielab_float(` `priv` (L10039-10040)

### fn `def _f_lab(values)` `priv` (L10073-10080)
...
### fn `def _f2xyz(values)` `priv` (L10081-10087)

### fn `def _hlrecovery_cielab_uint16(` `priv` (L10123-10124)

### fn `def _hlrecovery_blend_float(np_module, image_rgb, hlmax, maxval=1.0)` `priv` (L10158-10263)
...
### fn `def _hlrecovery_blend_uint16(` `priv` (L10264-10265)

### fn `def _dilate_mask_float(np_module, mask)` `priv` (L10300-10322)
...
### fn `def _box_mean_3x3_float(np_module, image_2d)` `priv` (L10323-10346)
...
### fn `def _hlrecovery_color_propagation_float(np_module, image_rgb, maxval=1.0)` `priv` (L10347-10391)
...
### fn `def _hlrecovery_color_propagation_uint16(` `priv` (L10392-10393)

### fn `def _hlrecovery_inpaint_opposed_float(` `priv` (L10423-10424)
...
### fn `def _hlrecovery_inpaint_opposed_uint16(` `priv` (L10477-10478)
...
### fn `def _apply_auto_brightness_rgb_float(` `priv` (L10520-10523)
...
### fn `def _clamp01(np_module, values)` `priv` (L10581-10594)
...
### fn `def _gaussian_kernel_2d(np_module, sigma, radius=None)` `priv` (L10595-10617)
...
### fn `def _rgb_to_hsl(np_module, rgb)` `priv` (L10618-10651)
...
### fn `def _hue_to_rgb(np_module, p_values, q_values, t_values)` `priv` (L10652-10682)
...
### fn `def _hsl_to_rgb(np_module, hue, saturation, lightness)` `priv` (L10683-10723)
...
### fn `def _selective_blur_contrast_gated_vectorized(` `priv` (L10724-10725)

### fn `def _level_per_channel_adaptive(np_module, rgb, low_pct=0.1, high_pct=99.9)` `priv` (L10774-10796)
...
### fn `def _sigmoidal_contrast(np_module, rgb, contrast=3.0, midpoint=0.5)` `priv` (L10797-10821)
...
### fn `def logistic(z_values)` (L10812-10814)
- @brief Execute sigmoidal contrast stage.
...
### fn `def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=0.8)` `priv` (L10822-10839)
...
### fn `def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)` `priv` (L10840-10863)
...
### fn `def _high_pass_math_gray(cv2_module, np_module, rgb, blur_sigma=2.5)` `priv` (L10864-10887)
...
### fn `def _overlay_composite(np_module, base_rgb, overlay_gray)` `priv` (L10888-10909)
...
### fn `def _apply_validated_auto_adjust_pipeline(` `priv` (L10910-10916)

### fn `def _load_piexif_dependency()` `priv` (L11025-11042)
...
### fn `def _encode_jpg(` `priv` (L11043-11054)

### fn `def _collect_processing_errors(rawpy_module)` `priv` (L11188-11216)
...
### fn `def _is_supported_runtime_os()` `priv` (L11217-11236)
...
|`DEFAULT_REINHARD02_BRIGHTNESS`|var|pub|123||
|`DEFAULT_REINHARD02_CONTRAST`|var|pub|124||
|`DEFAULT_REINHARD02_SATURATION`|var|pub|125||
|`DEFAULT_MANTIUK08_CONTRAST`|var|pub|126||
|`DEFAULT_OPENCV_POST_GAMMA`|var|pub|127||
|`DEFAULT_OPENCV_BRIGHTNESS`|var|pub|128||
|`DEFAULT_OPENCV_CONTRAST`|var|pub|129||
|`DEFAULT_OPENCV_SATURATION`|var|pub|130||
|`DEFAULT_OPENCV_MERGE_ALGORITHM`|var|pub|131||
|`DEFAULT_OPENCV_TONEMAP_ENABLED`|var|pub|132||
|`DEFAULT_OPENCV_TONEMAP_GAMMA`|var|pub|133||
|`DEFAULT_HDRPLUS_PROXY_MODE`|var|pub|134||
|`DEFAULT_HDRPLUS_SEARCH_RADIUS`|var|pub|135||
|`DEFAULT_HDRPLUS_TEMPORAL_FACTOR`|var|pub|136||
|`DEFAULT_HDRPLUS_TEMPORAL_MIN_DIST`|var|pub|137||
|`DEFAULT_HDRPLUS_TEMPORAL_MAX_DIST`|var|pub|138||
|`HDRPLUS_TILE_SIZE`|var|pub|139||
|`HDRPLUS_TILE_STRIDE`|var|pub|140||
|`HDRPLUS_DOWNSAMPLED_TILE_SIZE`|var|pub|141||
|`HDRPLUS_ALIGNMENT_LEVELS`|var|pub|142||
|`HDRPLUS_ALIGNMENT_DOWNSAMPLE_RATE`|var|pub|143||
|`HDRPLUS_TEMPORAL_FACTOR`|var|pub|144||
|`HDRPLUS_TEMPORAL_MIN_DIST`|var|pub|145||
|`HDRPLUS_TEMPORAL_MAX_DIST`|var|pub|146||
|`MIN_SUPPORTED_BITS_PER_COLOR`|var|pub|148||
|`AutoAdjustOptions`|class|pub|377-412|class AutoAdjustOptions|
|`AutoBrightnessOptions`|class|pub|414-442|class AutoBrightnessOptions|
|`AutoLevelsOptions`|class|pub|444-467|class AutoLevelsOptions|
|`PostGammaAutoOptions`|class|pub|469-487|class PostGammaAutoOptions|
|`MergeGammaOption`|class|pub|489-507|class MergeGammaOption|
|`ResolvedMergeGamma`|class|pub|509-534|class ResolvedMergeGamma|
|`ExifGammaTags`|class|pub|536-557|class ExifGammaTags|
|`PostprocessOptions`|class|pub|559-606|class PostprocessOptions|
|`DebugArtifactContext`|class|pub|608-624|class DebugArtifactContext|
|`SourceGammaInfo`|class|pub|626-644|class SourceGammaInfo|
|`LuminanceOptions`|class|pub|646-667|class LuminanceOptions|
|`OpenCvMergeOptions`|class|pub|669-689|class OpenCvMergeOptions|
|`HdrPlusOptions`|class|pub|691-714|class HdrPlusOptions|
|`HdrPlusTemporalRuntimeOptions`|class|pub|716-735|class HdrPlusTemporalRuntimeOptions|
|`JointAutoEvSolution`|class|pub|737-757|class JointAutoEvSolution|
|`AutoEvIterationStep`|class|pub|759-776|class AutoEvIterationStep|
|`AutoEvOptions`|class|pub|778-795|class AutoEvOptions|
|`AutoZeroEvaluation`|class|pub|797-815|class AutoZeroEvaluation|
|`_print_box_table`|fn|priv|816-852|def _print_box_table(headers, rows, header_rows=())|
|`_border`|fn|priv|836-838|def _border(left, middle, right)|
|`_line`|fn|priv|839-842|def _line(values)|
|`_build_two_line_operator_rows`|fn|priv|853-869|def _build_two_line_operator_rows(operator_entries)|
|`_print_help_section`|fn|priv|870-884|def _print_help_section(title)|
|`_print_help_option`|fn|priv|885-926|def _print_help_option(option_label, description, detail_...|
|`print_help`|fn|pub|927-1126|def print_help(version)|
|`_calculate_max_ev_from_bits`|fn|priv|1336-1354|def _calculate_max_ev_from_bits(bits_per_color)|
|`_calculate_safe_ev_zero_max`|fn|priv|1355-1367|def _calculate_safe_ev_zero_max(base_max_ev)|
|`_derive_supported_ev_values`|fn|priv|1368-1370|def _derive_supported_ev_values(|
|`_detect_dng_bits_per_color`|fn|priv|1394-1439|def _detect_dng_bits_per_color(raw_handle)|
|`_is_ev_value_on_supported_step`|fn|priv|1440-1452|def _is_ev_value_on_supported_step(ev_value)|
|`_parse_ev_option`|fn|priv|1453-1478|def _parse_ev_option(ev_raw)|
|`_parse_ev_zero_option`|fn|priv|1479-1503|def _parse_ev_zero_option(ev_zero_raw)|
|`_parse_auto_ev_option`|fn|priv|1504-1523|def _parse_auto_ev_option(auto_ev_raw)|
|`_parse_percentage_option`|fn|priv|1524-1546|def _parse_percentage_option(option_name, option_raw)|
|`_parse_auto_brightness_option`|fn|priv|1547-1566|def _parse_auto_brightness_option(auto_brightness_raw)|
...
|`_normalize_source_gamma_label`|fn|priv|1769-1787|def _normalize_source_gamma_label(label_raw)|
|`_decode_raw_metadata_text`|fn|priv|1788-1819|def _decode_raw_metadata_text(metadata_raw)|
|`_classify_explicit_source_gamma`|fn|priv|1820-1869|def _classify_explicit_source_gamma(raw_handle)|
|`_classify_tone_curve_gamma`|fn|priv|1870-1918|def _classify_tone_curve_gamma(raw_handle)|
|`_has_nonzero_matrix`|fn|priv|1919-1939|def _has_nonzero_matrix(matrix_raw)|
|`_classify_matrix_hint_gamma`|fn|priv|1940-1970|def _classify_matrix_hint_gamma(raw_handle)|
|`_extract_source_gamma_info`|fn|priv|1971-1998|def _extract_source_gamma_info(raw_handle)|
|`_describe_source_gamma_info`|fn|priv|1999-2020|def _describe_source_gamma_info(source_gamma_info)|
|`_coerce_positive_luminance`|fn|priv|2021-2040|def _coerce_positive_luminance(value, fallback)|
|`_calculate_bt709_luminance`|fn|priv|2041-2063|def _calculate_bt709_luminance(np_module, image_rgb_float)|
|`_smoothstep`|fn|priv|2064-2082|def _smoothstep(np_module, values, edge0, edge1)|
|`_calculate_entropy_optimized_ev`|fn|priv|2083-2128|def _calculate_entropy_optimized_ev(_cv2_module, np_modul...|
|`_calculate_ettr_ev`|fn|priv|2129-2148|def _calculate_ettr_ev(np_module, luminance_float)|
|`_calculate_detail_preservation_ev`|fn|priv|2149-2216|def _calculate_detail_preservation_ev(_cv2_module, np_mod...|
|`_calculate_auto_zero_evaluations`|fn|priv|2217-2254|def _calculate_auto_zero_evaluations(cv2_module, np_modul...|
|`_select_ev_zero_candidate`|fn|priv|2255-2284|def _select_ev_zero_candidate(evaluations, safe_ev_zero_max)|
|`_build_unclipped_bracket_images_from_linear_base_float`|fn|priv|2285-2289|def _build_unclipped_bracket_images_from_linear_base_float(|
|`_measure_any_channel_highlight_clipping_pct`|fn|priv|2319-2336|def _measure_any_channel_highlight_clipping_pct(np_module...|
|`_measure_any_channel_shadow_clipping_pct`|fn|priv|2337-2354|def _measure_any_channel_shadow_clipping_pct(np_module, i...|
|`_resolve_joint_auto_ev_solution`|fn|priv|2355-2361|def _resolve_joint_auto_ev_solution(|
|`_parse_luminance_text_option`|fn|priv|2460-2480|def _parse_luminance_text_option(option_name, option_raw)|
|`_parse_luminance_response_curve_option`|fn|priv|2481-2507|def _parse_luminance_response_curve_option(option_raw)|
|`_parse_positive_float_option`|fn|priv|2508-2531|def _parse_positive_float_option(option_name, option_raw)|
|`_parse_post_gamma_selector_option`|fn|priv|2532-2553|def _parse_post_gamma_selector_option(option_raw)|
|`_parse_positive_int_option`|fn|priv|2554-2577|def _parse_positive_int_option(option_name, option_raw)|
|`_parse_post_gamma_auto_options`|fn|priv|2578-2653|def _parse_post_gamma_auto_options(post_gamma_auto_raw_va...|
|`_parse_tmo_passthrough_value`|fn|priv|2654-2670|def _parse_tmo_passthrough_value(option_name, option_raw)|
|`_parse_jpg_compression_option`|fn|priv|2671-2693|def _parse_jpg_compression_option(compression_raw)|
|`_parse_float_exclusive_range_option`|fn|priv|2694-2718|def _parse_float_exclusive_range_option(option_name, opti...|
|`_parse_non_negative_float_option`|fn|priv|2719-2741|def _parse_non_negative_float_option(option_name, option_...|
|`_parse_float_in_range_option`|fn|priv|2742-2767|def _parse_float_in_range_option(option_name, option_raw,...|
|`_parse_positive_int_pair_option`|fn|priv|2768-2799|def _parse_positive_int_pair_option(option_name, option_raw)|
|`_parse_auto_brightness_options`|fn|priv|2800-2896|def _parse_auto_brightness_options(auto_brightness_raw_va...|
|`_parse_auto_levels_hr_method_option`|fn|priv|2897-2928|def _parse_auto_levels_hr_method_option(auto_levels_metho...|
|`_parse_auto_levels_options`|fn|priv|2929-3001|def _parse_auto_levels_options(auto_levels_raw_values)|
|`_parse_auto_adjust_options`|fn|priv|3002-3151|def _parse_auto_adjust_options(auto_adjust_raw_values)|
|`_parse_hdrplus_proxy_mode_option`|fn|priv|3152-3170|def _parse_hdrplus_proxy_mode_option(proxy_mode_raw)|
|`_parse_hdrplus_options`|fn|priv|3171-3247|def _parse_hdrplus_options(hdrplus_raw_values)|
|`_parse_auto_adjust_option`|fn|priv|3248-3271|def _parse_auto_adjust_option(auto_adjust_raw)|
|`_parse_white_balance_mode_option`|fn|priv|3272-3300|def _parse_white_balance_mode_option(white_balance_raw)|
|`_parse_hdr_merge_option`|fn|priv|3301-3330|def _parse_hdr_merge_option(hdr_merge_raw)|
|`_resolve_default_postprocess`|fn|priv|3331-3334|def _resolve_default_postprocess(|
|`_parse_gamma_option`|fn|priv|3406-3444|def _parse_gamma_option(option_value)|
|`_decode_exif_text_value`|fn|priv|3445-3464|def _decode_exif_text_value(exif_value)|
|`_exiftool_color_space_fallback`|fn|priv|3465-3516|def _exiftool_color_space_fallback(input_dng)|
|`_extract_exif_gamma_tags`|fn|priv|3517-3585|def _extract_exif_gamma_tags(input_dng)|
|`_resolve_auto_merge_gamma`|fn|priv|3586-3632|def _resolve_auto_merge_gamma(exif_gamma_tags, source_gam...|
|`_describe_resolved_merge_gamma`|fn|priv|3633-3692|def _describe_resolved_merge_gamma(resolved_merge_gamma)|
|`_format_gamma_number`|fn|priv|3644-3657|def _format_gamma_number(value)|
|`_describe_exif_gamma_tags`|fn|priv|3693-3735|def _describe_exif_gamma_tags(exif_gamma_tags)|
|`_ensure_three_channel_float_array_no_clip`|fn|priv|3736-3767|def _ensure_three_channel_float_array_no_clip(np_module, ...|
|`_apply_merge_gamma_float`|fn|priv|3768-3820|def _apply_merge_gamma_float(np_module, image_rgb_float, ...|
|`_parse_run_options`|fn|priv|3821-4020|def _parse_run_options(args)|
|`_load_image_dependencies`|fn|priv|4746-4783|def _load_image_dependencies()|
|`_parse_exif_datetime_to_timestamp`|fn|priv|4784-4814|def _parse_exif_datetime_to_timestamp(datetime_raw)|
|`_parse_exif_exposure_time_to_seconds`|fn|priv|4815-4877|def _parse_exif_exposure_time_to_seconds(exposure_raw)|
|`_extract_dng_exif_payload_and_timestamp`|fn|priv|4878-4972|def _extract_dng_exif_payload_and_timestamp(pil_image_mod...|
|`_read_exif_value`|fn|priv|4922-4939|def _read_exif_value(exif_tag)|
|`_resolve_thumbnail_transpose_map`|fn|priv|4973-5004|def _resolve_thumbnail_transpose_map(pil_image_module)|
|`_apply_orientation_transform`|fn|priv|5005-5027|def _apply_orientation_transform(pil_image_module, pil_im...|
|`_build_oriented_thumbnail_jpeg_bytes`|fn|priv|5028-5029|def _build_oriented_thumbnail_jpeg_bytes(|
|`_coerce_exif_int_like_value`|fn|priv|5060-5102|def _coerce_exif_int_like_value(raw_value)|
|`_normalize_ifd_integer_like_values_for_piexif_dump`|fn|priv|5103-5236|def _normalize_ifd_integer_like_values_for_piexif_dump(pi...|
|`_refresh_output_jpg_exif_thumbnail_after_save`|fn|priv|5237-5243|def _refresh_output_jpg_exif_thumbnail_after_save(|
|`_set_output_file_timestamps`|fn|priv|5293-5307|def _set_output_file_timestamps(output_jpg, exif_timestamp)|
|`_sync_output_file_timestamps_from_exif`|fn|priv|5308-5325|def _sync_output_file_timestamps_from_exif(output_jpg, ex...|
|`_build_exposure_multipliers`|fn|priv|5326-5344|def _build_exposure_multipliers(ev_value, ev_zero=0.0)|
|`_build_bracket_images_from_linear_base_float`|fn|priv|5345-5373|def _build_bracket_images_from_linear_base_float(np_modul...|
|`_validate_white_balance_triplet_shape`|fn|priv|5374-5401|def _validate_white_balance_triplet_shape(np_module, brac...|
|`_build_xphoto_analysis_proxy_rgb_float`|fn|priv|5402-5441|def _build_xphoto_analysis_proxy_rgb_float(np_module, ana...|
|`_extract_white_balance_channel_gains_from_xphoto`|fn|priv|5442-5446|def _extract_white_balance_channel_gains_from_xphoto(|
|`_estimate_xphoto_white_balance_gains_rgb`|fn|priv|5495-5499|def _estimate_xphoto_white_balance_gains_rgb(|
|`_estimate_color_constancy_white_balance_gains_rgb`|fn|priv|5547-5550|def _estimate_color_constancy_white_balance_gains_rgb(|
|`_estimate_ttl_white_balance_gains_rgb`|fn|priv|5578-5602|def _estimate_ttl_white_balance_gains_rgb(np_module, anal...|
|`_apply_channel_gains_to_white_balance_triplet`|fn|priv|5603-5606|def _apply_channel_gains_to_white_balance_triplet(|
|`_apply_white_balance_to_bracket_triplet`|fn|priv|5632-5635|def _apply_white_balance_to_bracket_triplet(|
|`_extract_bracket_images_float`|fn|priv|5710-5714|def _extract_bracket_images_float(|
|`_order_bracket_paths`|fn|priv|5748-5773|def _order_bracket_paths(bracket_paths)|
|`_order_hdr_plus_reference_paths`|fn|priv|5774-5789|def _order_hdr_plus_reference_paths(bracket_paths)|
|`_format_external_command_for_log`|fn|priv|5790-5805|def _format_external_command_for_log(command)|
|`_resolve_luminance_cli_merge_gamma_options`|fn|priv|5806-5847|def _resolve_luminance_cli_merge_gamma_options(resolved_m...|
|`_run_luminance_hdr_cli`|fn|priv|5848-5856|def _run_luminance_hdr_cli(|
|`_build_opencv_radiance_exposure_times`|fn|priv|5955-5958|def _build_opencv_radiance_exposure_times(|
|`_build_ev_times_from_ev_zero_and_delta`|fn|priv|5992-6011|def _build_ev_times_from_ev_zero_and_delta(ev_zero, ev_de...|
|`_normalize_opencv_hdr_to_unit_range`|fn|priv|6012-6037|def _normalize_opencv_hdr_to_unit_range(np_module, hdr_rg...|
|`_run_opencv_merge_mertens`|fn|priv|6038-6060|def _run_opencv_merge_mertens(cv2_module, np_module, expo...|
|`_estimate_opencv_camera_response`|fn|priv|6061-6065|def _estimate_opencv_camera_response(|
|`_run_opencv_merge_radiance`|fn|priv|6094-6101|def _run_opencv_merge_radiance(|
|`_normalize_debevec_hdr_to_unit_range`|fn|priv|6165-6184|def _normalize_debevec_hdr_to_unit_range(np_module, hdr_r...|
|`_run_opencv_merge_backend`|fn|priv|6185-6192|def _run_opencv_merge_backend(|
|`_hdrplus_box_down2_float32`|fn|priv|6277-6305|def _hdrplus_box_down2_float32(np_module, frames_float32)|
|`_hdrplus_gauss_down4_float32`|fn|priv|6306-6352|def _hdrplus_gauss_down4_float32(np_module, frames_float32)|
|`_hdrplus_build_scalar_proxy_float32`|fn|priv|6353-6386|def _hdrplus_build_scalar_proxy_float32(np_module, frames...|
|`_hdrplus_compute_tile_start_positions`|fn|priv|6387-6407|def _hdrplus_compute_tile_start_positions(np_module, axis...|
|`_hdrplus_trunc_divide_int32`|fn|priv|6408-6426|def _hdrplus_trunc_divide_int32(np_module, values_int32, ...|
|`_hdrplus_compute_alignment_bounds`|fn|priv|6427-6451|def _hdrplus_compute_alignment_bounds(search_radius)|
|`_hdrplus_compute_alignment_margin`|fn|priv|6452-6470|def _hdrplus_compute_alignment_margin(search_radius, divi...|
|`_hdrplus_extract_overlapping_tiles`|fn|priv|6471-6476|def _hdrplus_extract_overlapping_tiles(|
|`_hdrplus_extract_aligned_tiles`|fn|priv|6529-6535|def _hdrplus_extract_aligned_tiles(|
|`_hdrplus_align_layer`|fn|priv|6608-6615|def _hdrplus_align_layer(|
|`_hdrplus_align_layers`|fn|priv|6705-6792|def _hdrplus_align_layers(np_module, scalar_frames, hdrpl...|
|`_hdrplus_resolve_temporal_runtime_options`|fn|priv|6793-6817|def _hdrplus_resolve_temporal_runtime_options(hdrplus_opt...|
|`_hdrplus_compute_temporal_weights`|fn|priv|6818-6822|def _hdrplus_compute_temporal_weights(|
|`_hdrplus_merge_temporal_rgb`|fn|priv|6903-6909|def _hdrplus_merge_temporal_rgb(|
|`_hdrplus_merge_spatial_rgb`|fn|priv|6958-7030|def _hdrplus_merge_spatial_rgb(np_module, temporal_tiles,...|
|`_run_hdr_plus_merge`|fn|priv|7031-7035|def _run_hdr_plus_merge(|
|`_convert_compression_to_quality`|fn|priv|7123-7133|def _convert_compression_to_quality(jpg_compression)|
|`_collect_missing_external_executables`|fn|priv|7134-7136|def _collect_missing_external_executables(|
|`_resolve_auto_adjust_dependencies`|fn|priv|7155-7184|def _resolve_auto_adjust_dependencies()|
|`_resolve_numpy_dependency`|fn|priv|7185-7204|def _resolve_numpy_dependency()|
|`_to_float32_image_array`|fn|priv|7205-7236|def _to_float32_image_array(np_module, image_data)|
|`_normalize_float_rgb_image`|fn|priv|7237-7264|def _normalize_float_rgb_image(np_module, image_data)|
|`_write_rgb_float_tiff16`|fn|priv|7265-7291|def _write_rgb_float_tiff16(imageio_module, np_module, ou...|
|`_write_rgb_float_tiff32`|fn|priv|7292-7316|def _write_rgb_float_tiff32(imageio_module, np_module, ou...|
|`_write_debug_rgb_float_tiff`|fn|priv|7317-7322|def _write_debug_rgb_float_tiff(|
|`_build_debug_artifact_context`|fn|priv|7352-7372|def _build_debug_artifact_context(output_jpg, input_dng, ...|
|`_format_debug_ev_suffix_value`|fn|priv|7373-7390|def _format_debug_ev_suffix_value(ev_value)|
|`_materialize_bracket_tiffs_from_float`|fn|priv|7391-7395|def _materialize_bracket_tiffs_from_float(|
|`_to_uint8_image_array`|fn|priv|7425-7471|def _to_uint8_image_array(np_module, image_data)|
|`_to_uint16_image_array`|fn|priv|7472-7516|def _to_uint16_image_array(np_module, image_data)|
|`_apply_post_gamma_float`|fn|priv|7517-7542|def _apply_post_gamma_float(np_module, image_rgb_float, g...|
|`_build_auto_post_gamma_lut_float`|fn|priv|7543-7559|def _build_auto_post_gamma_lut_float(np_module, gamma_val...|
|`_ensure_three_channel_float_array_no_range_adjust`|fn|priv|7560-7586|def _ensure_three_channel_float_array_no_range_adjust(np_...|
|`_apply_auto_post_gamma_float`|fn|priv|7587-7631|def _apply_auto_post_gamma_float(np_module, image_rgb_flo...|
|`_apply_brightness_float`|fn|priv|7632-7654|def _apply_brightness_float(np_module, image_rgb_float, b...|
|`_apply_contrast_float`|fn|priv|7655-7679|def _apply_contrast_float(np_module, image_rgb_float, con...|
|`_apply_saturation_float`|fn|priv|7680-7710|def _apply_saturation_float(np_module, image_rgb_float, s...|
|`_apply_static_postprocess_float`|fn|priv|7711-7716|def _apply_static_postprocess_float(|
|`_to_linear_srgb`|fn|priv|7833-7850|def _to_linear_srgb(np_module, image_srgb)|
|`_from_linear_srgb`|fn|priv|7851-7868|def _from_linear_srgb(np_module, image_linear)|
|`_compute_bt709_luminance`|fn|priv|7869-7886|def _compute_bt709_luminance(np_module, linear_rgb)|
|`_analyze_luminance_key`|fn|priv|7887-7926|def _analyze_luminance_key(np_module, luminance, eps)|
|`_choose_auto_key_value`|fn|priv|7927-7972|def _choose_auto_key_value(key_analysis, auto_brightness_...|
|`_reinhard_global_tonemap_luminance`|fn|priv|7973-7978|def _reinhard_global_tonemap_luminance(|
|`_luminance_preserving_desaturate_to_fit`|fn|priv|8012-8039|def _luminance_preserving_desaturate_to_fit(np_module, rg...|
|`_apply_mild_local_contrast_bgr_uint16`|fn|priv|8040-8078|def _apply_mild_local_contrast_bgr_uint16(cv2_module, np_...|
|`_quantize_clahe_luminance_bins`|fn|priv|8079-8104|def _quantize_clahe_luminance_bins(np_module, luminance_v...|
|`_build_clahe_float_tile_histogram`|fn|priv|8105-8126|def _build_clahe_float_tile_histogram(np_module, luminanc...|
|`_clip_clahe_histogram`|fn|priv|8127-8174|def _clip_clahe_histogram(np_module, histogram, clip_limi...|
|`_build_clahe_float_lut`|fn|priv|8175-8194|def _build_clahe_float_lut(np_module, histogram, tile_pop...|
|`_pad_clahe_luminance_float`|fn|priv|8195-8233|def _pad_clahe_luminance_float(np_module, luminance_float...|
|`_build_clahe_axis_interpolation`|fn|priv|8234-8261|def _build_clahe_axis_interpolation(np_module, axis_lengt...|
|`_build_clahe_tile_luts_float`|fn|priv|8262-8313|def _build_clahe_tile_luts_float(np_module, luminance_flo...|
|`_interpolate_clahe_bilinear_float`|fn|priv|8314-8366|def _interpolate_clahe_bilinear_float(np_module, luminanc...|
|`_apply_clahe_luminance_float`|fn|priv|8367-8398|def _apply_clahe_luminance_float(np_module, luminance_flo...|
|`_reconstruct_rgb_from_ycrcb_luma_float`|fn|priv|8399-8422|def _reconstruct_rgb_from_ycrcb_luma_float(cv2_module, np...|
|`_apply_clahe_luma_rgb_float`|fn|priv|8423-8472|def _apply_clahe_luma_rgb_float(cv2_module, np_module, im...|
|`_rt_gamma2`|fn|priv|8473-8492|def _rt_gamma2(np_module, values)|
|`_rt_igamma2`|fn|priv|8493-8513|def _rt_igamma2(np_module, values)|
|`_auto_levels_index_to_normalized_value`|fn|priv|8514-8530|def _auto_levels_index_to_normalized_value(histogram_valu...|
|`_auto_levels_normalized_to_legacy_code_value`|fn|priv|8531-8546|def _auto_levels_normalized_to_legacy_code_value(value)|
|`_auto_levels_normalized_to_legacy_code`|fn|priv|8547-8563|def _auto_levels_normalized_to_legacy_code(np_module, val...|
|`_auto_levels_legacy_code_to_normalized`|fn|priv|8564-8579|def _auto_levels_legacy_code_to_normalized(np_module, val...|
|`_pack_auto_levels_metrics`|fn|priv|8580-8595|def _pack_auto_levels_metrics(|
|`_build_autoexp_histogram_rgb_float`|fn|priv|8647-8682|def _build_autoexp_histogram_rgb_float(np_module, image_r...|
|`_build_autoexp_histogram_rgb_uint16`|fn|priv|8683-8715|def _build_autoexp_histogram_rgb_uint16(np_module, image_...|
|`_compute_auto_levels_from_histogram`|fn|priv|8716-8915|def _compute_auto_levels_from_histogram(np_module, histog...|
|`_rt_simplebasecurve_scalar`|fn|priv|8989-9081|def _rt_simplebasecurve_scalar(x_value, black, shadow_rec...|
|`_basel`|fn|priv|9002-9018|def _basel(x_input, slope_start, slope_end)|
|`_baseu`|fn|priv|9019-9021|def _baseu(x_input, slope_start, slope_end)|
|`_cupper`|fn|priv|9022-9041|def _cupper(x_input, slope_value, highlight_recovery)|
|`_clower`|fn|priv|9042-9044|def _clower(x_input, slope_value, shadow_value)|
|`_clower2`|fn|priv|9045-9056|def _clower2(x_input, slope_value, shadow_value)|
|`_build_rt_nurbs_curve_lut`|fn|priv|9082-9215|def _build_rt_nurbs_curve_lut(np_module, x_points, y_poin...|
|`_sample_auto_levels_lut_float`|fn|priv|9216-9222|def _sample_auto_levels_lut_float(|
|`_build_auto_levels_full_histogram_rgb_float`|fn|priv|9254-9285|def _build_auto_levels_full_histogram_rgb_float(np_module...|
|`_rt_hlcurve_float`|fn|priv|9286-9316|def _rt_hlcurve_float(np_module, exp_scale, comp, hlrange...|
|`_build_auto_levels_tone_curve_state`|fn|priv|9317-9506|def _build_auto_levels_tone_curve_state(np_module, image_...|
|`_apply_auto_levels_tonal_transform_float`|fn|priv|9507-9510|def _apply_auto_levels_tonal_transform_float(|
|`_auto_levels_has_full_tone_metrics`|fn|priv|9594-9617|def _auto_levels_has_full_tone_metrics(auto_levels_metrics)|
|`_call_auto_levels_compat_helper`|fn|priv|9618-9623|def _call_auto_levels_compat_helper(|
|`_apply_auto_levels_float`|fn|priv|9667-9770|def _apply_auto_levels_float(np_module, image_rgb_float, ...|
|`_clip_auto_levels_out_of_gamut_float`|fn|priv|9771-9931|def _clip_auto_levels_out_of_gamut_float(np_module, image...|
|`_filmlike_clip_rgb_tone`|fn|priv|9801-9834|def _filmlike_clip_rgb_tone(primary, middle, lower)|
|`_clip_auto_levels_out_of_gamut_uint16`|fn|priv|9932-9933|def _clip_auto_levels_out_of_gamut_uint16(|
|`_hlrecovery_luminance_float`|fn|priv|9963-10009|def _hlrecovery_luminance_float(np_module, image_rgb, max...|
|`_hlrecovery_luminance_uint16`|fn|priv|10010-10038|def _hlrecovery_luminance_uint16(np_module, image_rgb, ma...|
|`_hlrecovery_cielab_float`|fn|priv|10039-10040|def _hlrecovery_cielab_float(|
|`_f_lab`|fn|priv|10073-10080|def _f_lab(values)|
|`_f2xyz`|fn|priv|10081-10087|def _f2xyz(values)|
|`_hlrecovery_cielab_uint16`|fn|priv|10123-10124|def _hlrecovery_cielab_uint16(|
|`_hlrecovery_blend_float`|fn|priv|10158-10263|def _hlrecovery_blend_float(np_module, image_rgb, hlmax, ...|
|`_hlrecovery_blend_uint16`|fn|priv|10264-10265|def _hlrecovery_blend_uint16(|
|`_dilate_mask_float`|fn|priv|10300-10322|def _dilate_mask_float(np_module, mask)|
|`_box_mean_3x3_float`|fn|priv|10323-10346|def _box_mean_3x3_float(np_module, image_2d)|
|`_hlrecovery_color_propagation_float`|fn|priv|10347-10391|def _hlrecovery_color_propagation_float(np_module, image_...|
|`_hlrecovery_color_propagation_uint16`|fn|priv|10392-10393|def _hlrecovery_color_propagation_uint16(|
|`_hlrecovery_inpaint_opposed_float`|fn|priv|10423-10424|def _hlrecovery_inpaint_opposed_float(|
|`_hlrecovery_inpaint_opposed_uint16`|fn|priv|10477-10478|def _hlrecovery_inpaint_opposed_uint16(|
|`_apply_auto_brightness_rgb_float`|fn|priv|10520-10523|def _apply_auto_brightness_rgb_float(|
|`_clamp01`|fn|priv|10581-10594|def _clamp01(np_module, values)|
|`_gaussian_kernel_2d`|fn|priv|10595-10617|def _gaussian_kernel_2d(np_module, sigma, radius=None)|
|`_rgb_to_hsl`|fn|priv|10618-10651|def _rgb_to_hsl(np_module, rgb)|
|`_hue_to_rgb`|fn|priv|10652-10682|def _hue_to_rgb(np_module, p_values, q_values, t_values)|
|`_hsl_to_rgb`|fn|priv|10683-10723|def _hsl_to_rgb(np_module, hue, saturation, lightness)|
|`_selective_blur_contrast_gated_vectorized`|fn|priv|10724-10725|def _selective_blur_contrast_gated_vectorized(|
|`_level_per_channel_adaptive`|fn|priv|10774-10796|def _level_per_channel_adaptive(np_module, rgb, low_pct=0...|
|`_sigmoidal_contrast`|fn|priv|10797-10821|def _sigmoidal_contrast(np_module, rgb, contrast=3.0, mid...|
|`logistic`|fn|pub|10812-10814|def logistic(z_values)|
|`_vibrance_hsl_gamma`|fn|priv|10822-10839|def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=...|
|`_gaussian_blur_rgb`|fn|priv|10840-10863|def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)|
|`_high_pass_math_gray`|fn|priv|10864-10887|def _high_pass_math_gray(cv2_module, np_module, rgb, blur...|
|`_overlay_composite`|fn|priv|10888-10909|def _overlay_composite(np_module, base_rgb, overlay_gray)|
|`_apply_validated_auto_adjust_pipeline`|fn|priv|10910-10916|def _apply_validated_auto_adjust_pipeline(|
|`_load_piexif_dependency`|fn|priv|11025-11042|def _load_piexif_dependency()|
|`_encode_jpg`|fn|priv|11043-11054|def _encode_jpg(|
|`_collect_processing_errors`|fn|priv|11188-11216|def _collect_processing_errors(rawpy_module)|
|`_is_supported_runtime_os`|fn|priv|11217-11236|def _is_supported_runtime_os()|
|`run`|fn|pub|11237-11436|def run(args)|
