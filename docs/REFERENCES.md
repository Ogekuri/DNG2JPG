...
# dng2jpg.py | Python | 10751L | 305 symbols | 26 imports | 224 comments
...
### class `class MergeGammaOption` `@dataclass(frozen=True)` (L446-464)
...
### class `class ResolvedMergeGamma` `@dataclass(frozen=True)` (L466-491)
...
### class `class ExifGammaTags` `@dataclass(frozen=True)` (L493-511)
- @brief Hold EXIF tags relevant to auto merge-gamma resolution.
- @details Encapsulates normalized EXIF color-space, interoperability, and image-model tokens extracted from the source RAW/DNG container via `exifread` binary stream processing. The payload is consumed only by merge-gamma resolution and diagnostics and never mutates bracket extraction.
...
### fn `def _extract_exif_gamma_tags(input_dng)` `priv` (L3255-3307)
- @brief Extract EXIF color-space metadata relevant to auto merge gamma.
- @details Opens the source RAW/DNG file as a binary stream via `exifread.process_file` and normalizes `EXIF ColorSpace`, `Interop InteroperabilityIndex`, and `Image Model` tags for deterministic auto transfer resolution. Does not use Pillow for this extraction. Complexity: O(file_size). Side effects: none (read-only file access).
...
### fn `def _resolve_auto_merge_gamma(exif_gamma_tags, source_gamma_info)` `priv` (L3308-3372)
...
### fn `def _describe_resolved_merge_gamma(resolved_merge_gamma)` `priv` (L3373-3432)
...
### fn `def _format_gamma_number(value)` `priv` (L3384-3397)
- @brief Format one deterministic merge-gamma runtime diagnostic line.
...
### fn `def _describe_exif_gamma_tags(exif_gamma_tags)` `priv` (L3433-3454)
- @brief Format one deterministic EXIF merge-gamma input diagnostic line.
- @details Renders one stable runtime payload exposing the normalized EXIF `ColorSpace`, `InteroperabilityIndex`, and `ImageModel` values consumed by automatic merge-gamma resolution. Missing values are rendered as `missing`.
- @param exif_gamma_tags {ExifGammaTags} Normalized EXIF merge-gamma evidence payload.
- @return {str} Deterministic runtime diagnostic line.
- @satisfies REQ-172

### fn `def _ensure_three_channel_float_array_no_clip(np_module, image_data)` `priv` (L3455-3486)
...
### fn `def _apply_merge_gamma_float(np_module, image_rgb_float, resolved_merge_gamma)` `priv` (L3487-3539)
...
### fn `def _parse_run_options(args)` `priv` (L3540-3739)
...
### fn `def _load_image_dependencies()` `priv` (L4401-4438)
...
### fn `def _parse_exif_datetime_to_timestamp(datetime_raw)` `priv` (L4439-4469)
...
### fn `def _parse_exif_exposure_time_to_seconds(exposure_raw)` `priv` (L4470-4532)
...
### fn `def _extract_dng_exif_payload_and_timestamp(pil_image_module, input_dng)` `priv` (L4533-4627)
...
### fn `def _read_exif_value(exif_tag)` `priv` (L4577-4594)
- @brief Extract DNG EXIF payload bytes, preferred datetime timestamp, source orientation, and exposure time.
...
### fn `def _resolve_thumbnail_transpose_map(pil_image_module)` `priv` (L4628-4659)
...
### fn `def _apply_orientation_transform(pil_image_module, pil_image, source_orientation)` `priv` (L4660-4682)
...
### fn `def _build_oriented_thumbnail_jpeg_bytes(` `priv` (L4683-4684)

### fn `def _coerce_exif_int_like_value(raw_value)` `priv` (L4715-4757)
...
### fn `def _normalize_ifd_integer_like_values_for_piexif_dump(piexif_module, exif_dict)` `priv` (L4758-4891)
...
### fn `def _refresh_output_jpg_exif_thumbnail_after_save(` `priv` (L4892-4898)

### fn `def _set_output_file_timestamps(output_jpg, exif_timestamp)` `priv` (L4948-4962)
...
### fn `def _sync_output_file_timestamps_from_exif(output_jpg, exif_timestamp)` `priv` (L4963-4980)
...
### fn `def _build_exposure_multipliers(ev_value, ev_zero=0.0)` `priv` (L4981-4999)
...
### fn `def _build_bracket_images_from_linear_base_float(np_module, base_rgb_float, multipliers)` `priv` (L5000-5028)
...
### fn `def _extract_bracket_images_float(` `priv` (L5029-5033)

### fn `def _order_bracket_paths(bracket_paths)` `priv` (L5067-5092)
...
### fn `def _order_hdr_plus_reference_paths(bracket_paths)` `priv` (L5093-5108)
...
### fn `def _format_external_command_for_log(command)` `priv` (L5109-5124)
...
### fn `def _run_luminance_hdr_cli(` `priv` (L5125-5132)

### fn `def _build_opencv_radiance_exposure_times(` `priv` (L5207-5210)
...
### fn `def _build_ev_times_from_ev_zero_and_delta(ev_zero, ev_delta)` `priv` (L5244-5263)
...
### fn `def _normalize_opencv_hdr_to_unit_range(np_module, hdr_rgb_float32)` `priv` (L5264-5287)
...
### fn `def _run_opencv_merge_mertens(cv2_module, np_module, exposures_float)` `priv` (L5288-5309)
...
### fn `def _estimate_opencv_camera_response(` `priv` (L5310-5314)

### fn `def _run_opencv_merge_radiance(` `priv` (L5343-5350)
...
### fn `def _normalize_debevec_hdr_to_unit_range(np_module, hdr_rgb_float32, white_point_percentile)` `priv` (L5414-5433)
...
### fn `def _run_opencv_hdr_merge(` `priv` (L5434-5441)

### fn `def _hdrplus_box_down2_float32(np_module, frames_float32)` `priv` (L5523-5551)
...
### fn `def _hdrplus_gauss_down4_float32(np_module, frames_float32)` `priv` (L5552-5598)
...
### fn `def _hdrplus_build_scalar_proxy_float32(np_module, frames_rgb_float32, hdrplus_options)` `priv` (L5599-5632)
...
### fn `def _hdrplus_compute_tile_start_positions(np_module, axis_length, tile_stride, pad_margin)` `priv` (L5633-5653)
...
### fn `def _hdrplus_trunc_divide_int32(np_module, values_int32, divisor)` `priv` (L5654-5672)
...
### fn `def _hdrplus_compute_alignment_bounds(search_radius)` `priv` (L5673-5697)
...
### fn `def _hdrplus_compute_alignment_margin(search_radius, divisor=1)` `priv` (L5698-5716)
...
### fn `def _hdrplus_extract_overlapping_tiles(` `priv` (L5717-5722)

### fn `def _hdrplus_extract_aligned_tiles(` `priv` (L5775-5781)

### fn `def _hdrplus_align_layer(` `priv` (L5854-5861)
...
### fn `def _hdrplus_align_layers(np_module, scalar_frames, hdrplus_options)` `priv` (L5951-6038)
...
### fn `def _hdrplus_resolve_temporal_runtime_options(hdrplus_options)` `priv` (L6039-6063)
...
### fn `def _hdrplus_compute_temporal_weights(` `priv` (L6064-6068)

### fn `def _hdrplus_merge_temporal_rgb(` `priv` (L6149-6155)
...
### fn `def _hdrplus_merge_spatial_rgb(np_module, temporal_tiles, width, height)` `priv` (L6204-6276)
...
### fn `def _run_hdr_plus_merge(` `priv` (L6277-6281)

### fn `def _convert_compression_to_quality(jpg_compression)` `priv` (L6369-6379)
...
### fn `def _collect_missing_external_executables(` `priv` (L6380-6382)

### fn `def _resolve_auto_adjust_dependencies()` `priv` (L6401-6426)
...
### fn `def _resolve_numpy_dependency()` `priv` (L6427-6446)
...
### fn `def _to_float32_image_array(np_module, image_data)` `priv` (L6447-6478)
...
### fn `def _normalize_float_rgb_image(np_module, image_data)` `priv` (L6479-6506)
...
### fn `def _write_rgb_float_tiff16(imageio_module, np_module, output_path, image_rgb_float)` `priv` (L6507-6530)
...
### fn `def _write_debug_rgb_float_tiff(` `priv` (L6531-6536)

### fn `def _build_debug_artifact_context(output_jpg, input_dng, postprocess_options)` `priv` (L6566-6586)
...
### fn `def _format_debug_ev_suffix_value(ev_value)` `priv` (L6587-6604)
...
### fn `def _materialize_bracket_tiffs_from_float(` `priv` (L6605-6609)

### fn `def _to_uint8_image_array(np_module, image_data)` `priv` (L6639-6685)
...
### fn `def _to_uint16_image_array(np_module, image_data)` `priv` (L6686-6730)
...
### fn `def _apply_post_gamma_float(np_module, image_rgb_float, gamma_value)` `priv` (L6731-6756)
...
### fn `def _apply_brightness_float(np_module, image_rgb_float, brightness_factor)` `priv` (L6757-6779)
...
### fn `def _apply_contrast_float(np_module, image_rgb_float, contrast_factor)` `priv` (L6780-6804)
...
### fn `def _apply_saturation_float(np_module, image_rgb_float, saturation_factor)` `priv` (L6805-6834)
...
### fn `def _apply_static_postprocess_float(` `priv` (L6835-6840)

### fn `def _to_linear_srgb(np_module, image_srgb)` `priv` (L6917-6934)
...
### fn `def _from_linear_srgb(np_module, image_linear)` `priv` (L6935-6952)
...
### fn `def _compute_bt709_luminance(np_module, linear_rgb)` `priv` (L6953-6970)
...
### fn `def _analyze_luminance_key(np_module, luminance, eps)` `priv` (L6971-7010)
...
### fn `def _choose_auto_key_value(key_analysis, auto_brightness_options)` `priv` (L7011-7056)
...
### fn `def _reinhard_global_tonemap_luminance(` `priv` (L7057-7062)

### fn `def _luminance_preserving_desaturate_to_fit(np_module, rgb_linear, luminance, eps)` `priv` (L7096-7123)
...
### fn `def _apply_mild_local_contrast_bgr_uint16(cv2_module, np_module, image_bgr_uint16, options)` `priv` (L7124-7162)
...
### fn `def _quantize_clahe_luminance_bins(np_module, luminance_values, histogram_size)` `priv` (L7163-7188)
...
### fn `def _build_clahe_float_tile_histogram(np_module, luminance_tile, histogram_size)` `priv` (L7189-7210)
...
### fn `def _clip_clahe_histogram(np_module, histogram, clip_limit, tile_population)` `priv` (L7211-7258)
...
### fn `def _build_clahe_float_lut(np_module, histogram, tile_population)` `priv` (L7259-7278)
...
### fn `def _pad_clahe_luminance_float(np_module, luminance_float, tile_grid_size)` `priv` (L7279-7317)
...
### fn `def _build_clahe_axis_interpolation(np_module, axis_length, tile_size, tile_count)` `priv` (L7318-7345)
...
### fn `def _build_clahe_tile_luts_float(np_module, luminance_float, clip_limit, tile_grid_size, histogram_size)` `priv` (L7346-7397)
...
### fn `def _interpolate_clahe_bilinear_float(np_module, luminance_float, tile_luts, tile_height, tile_width)` `priv` (L7398-7450)
...
### fn `def _apply_clahe_luminance_float(np_module, luminance_float, clip_limit, tile_grid_size)` `priv` (L7451-7482)
...
### fn `def _reconstruct_rgb_from_ycrcb_luma_float(cv2_module, np_module, luminance_float, cr_channel, cb_channel)` `priv` (L7483-7506)
...
### fn `def _apply_clahe_luma_rgb_float(cv2_module, np_module, image_rgb_float, auto_adjust_options)` `priv` (L7507-7556)
...
### fn `def _rt_gamma2(np_module, values)` `priv` (L7557-7576)
...
### fn `def _rt_igamma2(np_module, values)` `priv` (L7577-7597)
...
### fn `def _auto_levels_index_to_normalized_value(histogram_value, histcompr)` `priv` (L7598-7614)
...
### fn `def _auto_levels_normalized_to_legacy_code_value(value)` `priv` (L7615-7630)
...
### fn `def _auto_levels_normalized_to_legacy_code(np_module, values)` `priv` (L7631-7647)
...
### fn `def _auto_levels_legacy_code_to_normalized(np_module, values)` `priv` (L7648-7663)
...
### fn `def _pack_auto_levels_metrics(` `priv` (L7664-7679)

### fn `def _build_autoexp_histogram_rgb_float(np_module, image_rgb_float, histcompr)` `priv` (L7731-7766)
...
### fn `def _build_autoexp_histogram_rgb_uint16(np_module, image_rgb_uint16, histcompr)` `priv` (L7767-7799)
...
### fn `def _compute_auto_levels_from_histogram(np_module, histogram, histcompr, clip_percent)` `priv` (L7800-7999)
...
### fn `def _rt_simplebasecurve_scalar(x_value, black, shadow_recovery)` `priv` (L8073-8165)
...
### fn `def _basel(x_input, slope_start, slope_end)` `priv` (L8086-8102)
- @brief Evaluate RawTherapee `simplebasecurve` for one normalized sample.
...
### fn `def _baseu(x_input, slope_start, slope_end)` `priv` (L8103-8105)

### fn `def _cupper(x_input, slope_value, highlight_recovery)` `priv` (L8106-8125)

### fn `def _clower(x_input, slope_value, shadow_value)` `priv` (L8126-8128)

### fn `def _clower2(x_input, slope_value, shadow_value)` `priv` (L8129-8140)

### fn `def _build_rt_nurbs_curve_lut(np_module, x_points, y_points, sample_count)` `priv` (L8166-8299)
...
### fn `def _sample_auto_levels_lut_float(` `priv` (L8300-8306)

### fn `def _build_auto_levels_full_histogram_rgb_float(np_module, image_rgb_float)` `priv` (L8338-8369)
...
### fn `def _rt_hlcurve_float(np_module, exp_scale, comp, hlrange, levels_code)` `priv` (L8370-8400)
...
### fn `def _build_auto_levels_tone_curve_state(np_module, image_rgb_float, auto_levels_metrics)` `priv` (L8401-8590)
...
### fn `def _apply_auto_levels_tonal_transform_float(` `priv` (L8591-8594)

### fn `def _auto_levels_has_full_tone_metrics(auto_levels_metrics)` `priv` (L8678-8701)
...
### fn `def _call_auto_levels_compat_helper(` `priv` (L8702-8707)

### fn `def _apply_auto_levels_float(np_module, image_rgb_float, auto_levels_options)` `priv` (L8751-8857)
...
### fn `def _clip_auto_levels_out_of_gamut_float(np_module, image_rgb, maxval=1.0)` `priv` (L8858-9018)
...
### fn `def _filmlike_clip_rgb_tone(primary, middle, lower)` `priv` (L8888-8921)
- @brief Clip overflowing RGB triplets with RawTherapee film-like gamut logic.
...
### fn `def _clip_auto_levels_out_of_gamut_uint16(` `priv` (L9019-9020)

### fn `def _hlrecovery_luminance_float(np_module, image_rgb, maxval=1.0)` `priv` (L9050-9096)
...
### fn `def _hlrecovery_luminance_uint16(np_module, image_rgb, maxval=_AUTO_LEVELS_CODE_MAX)` `priv` (L9097-9125)
...
### fn `def _hlrecovery_cielab_float(` `priv` (L9126-9127)

### fn `def _f_lab(values)` `priv` (L9160-9167)
...
### fn `def _f2xyz(values)` `priv` (L9168-9174)

### fn `def _hlrecovery_cielab_uint16(` `priv` (L9210-9211)

### fn `def _hlrecovery_blend_float(np_module, image_rgb, hlmax, maxval=1.0)` `priv` (L9245-9350)
...
### fn `def _hlrecovery_blend_uint16(` `priv` (L9351-9352)

### fn `def _dilate_mask_float(np_module, mask)` `priv` (L9387-9409)
...
### fn `def _box_mean_3x3_float(np_module, image_2d)` `priv` (L9410-9433)
...
### fn `def _hlrecovery_color_propagation_float(np_module, image_rgb, maxval=1.0)` `priv` (L9434-9478)
...
### fn `def _hlrecovery_color_propagation_uint16(` `priv` (L9479-9480)

### fn `def _hlrecovery_inpaint_opposed_float(` `priv` (L9510-9511)
...
### fn `def _hlrecovery_inpaint_opposed_uint16(` `priv` (L9564-9565)
...
### fn `def _apply_auto_brightness_rgb_float(` `priv` (L9607-9610)
...
### fn `def _clamp01(np_module, values)` `priv` (L9667-9680)
...
### fn `def _gaussian_kernel_2d(np_module, sigma, radius=None)` `priv` (L9681-9703)
...
### fn `def _rgb_to_hsl(np_module, rgb)` `priv` (L9704-9737)
...
### fn `def _hue_to_rgb(np_module, p_values, q_values, t_values)` `priv` (L9738-9768)
...
### fn `def _hsl_to_rgb(np_module, hue, saturation, lightness)` `priv` (L9769-9809)
...
### fn `def _selective_blur_contrast_gated_vectorized(` `priv` (L9810-9811)

### fn `def _level_per_channel_adaptive(np_module, rgb, low_pct=0.1, high_pct=99.9)` `priv` (L9860-9882)
...
### fn `def _sigmoidal_contrast(np_module, rgb, contrast=3.0, midpoint=0.5)` `priv` (L9883-9907)
...
### fn `def logistic(z_values)` (L9898-9900)
- @brief Execute sigmoidal contrast stage.
...
### fn `def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=0.8)` `priv` (L9908-9925)
...
### fn `def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)` `priv` (L9926-9949)
...
### fn `def _high_pass_math_gray(cv2_module, np_module, rgb, blur_sigma=2.5)` `priv` (L9950-9973)
...
### fn `def _overlay_composite(np_module, base_rgb, overlay_gray)` `priv` (L9974-9995)
...
### fn `def _apply_validated_auto_adjust_pipeline(` `priv` (L9996-10002)

### fn `def _load_piexif_dependency()` `priv` (L10111-10128)
...
### fn `def _encode_jpg(` `priv` (L10129-10140)

### fn `def _collect_processing_errors(rawpy_module)` `priv` (L10272-10300)
...
### fn `def _is_supported_runtime_os()` `priv` (L10301-10320)
...
|`MergeGammaOption`|class|pub|446-464|class MergeGammaOption|
|`ResolvedMergeGamma`|class|pub|466-491|class ResolvedMergeGamma|
|`ExifGammaTags`|class|pub|493-511|class ExifGammaTags|
...
|`_extract_exif_gamma_tags`|fn|priv|3255-3307|def _extract_exif_gamma_tags(input_dng)|
|`_resolve_auto_merge_gamma`|fn|priv|3308-3372|def _resolve_auto_merge_gamma(exif_gamma_tags, source_gam...|
|`_describe_resolved_merge_gamma`|fn|priv|3373-3432|def _describe_resolved_merge_gamma(resolved_merge_gamma)|
|`_format_gamma_number`|fn|priv|3384-3397|def _format_gamma_number(value)|
|`_describe_exif_gamma_tags`|fn|priv|3433-3454|def _describe_exif_gamma_tags(exif_gamma_tags)|
|`_ensure_three_channel_float_array_no_clip`|fn|priv|3455-3486|def _ensure_three_channel_float_array_no_clip(np_module, ...|
|`_apply_merge_gamma_float`|fn|priv|3487-3539|def _apply_merge_gamma_float(np_module, image_rgb_float, ...|
|`_parse_run_options`|fn|priv|3540-3739|def _parse_run_options(args)|
|`_load_image_dependencies`|fn|priv|4401-4438|def _load_image_dependencies()|
|`_parse_exif_datetime_to_timestamp`|fn|priv|4439-4469|def _parse_exif_datetime_to_timestamp(datetime_raw)|
|`_parse_exif_exposure_time_to_seconds`|fn|priv|4470-4532|def _parse_exif_exposure_time_to_seconds(exposure_raw)|
|`_extract_dng_exif_payload_and_timestamp`|fn|priv|4533-4627|def _extract_dng_exif_payload_and_timestamp(pil_image_mod...|
|`_read_exif_value`|fn|priv|4577-4594|def _read_exif_value(exif_tag)|
|`_resolve_thumbnail_transpose_map`|fn|priv|4628-4659|def _resolve_thumbnail_transpose_map(pil_image_module)|
|`_apply_orientation_transform`|fn|priv|4660-4682|def _apply_orientation_transform(pil_image_module, pil_im...|
|`_build_oriented_thumbnail_jpeg_bytes`|fn|priv|4683-4684|def _build_oriented_thumbnail_jpeg_bytes(|
|`_coerce_exif_int_like_value`|fn|priv|4715-4757|def _coerce_exif_int_like_value(raw_value)|
|`_normalize_ifd_integer_like_values_for_piexif_dump`|fn|priv|4758-4891|def _normalize_ifd_integer_like_values_for_piexif_dump(pi...|
|`_refresh_output_jpg_exif_thumbnail_after_save`|fn|priv|4892-4898|def _refresh_output_jpg_exif_thumbnail_after_save(|
|`_set_output_file_timestamps`|fn|priv|4948-4962|def _set_output_file_timestamps(output_jpg, exif_timestamp)|
|`_sync_output_file_timestamps_from_exif`|fn|priv|4963-4980|def _sync_output_file_timestamps_from_exif(output_jpg, ex...|
|`_build_exposure_multipliers`|fn|priv|4981-4999|def _build_exposure_multipliers(ev_value, ev_zero=0.0)|
|`_build_bracket_images_from_linear_base_float`|fn|priv|5000-5028|def _build_bracket_images_from_linear_base_float(np_modul...|
|`_extract_bracket_images_float`|fn|priv|5029-5033|def _extract_bracket_images_float(|
|`_order_bracket_paths`|fn|priv|5067-5092|def _order_bracket_paths(bracket_paths)|
|`_order_hdr_plus_reference_paths`|fn|priv|5093-5108|def _order_hdr_plus_reference_paths(bracket_paths)|
|`_format_external_command_for_log`|fn|priv|5109-5124|def _format_external_command_for_log(command)|
|`_run_luminance_hdr_cli`|fn|priv|5125-5132|def _run_luminance_hdr_cli(|
|`_build_opencv_radiance_exposure_times`|fn|priv|5207-5210|def _build_opencv_radiance_exposure_times(|
|`_build_ev_times_from_ev_zero_and_delta`|fn|priv|5244-5263|def _build_ev_times_from_ev_zero_and_delta(ev_zero, ev_de...|
|`_normalize_opencv_hdr_to_unit_range`|fn|priv|5264-5287|def _normalize_opencv_hdr_to_unit_range(np_module, hdr_rg...|
|`_run_opencv_merge_mertens`|fn|priv|5288-5309|def _run_opencv_merge_mertens(cv2_module, np_module, expo...|
|`_estimate_opencv_camera_response`|fn|priv|5310-5314|def _estimate_opencv_camera_response(|
|`_run_opencv_merge_radiance`|fn|priv|5343-5350|def _run_opencv_merge_radiance(|
|`_normalize_debevec_hdr_to_unit_range`|fn|priv|5414-5433|def _normalize_debevec_hdr_to_unit_range(np_module, hdr_r...|
|`_run_opencv_hdr_merge`|fn|priv|5434-5441|def _run_opencv_hdr_merge(|
|`_hdrplus_box_down2_float32`|fn|priv|5523-5551|def _hdrplus_box_down2_float32(np_module, frames_float32)|
|`_hdrplus_gauss_down4_float32`|fn|priv|5552-5598|def _hdrplus_gauss_down4_float32(np_module, frames_float32)|
|`_hdrplus_build_scalar_proxy_float32`|fn|priv|5599-5632|def _hdrplus_build_scalar_proxy_float32(np_module, frames...|
|`_hdrplus_compute_tile_start_positions`|fn|priv|5633-5653|def _hdrplus_compute_tile_start_positions(np_module, axis...|
|`_hdrplus_trunc_divide_int32`|fn|priv|5654-5672|def _hdrplus_trunc_divide_int32(np_module, values_int32, ...|
|`_hdrplus_compute_alignment_bounds`|fn|priv|5673-5697|def _hdrplus_compute_alignment_bounds(search_radius)|
|`_hdrplus_compute_alignment_margin`|fn|priv|5698-5716|def _hdrplus_compute_alignment_margin(search_radius, divi...|
|`_hdrplus_extract_overlapping_tiles`|fn|priv|5717-5722|def _hdrplus_extract_overlapping_tiles(|
|`_hdrplus_extract_aligned_tiles`|fn|priv|5775-5781|def _hdrplus_extract_aligned_tiles(|
|`_hdrplus_align_layer`|fn|priv|5854-5861|def _hdrplus_align_layer(|
|`_hdrplus_align_layers`|fn|priv|5951-6038|def _hdrplus_align_layers(np_module, scalar_frames, hdrpl...|
|`_hdrplus_resolve_temporal_runtime_options`|fn|priv|6039-6063|def _hdrplus_resolve_temporal_runtime_options(hdrplus_opt...|
|`_hdrplus_compute_temporal_weights`|fn|priv|6064-6068|def _hdrplus_compute_temporal_weights(|
|`_hdrplus_merge_temporal_rgb`|fn|priv|6149-6155|def _hdrplus_merge_temporal_rgb(|
|`_hdrplus_merge_spatial_rgb`|fn|priv|6204-6276|def _hdrplus_merge_spatial_rgb(np_module, temporal_tiles,...|
|`_run_hdr_plus_merge`|fn|priv|6277-6281|def _run_hdr_plus_merge(|
|`_convert_compression_to_quality`|fn|priv|6369-6379|def _convert_compression_to_quality(jpg_compression)|
|`_collect_missing_external_executables`|fn|priv|6380-6382|def _collect_missing_external_executables(|
|`_resolve_auto_adjust_dependencies`|fn|priv|6401-6426|def _resolve_auto_adjust_dependencies()|
|`_resolve_numpy_dependency`|fn|priv|6427-6446|def _resolve_numpy_dependency()|
|`_to_float32_image_array`|fn|priv|6447-6478|def _to_float32_image_array(np_module, image_data)|
|`_normalize_float_rgb_image`|fn|priv|6479-6506|def _normalize_float_rgb_image(np_module, image_data)|
|`_write_rgb_float_tiff16`|fn|priv|6507-6530|def _write_rgb_float_tiff16(imageio_module, np_module, ou...|
|`_write_debug_rgb_float_tiff`|fn|priv|6531-6536|def _write_debug_rgb_float_tiff(|
|`_build_debug_artifact_context`|fn|priv|6566-6586|def _build_debug_artifact_context(output_jpg, input_dng, ...|
|`_format_debug_ev_suffix_value`|fn|priv|6587-6604|def _format_debug_ev_suffix_value(ev_value)|
|`_materialize_bracket_tiffs_from_float`|fn|priv|6605-6609|def _materialize_bracket_tiffs_from_float(|
|`_to_uint8_image_array`|fn|priv|6639-6685|def _to_uint8_image_array(np_module, image_data)|
|`_to_uint16_image_array`|fn|priv|6686-6730|def _to_uint16_image_array(np_module, image_data)|
|`_apply_post_gamma_float`|fn|priv|6731-6756|def _apply_post_gamma_float(np_module, image_rgb_float, g...|
|`_apply_brightness_float`|fn|priv|6757-6779|def _apply_brightness_float(np_module, image_rgb_float, b...|
|`_apply_contrast_float`|fn|priv|6780-6804|def _apply_contrast_float(np_module, image_rgb_float, con...|
|`_apply_saturation_float`|fn|priv|6805-6834|def _apply_saturation_float(np_module, image_rgb_float, s...|
|`_apply_static_postprocess_float`|fn|priv|6835-6840|def _apply_static_postprocess_float(|
|`_to_linear_srgb`|fn|priv|6917-6934|def _to_linear_srgb(np_module, image_srgb)|
|`_from_linear_srgb`|fn|priv|6935-6952|def _from_linear_srgb(np_module, image_linear)|
|`_compute_bt709_luminance`|fn|priv|6953-6970|def _compute_bt709_luminance(np_module, linear_rgb)|
|`_analyze_luminance_key`|fn|priv|6971-7010|def _analyze_luminance_key(np_module, luminance, eps)|
|`_choose_auto_key_value`|fn|priv|7011-7056|def _choose_auto_key_value(key_analysis, auto_brightness_...|
|`_reinhard_global_tonemap_luminance`|fn|priv|7057-7062|def _reinhard_global_tonemap_luminance(|
|`_luminance_preserving_desaturate_to_fit`|fn|priv|7096-7123|def _luminance_preserving_desaturate_to_fit(np_module, rg...|
|`_apply_mild_local_contrast_bgr_uint16`|fn|priv|7124-7162|def _apply_mild_local_contrast_bgr_uint16(cv2_module, np_...|
|`_quantize_clahe_luminance_bins`|fn|priv|7163-7188|def _quantize_clahe_luminance_bins(np_module, luminance_v...|
|`_build_clahe_float_tile_histogram`|fn|priv|7189-7210|def _build_clahe_float_tile_histogram(np_module, luminanc...|
|`_clip_clahe_histogram`|fn|priv|7211-7258|def _clip_clahe_histogram(np_module, histogram, clip_limi...|
|`_build_clahe_float_lut`|fn|priv|7259-7278|def _build_clahe_float_lut(np_module, histogram, tile_pop...|
|`_pad_clahe_luminance_float`|fn|priv|7279-7317|def _pad_clahe_luminance_float(np_module, luminance_float...|
|`_build_clahe_axis_interpolation`|fn|priv|7318-7345|def _build_clahe_axis_interpolation(np_module, axis_lengt...|
|`_build_clahe_tile_luts_float`|fn|priv|7346-7397|def _build_clahe_tile_luts_float(np_module, luminance_flo...|
|`_interpolate_clahe_bilinear_float`|fn|priv|7398-7450|def _interpolate_clahe_bilinear_float(np_module, luminanc...|
|`_apply_clahe_luminance_float`|fn|priv|7451-7482|def _apply_clahe_luminance_float(np_module, luminance_flo...|
|`_reconstruct_rgb_from_ycrcb_luma_float`|fn|priv|7483-7506|def _reconstruct_rgb_from_ycrcb_luma_float(cv2_module, np...|
|`_apply_clahe_luma_rgb_float`|fn|priv|7507-7556|def _apply_clahe_luma_rgb_float(cv2_module, np_module, im...|
|`_rt_gamma2`|fn|priv|7557-7576|def _rt_gamma2(np_module, values)|
|`_rt_igamma2`|fn|priv|7577-7597|def _rt_igamma2(np_module, values)|
|`_auto_levels_index_to_normalized_value`|fn|priv|7598-7614|def _auto_levels_index_to_normalized_value(histogram_valu...|
|`_auto_levels_normalized_to_legacy_code_value`|fn|priv|7615-7630|def _auto_levels_normalized_to_legacy_code_value(value)|
|`_auto_levels_normalized_to_legacy_code`|fn|priv|7631-7647|def _auto_levels_normalized_to_legacy_code(np_module, val...|
|`_auto_levels_legacy_code_to_normalized`|fn|priv|7648-7663|def _auto_levels_legacy_code_to_normalized(np_module, val...|
|`_pack_auto_levels_metrics`|fn|priv|7664-7679|def _pack_auto_levels_metrics(|
|`_build_autoexp_histogram_rgb_float`|fn|priv|7731-7766|def _build_autoexp_histogram_rgb_float(np_module, image_r...|
|`_build_autoexp_histogram_rgb_uint16`|fn|priv|7767-7799|def _build_autoexp_histogram_rgb_uint16(np_module, image_...|
|`_compute_auto_levels_from_histogram`|fn|priv|7800-7999|def _compute_auto_levels_from_histogram(np_module, histog...|
|`_rt_simplebasecurve_scalar`|fn|priv|8073-8165|def _rt_simplebasecurve_scalar(x_value, black, shadow_rec...|
|`_basel`|fn|priv|8086-8102|def _basel(x_input, slope_start, slope_end)|
|`_baseu`|fn|priv|8103-8105|def _baseu(x_input, slope_start, slope_end)|
|`_cupper`|fn|priv|8106-8125|def _cupper(x_input, slope_value, highlight_recovery)|
|`_clower`|fn|priv|8126-8128|def _clower(x_input, slope_value, shadow_value)|
|`_clower2`|fn|priv|8129-8140|def _clower2(x_input, slope_value, shadow_value)|
|`_build_rt_nurbs_curve_lut`|fn|priv|8166-8299|def _build_rt_nurbs_curve_lut(np_module, x_points, y_poin...|
|`_sample_auto_levels_lut_float`|fn|priv|8300-8306|def _sample_auto_levels_lut_float(|
|`_build_auto_levels_full_histogram_rgb_float`|fn|priv|8338-8369|def _build_auto_levels_full_histogram_rgb_float(np_module...|
|`_rt_hlcurve_float`|fn|priv|8370-8400|def _rt_hlcurve_float(np_module, exp_scale, comp, hlrange...|
|`_build_auto_levels_tone_curve_state`|fn|priv|8401-8590|def _build_auto_levels_tone_curve_state(np_module, image_...|
|`_apply_auto_levels_tonal_transform_float`|fn|priv|8591-8594|def _apply_auto_levels_tonal_transform_float(|
|`_auto_levels_has_full_tone_metrics`|fn|priv|8678-8701|def _auto_levels_has_full_tone_metrics(auto_levels_metrics)|
|`_call_auto_levels_compat_helper`|fn|priv|8702-8707|def _call_auto_levels_compat_helper(|
|`_apply_auto_levels_float`|fn|priv|8751-8857|def _apply_auto_levels_float(np_module, image_rgb_float, ...|
|`_clip_auto_levels_out_of_gamut_float`|fn|priv|8858-9018|def _clip_auto_levels_out_of_gamut_float(np_module, image...|
|`_filmlike_clip_rgb_tone`|fn|priv|8888-8921|def _filmlike_clip_rgb_tone(primary, middle, lower)|
|`_clip_auto_levels_out_of_gamut_uint16`|fn|priv|9019-9020|def _clip_auto_levels_out_of_gamut_uint16(|
|`_hlrecovery_luminance_float`|fn|priv|9050-9096|def _hlrecovery_luminance_float(np_module, image_rgb, max...|
|`_hlrecovery_luminance_uint16`|fn|priv|9097-9125|def _hlrecovery_luminance_uint16(np_module, image_rgb, ma...|
|`_hlrecovery_cielab_float`|fn|priv|9126-9127|def _hlrecovery_cielab_float(|
|`_f_lab`|fn|priv|9160-9167|def _f_lab(values)|
|`_f2xyz`|fn|priv|9168-9174|def _f2xyz(values)|
|`_hlrecovery_cielab_uint16`|fn|priv|9210-9211|def _hlrecovery_cielab_uint16(|
|`_hlrecovery_blend_float`|fn|priv|9245-9350|def _hlrecovery_blend_float(np_module, image_rgb, hlmax, ...|
|`_hlrecovery_blend_uint16`|fn|priv|9351-9352|def _hlrecovery_blend_uint16(|
|`_dilate_mask_float`|fn|priv|9387-9409|def _dilate_mask_float(np_module, mask)|
|`_box_mean_3x3_float`|fn|priv|9410-9433|def _box_mean_3x3_float(np_module, image_2d)|
|`_hlrecovery_color_propagation_float`|fn|priv|9434-9478|def _hlrecovery_color_propagation_float(np_module, image_...|
|`_hlrecovery_color_propagation_uint16`|fn|priv|9479-9480|def _hlrecovery_color_propagation_uint16(|
|`_hlrecovery_inpaint_opposed_float`|fn|priv|9510-9511|def _hlrecovery_inpaint_opposed_float(|
|`_hlrecovery_inpaint_opposed_uint16`|fn|priv|9564-9565|def _hlrecovery_inpaint_opposed_uint16(|
|`_apply_auto_brightness_rgb_float`|fn|priv|9607-9610|def _apply_auto_brightness_rgb_float(|
|`_clamp01`|fn|priv|9667-9680|def _clamp01(np_module, values)|
|`_gaussian_kernel_2d`|fn|priv|9681-9703|def _gaussian_kernel_2d(np_module, sigma, radius=None)|
|`_rgb_to_hsl`|fn|priv|9704-9737|def _rgb_to_hsl(np_module, rgb)|
|`_hue_to_rgb`|fn|priv|9738-9768|def _hue_to_rgb(np_module, p_values, q_values, t_values)|
|`_hsl_to_rgb`|fn|priv|9769-9809|def _hsl_to_rgb(np_module, hue, saturation, lightness)|
|`_selective_blur_contrast_gated_vectorized`|fn|priv|9810-9811|def _selective_blur_contrast_gated_vectorized(|
|`_level_per_channel_adaptive`|fn|priv|9860-9882|def _level_per_channel_adaptive(np_module, rgb, low_pct=0...|
|`_sigmoidal_contrast`|fn|priv|9883-9907|def _sigmoidal_contrast(np_module, rgb, contrast=3.0, mid...|
|`logistic`|fn|pub|9898-9900|def logistic(z_values)|
|`_vibrance_hsl_gamma`|fn|priv|9908-9925|def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=...|
|`_gaussian_blur_rgb`|fn|priv|9926-9949|def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)|
|`_high_pass_math_gray`|fn|priv|9950-9973|def _high_pass_math_gray(cv2_module, np_module, rgb, blur...|
|`_overlay_composite`|fn|priv|9974-9995|def _overlay_composite(np_module, base_rgb, overlay_gray)|
|`_apply_validated_auto_adjust_pipeline`|fn|priv|9996-10002|def _apply_validated_auto_adjust_pipeline(|
|`_load_piexif_dependency`|fn|priv|10111-10128|def _load_piexif_dependency()|
|`_encode_jpg`|fn|priv|10129-10140|def _encode_jpg(|
|`_collect_processing_errors`|fn|priv|10272-10300|def _collect_processing_errors(rawpy_module)|
|`_is_supported_runtime_os`|fn|priv|10301-10320|def _is_supported_runtime_os()|
|`run`|fn|pub|10321-10520|def run(args)|
