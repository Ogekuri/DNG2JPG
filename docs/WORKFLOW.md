  ...
  - main(argv): parse top-level management commands and dispatch conversion [src/dng2jpg/core.py]
    ...
    - run(args): HDR conversion pipeline orchestration [src/dng2jpg/dng2jpg.py]
      ...
      - _parse_run_options(args): parse and validate conversion options, mapping `--ev-bracketing=auto` to automatic exposure, finite numeric `--ev-bracketing` to static exposure, and `--auto-white-balance=disable` to explicit stage bypass [src/dng2jpg/dng2jpg.py]
        ...
        - _parse_ev_center_option(...): EV center parsing for `--ev=<value>` (numeric only; auto sentinel handled inline) [src/dng2jpg/dng2jpg.py]
      ...
      - _resolve_joint_auto_ev_solution(...): joint auto EV center+delta resolution when both `--ev=auto` and `--ev-bracketing=auto` [src/dng2jpg/dng2jpg.py]
      ...
      - _resolve_auto_ev_delta(...): standalone auto EV delta resolution when only `--ev-bracketing=auto` (without `--ev=auto`) [src/dng2jpg/dng2jpg.py]
