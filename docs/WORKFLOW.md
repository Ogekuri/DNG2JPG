  ...
  - `main(...)`: parse top-level argv and dispatch management/conversion branches [`src/dng2jpg/core.py`]
    ...
    - `run(...)`: execute conversion pipeline [`src/dng2jpg/dng2jpg.py`]
      ...
      - `_encode_jpg(...)`: encode merged TIFF into final JPG with uint16 static postprocess and single final quantization [`src/dng2jpg/dng2jpg.py`]
        ...
        - `_apply_static_postprocess_uint16(...)`: execute static gamma/brightness/contrast/saturation chain in uint16 with Pillow-compatible byte-domain parity mapping [`src/dng2jpg/dng2jpg.py`]
          - `_apply_post_gamma_uint16(...)`: apply post-gamma curve over uint16 tensor [`src/dng2jpg/dng2jpg.py`]
          - `_apply_brightness_uint16(...)`: apply Pillow-compatible Brightness on byte-quantized view and lift to uint16 [`src/dng2jpg/dng2jpg.py`]
          - `_apply_contrast_uint16(...)`: apply Pillow-compatible Contrast on byte-quantized view and lift to uint16 [`src/dng2jpg/dng2jpg.py`]
          - `_apply_saturation_uint16(...)`: apply Pillow-compatible Color on byte-quantized view and lift to uint16 [`src/dng2jpg/dng2jpg.py`]
