    ...
    - `run(...)`: execute conversion pipeline [`src/dng2jpg/dng2jpg.py`]
      ...
      - `_parse_run_options(...)`: parse positional/optional CLI arguments into normalized option dataclasses [`src/dng2jpg/dng2jpg.py`]
        ...
        - `_parse_hdr_merge_option(...)`: parse `--hdr-merge` backend selector [`src/dng2jpg/dng2jpg.py`]
          - `print_error(...)`: emit parse diagnostics [`src/shell_scripts/utils.py`]
        - `_resolve_default_postprocess(...)`: derive backend-specific default postprocess factors from resolved `--hdr-merge` mode [`src/dng2jpg/dng2jpg.py`]
        - `_parse_auto_brightness_options(...)`: build auto-brightness options dataclass with original photographic tonemap controls [`src/dng2jpg/dng2jpg.py`]
          - `_parse_float_exclusive_range_option(...)`: parse `(min,max)` float range [`src/dng2jpg/dng2jpg.py`]
            - `print_error(...)`: emit parse diagnostics [`src/shell_scripts/utils.py`]
          - `_parse_explicit_boolean_option(...)`: parse explicit on/off values for auto-brightness local-contrast and desaturation flags [`src/dng2jpg/dng2jpg.py`]
            - `print_error(...)`: emit parse diagnostics [`src/shell_scripts/utils.py`]
          - `_parse_float_in_range_option(...)`: parse inclusive float range for auto-brightness local-contrast blend strength [`src/dng2jpg/dng2jpg.py`]
            - `print_error(...)`: emit parse diagnostics [`src/shell_scripts/utils.py`]
          - `_parse_positive_float_option(...)`: parse positive auto-brightness scalar controls [`src/dng2jpg/dng2jpg.py`]
            - `print_error(...)`: emit parse diagnostics [`src/shell_scripts/utils.py`]
          - `_parse_positive_int_pair_option(...)`: parse CLAHE tile-grid pair for auto-brightness local contrast [`src/dng2jpg/dng2jpg.py`]
            - `print_error(...)`: emit parse diagnostics [`src/shell_scripts/utils.py`]
      ...
      - `_run_hdr_plus_merge(...)`: merge in-memory float brackets through HDR+ temporal/spatial tile backend and return normalized RGB float output [`src/dng2jpg/dng2jpg.py`]
        - `_normalize_float_rgb_image(...)`: normalize HDR+ input brackets to RGB float `[0,1]` [`src/dng2jpg/dng2jpg.py`]
        - `_to_uint16_image_array(...)`: adapt normalized float brackets to `uint16` only inside the HDR+ step [`src/dng2jpg/dng2jpg.py`]
        - `_hdrplus_build_scalar_proxy_float32(...)`: derive HDR+ scalar proxy from aligned RGB brackets using the selected proxy mode [`src/dng2jpg/dng2jpg.py`]
        ...
        - `_hdrplus_compute_temporal_weights(...)`: compute aligned temporal inverse-distance weights on 16x16 downsampled tiles [`src/dng2jpg/dng2jpg.py`]
        ...
        - `_hdrplus_merge_spatial_rgb(...)`: blend temporally merged tiles with raised-cosine overlap weights [`src/dng2jpg/dng2jpg.py`]
        - `_normalize_float_rgb_image(...)`: normalize HDR+ merged output to RGB float `[0,1]` [`src/dng2jpg/dng2jpg.py`]
      - `_run_enfuse(...)`: merge float brackets through enfuse backend and return normalized RGB float output [`src/dng2jpg/dng2jpg.py`]
        - `_materialize_bracket_tiffs_from_float(...)`: serialize float brackets as canonical 16-bit TIFF files [`src/dng2jpg/dng2jpg.py`]
          - `_write_rgb_float_tiff16(...)`: convert one RGB float tensor to TIFF16 artifact [`src/dng2jpg/dng2jpg.py`]
            - `_normalize_float_rgb_image(...)`: normalize stage payloads to RGB float `[0,1]` [`src/dng2jpg/dng2jpg.py`]
            - `_to_uint16_image_array(...)`: quantize normalized float tensors to `uint16` for file-oriented tools [`src/dng2jpg/dng2jpg.py`]
        - `_normalize_float_rgb_image(...)`: normalize enfuse output to RGB float `[0,1]` [`src/dng2jpg/dng2jpg.py`]
      - `_encode_jpg(...)`: encode merged RGB float into final JPG with float stage interfaces and one final quantization [`src/dng2jpg/dng2jpg.py`]
        ...
        - `_apply_auto_brightness_rgb_float(...)`: apply original BT.709 key-adaptive photographic auto-brightness on RGB float [`src/dng2jpg/dng2jpg.py`]
          ...
          - `_to_uint16_image_array(...)`: adapt the auto-brightness local-contrast substep to OpenCV 16-bit CLAHE when enabled [`src/dng2jpg/dng2jpg.py`]
          - `_apply_mild_local_contrast_bgr_uint16(...)`: optionally blend CLAHE micro-contrast on 16-bit luminance channel after sRGB re-encoding [`src/dng2jpg/dng2jpg.py`]
        ...
        - `_apply_static_postprocess_float(...)`: execute static gamma/brightness/contrast/saturation chain with float interfaces and step-local uint16 adaptation [`src/dng2jpg/dng2jpg.py`]
          - `_to_uint16_image_array(...)`: adapt normalized float tensors to `uint16` only inside static postprocess [`src/dng2jpg/dng2jpg.py`]
        ...
        - `_apply_validated_auto_adjust_pipeline(...)`: run ImageMagick auto-adjust chain behind step-local TIFF16 artifacts and return RGB float [`src/dng2jpg/dng2jpg.py`]
          ...
          - `_normalize_float_rgb_image(...)`: normalize ImageMagick output back to RGB float `[0,1]` [`src/dng2jpg/dng2jpg.py`]
        - `_apply_validated_auto_adjust_pipeline_opencv(...)`: run OpenCV auto-adjust chain fully in float domain [`src/dng2jpg/dng2jpg.py`]
