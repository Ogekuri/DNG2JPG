    ...
    - `run(...)`: execute conversion pipeline [`src/dng2jpg/dng2jpg.py`]
      ...
      - `_parse_run_options(...)`: parse positional/optional CLI arguments into normalized option dataclasses [`src/dng2jpg/dng2jpg.py`]
        ...
        - `_parse_auto_ev_option(...)`: parse `--auto-ev <enable|disable>` selector value [`src/dng2jpg/dng2jpg.py`]
          - `print_error(...)`: emit parse diagnostics [`src/shell_scripts/utils.py`]
        - `_parse_auto_zero_option(...)`: parse `--auto-zero <enable|disable>` selector value [`src/dng2jpg/dng2jpg.py`]
          - `print_error(...)`: emit parse diagnostics [`src/shell_scripts/utils.py`]
      ...
      - `_resolve_auto_adjust_dependencies(...)`: import OpenCV/numpy dependencies for enabled auto-adjust, auto-zero, or OpenCV merge modes [`src/dng2jpg/dng2jpg.py`]
      ...
      - `_resolve_ev_zero(...)`: resolve final EV center from manual or migrated auto-zero evaluation [`src/dng2jpg/dng2jpg.py`]
        - `_resolve_auto_adjust_dependencies(...)`: load OpenCV/numpy when auto-zero dependencies were not pre-resolved [`src/dng2jpg/dng2jpg.py`]
          - `print_error(...)`: emit dependency diagnostics [`src/shell_scripts/utils.py`]
        - `_extract_base_rgb_linear_float(...)`: extract one normalized linear gamma=`1` RGB base tensor when the caller did not precompute it [`src/dng2jpg/dng2jpg.py`]
        - `_evaluate_auto_zero_exposure(...)`: compute and print the three migrated auto-zero candidate evaluations before selecting the minimum [`src/dng2jpg/dng2jpg.py`]
        ...
        - `_apply_auto_percentage_scaling(...)`: scale the selected auto-zero EV by percentage and quantize toward zero [`src/dng2jpg/dng2jpg.py`]
      ...
      - `_run_opencv_hdr_merge(...)`: merge in-memory float brackets through selected OpenCV `Debevec`, `Robertson`, or `Mertens` path and return normalized RGB float output [`src/dng2jpg/dng2jpg.py`]
        ...
        - `_run_opencv_merge_radiance(...)`: execute Debevec or Robertson radiance path on the shared linear bracket contract with calibrated inverse response and optional simple tone mapping [`src/dng2jpg/dng2jpg.py`]
          - `_estimate_opencv_camera_response(...)`: estimate inverse camera response with the OpenCV calibrator matching the selected radiance algorithm [`src/dng2jpg/dng2jpg.py`]
          - `cv2.createMergeDebevec().process(...)`: merge Debevec HDR radiance from linear RGB float brackets with `times` and calibrated `response` (external boundary)
          - `cv2.createMergeRobertson().process(...)`: merge Robertson HDR radiance from linear RGB float brackets with `times` and calibrated `response` (external boundary)
