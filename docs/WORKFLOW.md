  ...
  - `main(...)`: parse top-level argv and dispatch management/conversion branches [`src/dng2jpg/core.py`]
    ...
    - `run(...)`: execute conversion pipeline [`src/dng2jpg/dng2jpg.py`]
      ...
      - `_parse_run_options(...)`: parse positional/optional CLI arguments into normalized option dataclasses [`src/dng2jpg/dng2jpg.py`]
        ...
        - `_parse_auto_brightness_options(...)`: build auto-brightness options dataclass with photographic tonemap knobs [`src/dng2jpg/dng2jpg.py`]
          - `_parse_float_exclusive_range_option(...)`: parse `(min,max)` float range [`src/dng2jpg/dng2jpg.py`]
            - `print_error(...)`: emit parse diagnostics [`src/shell_scripts/utils.py`]
        - `_parse_auto_levels_options(...)`: build auto-levels options dataclass with RawTherapee-compatible defaults [`src/dng2jpg/dng2jpg.py`]
          - `_parse_non_negative_float_option(...)`: parse non-negative float controls [`src/dng2jpg/dng2jpg.py`]
            - `print_error(...)`: emit parse diagnostics [`src/shell_scripts/utils.py`]
          - `_parse_optional_bool_option(...)`: parse explicit enable/disable tokens for auto-levels checkbox-style knobs [`src/dng2jpg/dng2jpg.py`]
            - `print_error(...)`: emit parse diagnostics [`src/shell_scripts/utils.py`]
      ...
      - `_run_hdr_plus_merge(...)`: merge bracket images through HDR+ temporal/spatial tile backend [`src/dng2jpg/dng2jpg.py`]
        ...
        - `_hdrplus_luminance_proxy_uint16(...)`: derive scalar HDR+ merge proxy from aligned RGB brackets [`src/dng2jpg/dng2jpg.py`]
        - `_hdrplus_box_down2_uint16(...)`: downsample scalar merge proxy with source 2x2 box average [`src/dng2jpg/dng2jpg.py`]
        - `_hdrplus_extract_overlapping_tiles(...)`: build half-overlapped tile tensors for scalar and RGB stages [`src/dng2jpg/dng2jpg.py`]
        - `_hdrplus_compute_temporal_weights(...)`: compute source temporal inverse-distance weights on 16x16 downsampled tiles [`src/dng2jpg/dng2jpg.py`]
        - `_hdrplus_merge_temporal_rgb(...)`: accumulate 32x32 RGB tiles with reference-inclusive normalization [`src/dng2jpg/dng2jpg.py`]
      ...
      - `_encode_jpg(...)`: encode merged TIFF into final JPG with uint16 static postprocess and single final quantization [`src/dng2jpg/dng2jpg.py`]
        ...
        - `_apply_auto_levels_uint16(...)`: apply RawTherapee-derived auto-levels transfer, optional highlight reconstruction, and optional film-like out-of-gamut clipping [`src/dng2jpg/dng2jpg.py`]
          - `_build_autoexp_histogram_rgb_uint16(...)`: build RGB histogram for auto-levels [`src/dng2jpg/dng2jpg.py`]
          - `_compute_auto_levels_from_histogram(...)`: compute histogram-derived exposure, black, brightness, contrast, and clipping metrics [`src/dng2jpg/dng2jpg.py`]
          ...
          - `_apply_auto_levels_rt_tone_map(...)`: execute the reconstructed RawTherapee auto-levels tone transfer in float domain [`src/dng2jpg/dng2jpg.py`]
            - `_apply_rt_highlight_compression(...)`: apply exposure scaling and highlight shoulder compression [`src/dng2jpg/dng2jpg.py`]
            - `_apply_rt_black_shadow_curve(...)`: apply black-point/shadow correction through RawTherapee base curves [`src/dng2jpg/dng2jpg.py`]
              - `_rt_simplebasecurve(...)`: evaluate RawTherapee shadow/base curve [`src/dng2jpg/dng2jpg.py`]
                - `_rt_clower(...)`: evaluate lower shadow curve [`src/dng2jpg/dng2jpg.py`]
                  - `_rt_cupper(...)`: evaluate upper highlight curve [`src/dng2jpg/dng2jpg.py`]
                    - `_rt_baseu(...)`: evaluate concave base curve [`src/dng2jpg/dng2jpg.py`]
                      - `_rt_basel(...)`: evaluate convex base curve [`src/dng2jpg/dng2jpg.py`]
                - `_rt_clower2(...)`: evaluate negative-black lower curve [`src/dng2jpg/dng2jpg.py`]
            - `_apply_rt_brightness_contrast(...)`: apply brightness and contrast control-point curves in gamma domain [`src/dng2jpg/dng2jpg.py`]
              - `_apply_piecewise_curve(...)`: interpolate monotonic control-point curves over float tensors [`src/dng2jpg/dng2jpg.py`]
          ...
          - `_hlrecovery_inpaint_opposed_uint16(...)`: execute simplified RawTherapee `Inpaint Opposed` highlight reconstruction [`src/dng2jpg/dng2jpg.py`]
            - `_dilate_mask3x3(...)`: dilate clipped highlight masks with one 3x3 step [`src/dng2jpg/dng2jpg.py`]
          - `_filmlike_clip_rgb_float(...)`: apply hue-stable film-like clipping to out-of-range RGB pixels [`src/dng2jpg/dng2jpg.py`]
