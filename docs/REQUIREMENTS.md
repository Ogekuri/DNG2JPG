...
- **DES-008**: MUST resolve OpenCV backend static postprocess defaults per `Debevec`, `Robertson`, and `Mertens` as `(1.0,1.0,1.5,1.05)`, `(1.0,1.0,1.4,1.05)`, and `(1.0,0.9,1.3,1.1)` for `(post_gamma,brightness,contrast,saturation)`.
...
- **REQ-141**: MUST expose OpenCV controls `--opencv-merge-algorithm`, `--opencv-tonemap`, and `--opencv-tonemap-gamma`, defaulting to `Robertson`, `enable`, and algorithm-specific gamma `Debevec=1.0`, `Robertson=0.9`, `Mertens=0.8`.
...
- **REQ-145**: MUST resolve OpenCV backend downstream postprocess defaults per `Debevec`, `Robertson`, and `Mertens` as `(1.0,1.0,1.5,1.05)`, `(1.0,1.0,1.4,1.05)`, and `(1.0,0.9,1.3,1.1)` in `(post_gamma,brightness,contrast,saturation)` order.
...
- **REQ-148**: MUST use monotonically increasing numeric stage suffixes with phase labels covering bracket extraction (`ev_min`, `ev_zero`, `ev_max`), HDR merge, auto-brightness, static postprocess, auto-levels, and auto-adjust checkpoints when those stages execute.
...
- **REQ-199**: MUST parse optional `--white-balance-analysis-source=<ev-zero|linear-base>`, defaulting to `ev-zero`, and MUST reject unknown values.
- **REQ-182**: MUST execute auto-white-balance only when `--auto-white-balance` is specified, after bracket triplet extraction and before HDR backend merge; omitted option MUST print `Auto-white-balance stage: disabled` and bypass the stage.
- **REQ-183**: MUST estimate auto-white-balance correction parameters from one configured analysis image, apply one identical gain vector to all three brackets, and preserve normalized float RGB tensors as canonical triplet contracts.
- **REQ-200**: MUST derive `linear-base` white-balance analysis from the shared unclipped linear base image scaled by `2^ev_zero` before bracket clipping, including `ev_zero != 0`.
...
- **REQ-201**: MUST confine xphoto quantization to backend-local estimation boundaries while preserving float-domain triplet processing and identical triplet gain application.
...
- **TST-031**: MUST verify `_resolve_default_postprocess` resolves `Debevec`, `Robertson`, and `Mertens` OpenCV defaults independently and returns `(1.0,1.0,1.5,1.05)`, `(1.0,1.0,1.4,1.05)`, and `(1.0,0.9,1.3,1.1)`.
...
- **TST-038**: MUST verify debug checkpoint writers emit progressive TIFF filenames for extraction, merge, auto-brightness, static postprocess, auto-levels, and auto-adjust outputs in the output directory.
...
- **TST-060**: MUST verify `_parse_run_options` defaults auto-white-balance to disabled, defaults white-balance analysis source to `ev-zero`, defaults xphoto domain to `source-auto`, and accepts all supported selectors.
- **TST-061**: MUST verify `_parse_run_options` rejects missing or unsupported `--auto-white-balance`, `--white-balance-analysis-source`, and `--white-balance-xphoto-domain` values with deterministic diagnostics.
- **TST-062**: MUST verify `run` prints `Auto-white-balance stage: disabled`, skips auto-white-balance when `--auto-white-balance` is omitted, and forwards extracted bracket tensors unchanged to selected HDR backend.
...
- **TST-066**: MUST verify `IA` white-balance path invokes OpenCV xphoto `createLearningBasedWB`, uses real-image payloads, and configures bit-depth-coherent `range_max`/histogram settings with `uint16` payloads when supported.
- **TST-067**: MUST verify `ColorConstancy` white-balance path applies percentile-based robust masks in linear domain before luminance/channel mean computation.
- **TST-068**: MUST verify `TTL` white-balance path applies percentile-based robust masks in linear domain before `avg_gray/avg_channel` gain computation.
...
- **TST-077**: MUST verify xphoto estimation-domain selector applies only to estimation (`linear|srgb|source-auto`) without altering float-domain triplet gain application contracts.
...
| DES-008 | `src/dng2jpg/dng2jpg.py::_resolve_default_postprocess`; excerpt: OpenCV backend defaults map Debevec/Robertson/Mertens to `(1.0,1.0,1.5,1.05)`, `(1.0,1.0,1.4,1.05)`, `(1.0,0.9,1.3,1.1)`. |
...
| REQ-013 | `src/dng2jpg/dng2jpg.py::_encode_jpg`, `_apply_static_postprocess_float`; excerpt: executes optional auto-brightness before static order `gamma->brightness->contrast->saturation`, then optional auto-levels. |
...
| REQ-100 | `src/dng2jpg/dng2jpg.py::_encode_jpg`, `_apply_auto_levels_float`, `_apply_static_postprocess_float`, `_apply_auto_brightness_rgb_float`; excerpt: executes auto-levels only when resolved state is `enable`, after static postprocess. |
...
| REQ-148 | `src/dng2jpg/dng2jpg.py::run`, `_apply_static_postprocess_float`, `_encode_jpg`, `_apply_validated_auto_adjust_pipeline`; excerpt: emits progressive numeric stage suffixes across extraction, merge, auto-brightness, postprocess, and optional stage checkpoints. |
...
| TST-031 | `tests/test_uint16_postprocess_pipeline.py::test_resolve_default_postprocess_opencv_uses_updated_static_defaults`; verifies OpenCV default static postprocess factors resolve to per-algorithm tuples `(1.0,1.0,1.5,1.05)`, `(1.0,1.0,1.4,1.05)`, `(1.0,0.9,1.3,1.1)`. |
...
| REQ-181 | `src/dng2jpg/dng2jpg.py::_parse_white_balance_mode_option`, `_parse_run_options`, `print_help`; excerpt: parses optional `--auto-white-balance`, validates mode set, and leaves the auto-white-balance stage disabled when omitted. |
...
| REQ-210 | `src/dng2jpg/dng2jpg.py::_parse_white_balance_xphoto_domain_option`, `_parse_run_options`, `print_help`; excerpt: parses optional `--white-balance-xphoto-domain`, validates selector set, and defaults to `source-auto`. |
| REQ-182 | `src/dng2jpg/dng2jpg.py::run`; excerpt: executes auto-white-balance stage only when mode is configured and otherwise prints `Auto-white-balance stage: disabled` before HDR merge dispatch. |
| REQ-183 | `src/dng2jpg/dng2jpg.py::_apply_white_balance_to_bracket_triplet`, `_apply_channel_gains_to_white_balance_triplet`; excerpt: estimates one auto-white-balance gain vector from configured analysis payload and applies it identically to all three bracket tensors. |
| REQ-200 | `src/dng2jpg/dng2jpg.py::_build_white_balance_analysis_image_from_linear_base_float`, `run`; excerpt: builds analysis image from unclipped linear base scaled by `2^ev_zero` before bracket clipping. |
...
| REQ-201 | `src/dng2jpg/dng2jpg.py::_extract_white_balance_channel_gains_from_xphoto`; excerpt: confines quantized adaptation to xphoto estimation boundary while keeping triplet processing and gain application in float domain. |
| REQ-211 | `src/dng2jpg/dng2jpg.py::_estimate_xphoto_white_balance_gains_rgb`, `_extract_white_balance_channel_gains_from_xphoto`; excerpt: `IA` mode applies bit-depth-coherent `setRangeMax`/histogram-bin settings and uses uint16 xphoto payloads when backend support exists. |
...
| TST-060 | `tests/test_uint16_postprocess_pipeline.py::test_parse_run_options_accepts_white_balance_modes_and_selector_defaults`; verifies accepted white-balance selectors and defaults (`ev-zero`, `source-auto`) with auto-white-balance disabled by default. |
| TST-061 | `tests/test_uint16_postprocess_pipeline.py::test_parse_run_options_rejects_invalid_white_balance_mode_or_selectors`; verifies deterministic parse rejection for unsupported white-balance mode, analysis-source, and xphoto-domain values. |
...
| TST-063 | `tests/test_uint16_postprocess_pipeline.py::test_apply_white_balance_to_bracket_triplet_uses_configured_analysis_image_and_identical_gains`; verifies configured analysis image input and identical gain application to full triplet. |
...
| TST-066 | `tests/test_uint16_postprocess_pipeline.py::test_apply_white_balance_to_bracket_triplet_ia_mode_sets_hist_bins`, `test_extract_white_balance_channel_gains_from_xphoto_supports_uint16_payload_for_ia`; verifies `IA` mode range/histogram configuration and uint16 xphoto payload support. |
| TST-067 | `tests/test_uint16_postprocess_pipeline.py::test_apply_white_balance_to_bracket_triplet_color_constancy_mode_uses_robust_masked_statistics`, `test_build_white_balance_robust_analysis_mask_uses_percentile_thresholds`; verifies linear-domain percentile-based robust masking before ColorConstancy statistics. |
| TST-068 | `tests/test_uint16_postprocess_pipeline.py::test_apply_white_balance_to_bracket_triplet_ttl_mode_uses_robust_masked_statistics`, `test_build_white_balance_robust_analysis_mask_uses_percentile_thresholds`; verifies linear-domain percentile-based robust masking before TTL `avg_gray/avg_channel` gains. |
...
| TST-077 | `tests/test_uint16_postprocess_pipeline.py::test_apply_white_balance_to_bracket_triplet_uses_configured_analysis_image_and_identical_gains`, `test_estimate_xphoto_white_balance_gains_source_auto_resolves_srgb_domain`, `test_run_passes_linear_base_white_balance_analysis_when_selected`; verifies estimation-domain selector is estimation-only and triplet gain contracts/stage ordering remain invariant. |
