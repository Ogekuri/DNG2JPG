...
- **TST-060**: MUST verify `_parse_run_options` defaults auto-white-balance to disabled, defaults white-balance analysis source to `ev-zero`, and accepts all supported white-balance modes and analysis-source selectors.
...
- **TST-066**: MUST verify `IA` white-balance path invokes OpenCV xphoto `createLearningBasedWB`, applies `setHistBinNum(256)` when supported, and uses real-image analysis payloads without fixed `(1,9,3)` proxy assumptions.
- **TST-067**: MUST verify `ColorConstancy` white-balance path applies robust masked statistics that exclude near-black and near-saturated pixels before luminance/channel means.
...
| REQ-184 | `src/dng2jpg/dng2jpg.py::_extract_white_balance_channel_gains_from_xphoto`, `_build_xphoto_analysis_image_rgb_float`; excerpt: `Simple` mode uses real-image analysis payload with structured downsampling and no fixed-size proxy strip. |
| REQ-185 | `src/dng2jpg/dng2jpg.py::_extract_white_balance_channel_gains_from_xphoto`, `_build_xphoto_analysis_image_rgb_float`; excerpt: `GrayworldWB` mode uses real-image analysis payload with structured downsampling and no fixed-size proxy strip. |
| REQ-186 | `src/dng2jpg/dng2jpg.py::_extract_white_balance_channel_gains_from_xphoto`, `_estimate_xphoto_white_balance_gains_rgb`; excerpt: `IA` mode configures `setHistBinNum(256)` and uses real-image analysis payload for xphoto parameter estimation. |
...
| REQ-187 | `src/dng2jpg/dng2jpg.py::_build_white_balance_robust_analysis_mask`, `_estimate_color_constancy_white_balance_gains_rgb`; excerpt: computes luminance/channel statistics only on robust masked pixels excluding near-black and near-saturated regions. |
| REQ-188 | `src/dng2jpg/dng2jpg.py::_build_white_balance_robust_analysis_mask`, `_estimate_ttl_white_balance_gains_rgb`; excerpt: computes `avg_gray/avg_channel` gains on robust masked pixels excluding near-black and near-saturated regions. |
| TST-060 | `tests/test_uint16_postprocess_pipeline.py::test_parse_run_options_accepts_white_balance_modes_and_analysis_source_defaults`; verifies accepted mode/source sets and default auto-white-balance-stage-disabled plus `ev-zero` analysis source behavior. |
| TST-061 | `tests/test_uint16_postprocess_pipeline.py::test_parse_run_options_rejects_invalid_white_balance_mode_or_analysis_source`; verifies deterministic parse rejection for unsupported white-balance mode or analysis-source values. |
...
| TST-067 | `tests/test_uint16_postprocess_pipeline.py::test_apply_white_balance_to_bracket_triplet_color_constancy_mode_uses_robust_masked_statistics`; verifies robust masked ColorConstancy statistics before luminance/channel means. |
| TST-068 | `tests/test_uint16_postprocess_pipeline.py::test_apply_white_balance_to_bracket_triplet_ttl_mode_uses_robust_masked_statistics`; verifies robust masked TTL statistics before `avg_gray/avg_channel` gain computation. |
