...
- **DES-003**: MUST parse static EV selectors as finite numeric values without quantization-step enforcement or bit-depth-derived upper-bound contracts, and MUST require derived EV scaling operations to remain finite and representable.
...
- **REQ-170**: MUST apply resolved merge gamma only as the explicit camera-linear-to-display-RGB boundary of OpenCV `Debevec`/`Robertson` and `HDR-Plus`, after backend normalization and before shared static postprocess, without extra clipping.
...
- **REQ-030**: MUST accept finite numeric `--bracketing` values `>=0` and finite numeric `--exposure` values, round each static parsed value to one decimal place, and reject values whose derived `2**EV` scaling is non-representable.
...
- **REQ-032**: MUST evaluate `ev_best`, `ev_ettr`, and `ev_detail` on the normalized linear gamma=`1` RGB image only when `--exposure=auto`, then select `ev_zero=min(ev_best, ev_ettr, ev_detail)` while preserving candidate signs.
...
- **REQ-050**: MUST implement `/tmp/auto-brightness.py` auto-brightness on camera-linear RGB float using BT.709 coefficients only as a deterministic luminance proxy, then return camera-linear RGB float without sRGB encode/decode.
...
- **REQ-104**: MUST map luminance with `L=(a/Lw_bar)*Y` and apply modified Reinhard `Ld=(L*(1+L/Lwhite^2))/(1+L)` with percentile-derived robust `Lwhite` only when finite-luminance spread and sample-count safeguards pass.
- **REQ-226**: MUST use fallback simple Reinhard `Ld=L/(1+L)` when finite-luminance spread is below a configured low-variance threshold or finite luminance sample count is below a minimum threshold.
...
- **REQ-245**: MUST execute Debevec/Robertson radiance merge without any float-to-uint8 conversion before merge.
- **REQ-247**: MUST raise `RuntimeError` and print `opencv-radiance-path: fail-fast` when Debevec/Robertson runtime lacks high-precision merge support.
...
- **REQ-246**: MUST print one deterministic runtime line `opencv-radiance-path: high-precision` before supported Debevec/Robertson merge execution.
...
- **REQ-145**: MUST resolve downstream defaults as `HDR-Plus=(0.9,0.9,1.2,1.0)`, `Luminace-HDR mantiuk08=(0.9,0.8,1.2,1.05)`, and `Luminace-HDR reinhard02=(0.9,1.3,0.9,0.7)`.
- **REQ-248**: MUST resolve downstream defaults as `OpenCV-Merge Debevec=(1.0,1.2,1.5,1.0)`, `OpenCV-Merge Mertens=(1.0,0.9,1.4,1.1)`, and `OpenCV-Merge Robertson=(1.0,1.4,1.4,1.0)`.
- **REQ-249**: MUST resolve downstream defaults as `OpenCV-Tonemap drago=(1.0,1.0,1.4,1.0)`.
...
- **REQ-244**: MUST apply numeric post-gamma to finite RGB float samples as `sign(x)*abs(x)^gamma`, preserving legacy `x^gamma` results for non-negative samples and preventing `NaN`/`Inf` on signed backend outputs.
...
- **REQ-222**: MUST replace `NaN`, `+Inf`, and `-Inf` with `0.0` in `_to_float32_image_array`, `_ensure_three_channel_float_array_no_range_adjust`, and `_coerce_positive_luminance`, and MUST exclude non-finite samples in `_extract_normalized_preview_luminance_stats` before percentile or maximum reductions.
- **REQ-229**: MUST raise `ValueError` with deterministic diagnostics when normalized preview luminance statistics `p_low`, `p_median`, or `p_high` are non-finite after sanitization.
...
- **REQ-197**: MUST apply resolved merge gamma as the explicit tone-mapped-to-display-RGB boundary of OpenCV-Tonemap after tone mapping and before shared static postprocess, and MUST NOT pre-apply merge gamma before tone mapping.
...
- **REQ-237**: MUST cross from camera-linear RGB to display-referred standard RGB only at backend-local merge-gamma or equivalent tone-mapped backend return boundaries before shared static postprocess.
- **REQ-238**: MUST treat bracket `[0,1]` clamp, OpenCV divide-by-max normalization, Mertens `*255.0`, auto-gamma LUT sampling, auto-levels entry clip, `_postprocess` exit clip, and `_encode_jpg` quantization as explicit main-flow boundaries.
- **REQ-239**: MUST classify each explicit main-flow boundary as contract clipping, normalization/rescale, LUT discretization, or integer quantization and MUST document source range, target range, and stage intent.
- **REQ-240**: MUST keep merge gamma and post-gamma as separate operations: merge gamma is backend-local pre-postprocess transfer, while post-gamma is the static postprocess gamma substage.
- **REQ-241**: MUST execute YCrCb, HSL, and CIELab-like image transforms only inside postprocess stages that already operate on display-referred standard RGB float.
- **REQ-242**: MUST keep exposure planning, bracket synthesis, and pre-merge auto-white-balance in camera-linear RGB float and MUST NOT route main-flow image data through YCrCb, HSL, or CIELab-like transforms before merge gamma.
- **REQ-243**: MUST treat repeated auto-adjust stage clamps as explicit display-referred contract boundaries before stage-local statistics and after stage-local arithmetic.
...
- **TST-005**: MUST verify static exposure resolution rounds manual `--bracketing` and `--exposure` to one decimal, defaults omitted selectors to automatic solving, rejects negative or non-finite `--bracketing`, and does not enforce `0.25` EV increments or bit-depth-derived upper bounds.
...
- **TST-011**: MUST verify `_apply_auto_brightness_rgb_float` preserves float I/O and executes the original step order, key-analysis thresholds, safeguarded Reinhard-operator selection, and optional desaturation.
- **TST-090**: MUST verify low-variance safeguard on `1x1` uniform, `8x8` uniform, and near-uniform noisy luminance inputs preserves monotonic mean-level ordering, avoids full whitening, keeps outputs in `[0,1]`, and avoids added quantization.
- **TST-091**: MUST verify EV overflow inputs (for example `1024.0`) raise deterministic `ValueError` in bracket multiplier construction without process crashes.
- **TST-092**: MUST verify preview luminance helpers sanitize `NaN`, `+Inf`, `-Inf` and either return finite statistics or raise deterministic `ValueError` for non-finite normalized percentiles.
- **TST-093**: MUST verify `_calculate_ettr_ev` and `_calculate_detail_preservation_ev` return finite deterministic values for arrays containing only non-finite samples while preserving nominal finite-input behavior.
...
- **TST-094**: MUST verify supported Debevec/Robertson radiance dispatch emits `opencv-radiance-path: high-precision` before merge execution.
- **TST-095**: MUST verify every successful Debevec/Robertson radiance path reaches merge execution without any float-to-uint8 bracket conversion before merge while preserving normalized RGB float32 backend output.
- **TST-099**: MUST verify deterministic unsupported Debevec/Robertson runtimes emit `opencv-radiance-path: fail-fast` and raise `RuntimeError` before any pre-merge uint8 quantization.
...
- **TST-036**: MUST verify OpenCV merge preserves RGB float input/output boundaries, confines Debevec/Robertson `uint8` adaptation to backend-local radiance steps, and keeps Mertens on float payloads before backend normalization.
...
- **TST-096**: MUST verify `_apply_post_gamma_float` maps finite signed samples with `sign(x)*abs(x)^gamma`, yields finite outputs, and preserves legacy results for samples already in `[0,1]`.
- **TST-097**: MUST verify `_apply_static_postprocess_float` with default `OpenCV-Tonemap mantiuk` factors returns finite float output for signed backend payloads without inserting uint8 adaptation before final quantization.
- **TST-098**: MUST verify non-negative default static postprocess outputs remain unchanged for `OpenCV-Merge` and `HDR-Plus` backend-default factor tuples.
...
| DES-003 | `src/dng2jpg/dng2jpg.py::_parse_ev_option`, `_parse_ev_center_option`, `_safe_pow2_ev`; excerpt: finite-number static EV parsing without quantization or bit-depth upper-bound contracts plus deterministic finite/representable EV scaling guard. |
...
| REQ-012 | `src/dng2jpg/dng2jpg.py::_extract_base_rgb_linear_float`, `_extract_bracket_images_float`, `_run_opencv_merge_backend`, `_run_opencv_tonemap_backend`, `_run_hdr_plus_merge`, `_postprocess`, `_encode_jpg`; excerpt: exchanges RGB float tensors across camera-linear pre-merge stages and display-referred postprocess/final-save stages. |
...
| REQ-030 | `src/dng2jpg/dng2jpg.py::_parse_ev_option`, `_parse_ev_center_option`, `_safe_pow2_ev`; excerpt: finite static EV parsing enforces non-negative `--bracketing` only and rejects non-representable derived `2**EV` scaling. |
...
| REQ-227 | `src/dng2jpg/dng2jpg.py::_safe_pow2_ev`, `_build_exposure_multipliers`, `_build_white_balance_analysis_image_from_linear_base_float`, `_build_opencv_radiance_exposure_times`; excerpt: raises deterministic context-specific `ValueError` when EV exponentiation is non-finite, overflowed, or non-positive/non-finite after exponentiation. |
| REQ-228 | `src/dng2jpg/dng2jpg.py::_calculate_ettr_ev`, `_calculate_detail_preservation_ev`; excerpt: finite-sample filtering and deterministic `0.0` fallback for non-finite/empty evaluator payloads. |
...
| REQ-050 | `src/dng2jpg/dng2jpg.py::_apply_auto_brightness_rgb_float`, `_compute_bt709_luminance`; excerpt: executes camera-linear auto-brightness with BT.709-coefficient luminance proxy and returns camera-linear RGB float without sRGB re-encoding. |
...
| REQ-104 | `src/dng2jpg/dng2jpg.py::_reinhard_global_tonemap_luminance`, `_apply_auto_brightness_rgb_float`; excerpt: applies modified Reinhard with percentile-derived robust `Lwhite` only when finite-luminance spread/sample-count safeguards pass. |
| REQ-226 | `src/dng2jpg/dng2jpg.py::_should_use_low_variance_auto_brightness_fallback`, `_reinhard_global_tonemap_luminance`; excerpt: routes low-variance or low-sample luminance inputs to fallback simple Reinhard `Ld=L/(1+L)`. |
| REQ-222 | `src/dng2jpg/dng2jpg.py::_to_float32_image_array`, `_ensure_three_channel_float_array_no_range_adjust`, `_coerce_positive_luminance`, `_extract_normalized_preview_luminance_stats`; excerpt: replaces `NaN/+Inf/-Inf` with `0.0` in float ingress helpers and excludes non-finite preview luminance samples from percentile/max reductions. |
| REQ-229 | `src/dng2jpg/dng2jpg.py::_extract_normalized_preview_luminance_stats`; excerpt: raises deterministic `ValueError` when normalized preview percentile statistics are non-finite after sanitization. |
...
| REQ-106 | `src/dng2jpg/dng2jpg.py::_postprocess`, `_apply_auto_adjust_stage_float`; excerpt: invokes auto-adjust stage entry after auto-levels and before final quantization while preserving RGB float I/O boundaries. |
| REQ-234 | `src/dng2jpg/dng2jpg.py::_apply_auto_adjust_stage_float`, `_postprocess`; excerpt: auto-adjust stage entry performs enable-state validation and pass-through when disabled while consuming auto-levels output as input. |
| REQ-235 | `src/dng2jpg/dng2jpg.py::_apply_auto_adjust_stage_float`, `_apply_validated_auto_adjust_pipeline`; excerpt: keeps float-to-uint16 or TIFF16 adaptation boundaries confined to auto-adjust internals. |
...
| REQ-145 | `src/dng2jpg/dng2jpg.py::_resolve_default_postprocess`; excerpt: assigns downstream postprocess defaults for HDR-Plus and Luminace-HDR tone-mapping variants. |
| REQ-248 | `src/dng2jpg/dng2jpg.py::_resolve_default_postprocess`; excerpt: assigns downstream postprocess defaults for OpenCV-Merge algorithms Debevec, Mertens, and Robertson. |
| REQ-249 | `src/dng2jpg/dng2jpg.py::_resolve_default_postprocess`; excerpt: assigns downstream postprocess defaults for OpenCV-Tonemap drago. |
| REQ-250 | `src/dng2jpg/dng2jpg.py::_resolve_default_postprocess`; excerpt: assigns downstream postprocess defaults for OpenCV-Tonemap mantiuk. |
| REQ-251 | `src/dng2jpg/dng2jpg.py::_resolve_default_postprocess`; excerpt: assigns downstream postprocess defaults for OpenCV-Tonemap reinhard. |
...
| REQ-170 | `src/dng2jpg/dng2jpg.py::_apply_merge_gamma_float`, `_run_opencv_merge_backend`, `_run_hdr_plus_merge`; excerpt: applies merge gamma as the camera-linear-to-display-RGB boundary after backend normalization and before shared postprocess. |
...
| REQ-245 | `src/dng2jpg/dng2jpg.py::_adapt_opencv_radiance_input_high_precision`, `_select_opencv_radiance_path_adapters`, `_run_opencv_merge_radiance`; excerpt: Debevec/Robertson success paths keep float32 payloads through merge entry and never select pre-merge uint8 quantization. |
| REQ-246 | `src/dng2jpg/dng2jpg.py::_print_opencv_radiance_path_diagnostic`, `_run_opencv_merge_radiance`; excerpt: supported Debevec/Robertson execution prints `opencv-radiance-path: high-precision` before merge. |
| REQ-247 | `src/dng2jpg/dng2jpg.py::_probe_opencv_high_precision_radiance_support`, `_select_opencv_radiance_path_adapters`, `_run_opencv_merge_radiance`; excerpt: unsupported Debevec/Robertson runtimes print `opencv-radiance-path: fail-fast` and raise deterministic `RuntimeError` before any pre-merge uint8 quantization. |
...
| REQ-244 | `src/dng2jpg/dng2jpg.py::_apply_post_gamma_float`; excerpt: applies sign-preserving numeric power on finite signed RGB float samples while preserving legacy non-negative power-law outputs. |
...
| TST-011 | `tests/test_uint16_postprocess_pipeline.py::test_apply_auto_brightness_rgb_float_executes_original_stage_order`; verifies float-interface auto-brightness stage order, safeguarded Reinhard-operator selection, and optional desaturation without CLAHE local contrast. |
| TST-090 | `tests/test_uint16_postprocess_pipeline.py::test_apply_auto_brightness_low_variance_fallback_uniform_and_near_uniform_inputs`; verifies low-variance safeguard monotonicity, anti-whitening behavior, bounded output, and float-domain no-quantization behavior on uniform and near-uniform inputs. |
| TST-091 | `tests/test_uint16_postprocess_pipeline.py::test_build_exposure_multipliers_rejects_overflowing_ev_scaling`, `test_build_opencv_radiance_exposure_times_rejects_overflowing_ev_scaling`; verifies deterministic `ValueError` on non-representable EV scaling in static multiplier and OpenCV radiance timing paths. |
| TST-092 | `tests/test_uint16_postprocess_pipeline.py::test_coerce_positive_luminance_rejects_non_finite_values`, `test_extract_normalized_preview_luminance_stats_filters_non_finite_samples`, `test_extract_normalized_preview_luminance_stats_rejects_all_non_finite_samples`; verifies preview luminance sanitization for `NaN/+Inf/-Inf` and deterministic failure on invalid payloads. |
| TST-093 | `tests/test_uint16_postprocess_pipeline.py::test_calculate_ettr_ev_returns_zero_for_all_non_finite_samples`, `test_calculate_detail_preservation_ev_returns_zero_for_all_non_finite_samples`, `test_ev_evaluators_keep_finite_outputs_with_mixed_non_finite_samples`; verifies evaluator finite fallback behavior on non-finite and mixed payloads. |
...
| TST-014 | `tests/test_uint16_postprocess_pipeline.py::test_build_ev_times_from_ev_zero_and_delta_matches_bracket_sequence`, `test_build_opencv_radiance_exposure_times_rejects_overflowing_ev_scaling`; verifies deterministic EV-time bracket ordering and deterministic `ValueError` on non-representable radiance EV scaling. |
| TST-015 | `tests/test_uint16_postprocess_pipeline.py::test_normalize_debevec_hdr_to_unit_range_clamps_to_valid_interval`, `test_run_opencv_merge_backend_keeps_mertens_inputs_as_float32`, `test_run_opencv_merge_backend_dispatches_debevec_uint8_radiance_path_with_tonemap`, `test_run_opencv_merge_backend_dispatches_robertson_uint8_radiance_path`; verifies OpenCV merge outputs remain normalized RGB float images bounded to `[0,1]`. |
...
| TST-031 | `tests/test_uint16_postprocess_pipeline.py::test_resolve_default_postprocess_opencv_uses_updated_static_defaults`, `test_resolve_default_postprocess_hdrplus_uses_updated_static_defaults`, `test_resolve_default_postprocess_luminance_uses_updated_tmo_defaults`, `test_resolve_default_postprocess_opencv_tonemap_uses_algorithm_defaults`; verifies exact REQ-145, REQ-248, REQ-249, REQ-250, and REQ-251 tuples for every configured backend variant. |
...
| TST-036 | `tests/test_uint16_postprocess_pipeline.py::test_run_opencv_merge_backend_keeps_mertens_inputs_as_float32`, `test_run_opencv_merge_backend_dispatches_debevec_uint8_radiance_path_with_tonemap`, `test_run_opencv_merge_backend_dispatches_robertson_uint8_radiance_path`; verifies OpenCV merge preserves RGB float I/O while confining radiance-path `uint8` adaptation to backend-local steps. |
...
| TST-096 | `tests/test_uint16_postprocess_pipeline.py::test_apply_post_gamma_float_preserves_signed_finite_outputs_and_non_negative_legacy_behavior`; verifies sign-preserving power, finite outputs, and unchanged non-negative behavior. |
| TST-097 | `tests/test_uint16_postprocess_pipeline.py::test_apply_static_postprocess_float_preserves_signed_opencv_tonemap_mantiuk_payloads_without_uint8_roundtrip`; verifies signed Mantiuk postprocess stays finite and avoids pre-quantization uint8 adaptation. |
| TST-098 | `tests/test_uint16_postprocess_pipeline.py::test_apply_static_postprocess_float_keeps_non_negative_backend_defaults_stable_for_opencv_merge_and_hdrplus`; verifies unchanged non-negative default static outputs for OpenCV-Merge and HDR-Plus tuples. |
| TST-099 | N/A (No existing unit test covers unsupported-runtime Debevec/Robertson fail-fast rejection at this revision). |
...
| REQ-182 | `src/dng2jpg/dng2jpg.py::_postprocess`, `_apply_auto_white_balance_stage_float`, `run`; excerpt: always invokes auto-white-balance stage entry after auto-brightness, performs mode-enabled validation internally, and prints disabled diagnostics for disabled mode. |
| REQ-236 | `src/dng2jpg/dng2jpg.py::_postprocess`; excerpt: forwards auto-brightness stage output as the mandatory input payload for auto-white-balance stage invocation. |
...
| TST-062 | `tests/test_uint16_postprocess_pipeline.py::test_run_invokes_white_balance_stage_entry_when_mode_not_specified`; verifies disabled diagnostic and mandatory auto-white-balance stage-entry invocation with pass-through behavior when mode is omitted. |
| TST-095 | `tests/test_uint16_postprocess_pipeline.py::test_postprocess_invokes_auto_adjust_stage_entry_after_auto_levels`; verifies postprocess always invokes auto-adjust stage entry after auto-levels with pass-through behavior when disabled. |
...
| REQ-197 | `src/dng2jpg/dng2jpg.py::_run_opencv_tonemap_backend`; excerpt: applies resolved merge gamma only as the tone-mapped-to-display-RGB boundary after tone mapping. |
...
| REQ-237 | `src/dng2jpg/dng2jpg.py::_apply_merge_gamma_float`, `_apply_merge_gamma_float_no_clip`, `_run_luminance_hdr_cli`, `_prepare_postprocess_entry_rgb_float`; excerpt: backend-local merge-gamma or tone-mapped return boundaries hand shared postprocess one display-referred RGB float payload. |
| REQ-238 | `src/dng2jpg/dng2jpg.py::_build_bracket_images_from_linear_base_float`, `_normalize_opencv_hdr_to_unit_range`, `_run_opencv_merge_mertens`, `_apply_auto_post_gamma_float`, `_apply_auto_levels_float`, `_postprocess`, `_encode_jpg`; excerpt: explicit main-flow boundaries cover bracket clamp, OpenCV normalization, Mertens `*255.0`, auto-levels entry clip, postprocess exit clip, and final JPEG quantization. |
| REQ-239 | `src/dng2jpg/dng2jpg.py::_build_bracket_images_from_linear_base_float`, `_normalize_opencv_hdr_to_unit_range`, `_build_auto_post_gamma_lut_float`, `_apply_auto_levels_float`, `_postprocess`, `_encode_jpg`; excerpt: boundary helpers document intent plus source/target ranges for clipping, normalization, LUT discretization, and quantization. |
| REQ-240 | `src/dng2jpg/dng2jpg.py::_apply_merge_gamma_float`, `_apply_merge_gamma_float_no_clip`, `_apply_static_postprocess_float`; excerpt: merge gamma remains backend-local while post-gamma remains the shared static postprocess gamma substage. |
| REQ-241 | `src/dng2jpg/dng2jpg.py::_apply_clahe_luma_rgb_float`, `_reconstruct_rgb_from_ycrcb_luma_float`, `_vibrance_hsl_gamma`, `_hlrecovery_cielab_float`; excerpt: YCrCb, HSL, and CIELab-like transforms are confined to display-referred postprocess stages. |
| REQ-242 | `src/dng2jpg/dng2jpg.py::_extract_base_rgb_linear_float`, `_calculate_auto_zero_evaluations`, `_build_bracket_images_from_linear_base_float`, `_apply_auto_white_balance_stage_float`; excerpt: exposure planning, bracket synthesis, and pre-merge auto-white-balance remain camera-linear and do not use YCrCb/HSL/CIELab-like main-flow transforms. |
| REQ-243 | `src/dng2jpg/dng2jpg.py::_apply_validated_auto_adjust_pipeline`; excerpt: repeated display-referred clamp points bracket auto-adjust stage statistics and stage-local arithmetic. |
