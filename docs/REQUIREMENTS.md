...
- **REQ-052**: MUST print deterministic runtime diagnostics for input path, gamma, postprocess factors, backend, EV selections, auto-zero candidate evaluations, and EV triplet.
...
| TST-035 | `tests/test_uint16_postprocess_pipeline.py::test_build_ev_times_from_ev_zero_and_delta_matches_bracket_sequence`, `test_run_opencv_hdr_merge_dispatches_debevec_direct_float_path_with_tonemap`; verifies zero-centered OpenCV exposure reference preserves extracted non-zero `ev_zero` as embedded pixel correction. |
