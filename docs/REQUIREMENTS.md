...
- **REQ-119**: MUST apply a RawTherapee-equivalent float-domain tonal transformation that consumes `expcomp`, `black`, `brightness`, `contrast`, `hlcompr`, and `hlcomprthresh`, including exposure scaling, highlight curve, brightness curve, contrast curve, inverse-gamma output mapping, and default shadow compression `0`.
...
- **REQ-165**: MUST apply `Clip out-of-gamut colors` after the complete auto-levels tonal transformation and optional highlight reconstruction using per-pixel normalization that bounds each overflowing RGB triplet to `1.0` while preserving channel ratios.
...
- **TST-018**: MUST verify auto-levels tonal transformation consumes the RawTherapee-compatible metric set in normalized float space and preserves float-only internal math within the auto-levels stage.
...
| REQ-119 | `src/dng2jpg/dng2jpg.py::_apply_auto_levels_tonal_transform_float`, `_build_auto_levels_tone_curve_state`, `_build_rt_nurbs_curve_lut`; excerpt: ports RawTherapee `complexCurve` semantics into float-domain highlight, shadow, brightness, contrast, and inverse-gamma tone processing. |
